...
/* package */ final class ExoPlayerImplInternal implements Handler.Callback {
  ...
  private boolean mVideoConfigured = false;
  ...
  private void doSomeWork(Message msg) throws ExoPlaybackException {
    ...
    for (int i = 0; i < enabledRenderers.size(); i++) {
      ...
      if (renderer.getType() == renderer.TYPE_AUDIO && renderer.isTunneled()) {
          if (!mVideoConfigured) {
              // We have to configure video codec first on Tunneled-Playback.
              continue;
          }
      }
      ...
      renderer.doSomeWork(positionUs, elapsedRealtimeUs, msg);
      allRenderersEnded = allRenderersEnded && renderer.isEnded();

      if (renderer.getType() == renderer.TYPE_VIDEO && !mVideoConfigured) {
          mVideoConfigured = renderer.isInitialized();
          renderer.setCallbackHandler(handler);
      }
