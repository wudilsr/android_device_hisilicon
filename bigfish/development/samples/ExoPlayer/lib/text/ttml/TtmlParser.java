...
public final class TtmlParser implements SubtitleParser {
  ...
  private TtmlStyle parseStyleAttributes(XmlPullParser parser, TtmlStyle style) {
    ...
    for (int i = 0; i < attributeCount; i++) {
      ...
      String temp = ParserUtil.removeNamespacePrefix(attributeName);
      if (temp.equals(TtmlNode.ATTR_ID)) {
          if (TtmlNode.TAG_STYLE.equals(parser.getName())) {
            style = createIfNull(style).setId(attributeValue);
          }
      } else if (temp.equals(TtmlNode.ATTR_TTS_BACKGROUND_COLOR)) {
      ...
      } else if (temp.equals(TtmlNode.ATTR_TTS_COLOR)) {
          style = createIfNull(style);
          try {
      ...
      } else if (temp.equals(TtmlNode.ATTR_TTS_FONT_FAMILY)) {
          style = createIfNull(style).setFontFamily(attributeValue);
      } else if (temp.equals(TtmlNode.ATTR_TTS_FONT_SIZE)) {
          try {
            style = createIfNull(style);
      ...
      } else if (temp.equals(TtmlNode.ATTR_TTS_FONT_WEIGHT)) {
          style = createIfNull(style).setBold(
              TtmlNode.BOLD.equalsIgnoreCase(attributeValue));
      } else if (temp.equals(TtmlNode.ATTR_TTS_FONT_STYLE)) {
          style = createIfNull(style).setItalic(
              TtmlNode.ITALIC.equalsIgnoreCase(attributeValue));
      } else if (temp.equals(TtmlNode.ATTR_TTS_TEXT_ALIGN)) {
          String temp1 = Util.toLowerInvariant(attributeValue);
          if (temp1.equals(TtmlNode.LEFT)) {
              style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_NORMAL);
          } else if (temp1.equals(TtmlNode.START)) {
              style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_NORMAL);
          } else if (temp1.equals(TtmlNode.RIGHT)) {
              style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_OPPOSITE);
          } else if (temp1.equals(TtmlNode.END)) {
              style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_OPPOSITE);
          } else if (temp1.equals(TtmlNode.CENTER)) {
              style = createIfNull(style).setTextAlign(Layout.Alignment.ALIGN_CENTER);
          }
      } else if (temp.equals(TtmlNode.ATTR_TTS_TEXT_DECORATION)) {
          String temp2 = Util.toLowerInvariant(attributeValue);
          if (temp2.equals(TtmlNode.LINETHROUGH)) {
              style = createIfNull(style).setLinethrough(true);
          } else if (temp2.equals(TtmlNode.NO_LINETHROUGH)) {
              style = createIfNull(style).setLinethrough(false);
          } else if (temp2.equals(TtmlNode.UNDERLINE)) {
              style = createIfNull(style).setUnderline(true);
          } else if (temp2.equals(TtmlNode.NO_UNDERLINE)) {
