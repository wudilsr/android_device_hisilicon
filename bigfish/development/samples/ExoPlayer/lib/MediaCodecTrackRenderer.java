...
public abstract class MediaCodecTrackRenderer extends SampleSourceTrackRenderer {
  ...
  private boolean mSetCallback = false;
  ...
  protected DecoderInfo getDecoderInfo(MediaCodecSelector mediaCodecSelector, String mimeType,
    ...
    if (MimeTypes.isVideo(mimeType)) {
        MediaCodecUtil.setRequiresTunneled(isTunneled());
    }
    MediaCodecUtil.setSoftwareCodec(getSoftwareCodec());
  ...
  protected final void maybeInitCodec() throws ExoPlaybackException {
    ...
    if (drmSessionManager != null && drmInitData == null && isVideo) {
      byte[] schemeUuid = new byte[1];
      DrmInitData.Universal emptyDrmInitData = new DrmInitData.Universal(
          new SchemeInitData(MimeTypes.VIDEO_MP4, schemeUuid));
      drmInitData = emptyDrmInitData;
    }
    ...
    try {
      ...
      android.media.MediaFormat mediaFormat = getFrameworkMediaFormat(format);
      if (isTunneled() && isVideo) {
          setCallback();

          //mediaFormat.setFeatureEnabled(CodecCapabilities.FEATURE_TunneledPlayback, true);
          //int audioSessionId = getAudioSessionId();
          //mediaFormat.setInteger(android.media.MediaFormat.KEY_AUDIO_SESSION_ID, audioSessionId);
          //Log.i(TAG, "mediaFormat.setAudioSessionId: " + audioSessionId);
      }

      TraceUtil.beginSection("configureCodec");
      configureCodec(codec, decoderInfo.adaptive, mediaFormat, mediaCrypto);
      TraceUtil.endSection();
  ...
  @Override
  protected void doSomeWork(long positionUs, long elapsedRealtimeUs, boolean sourceIsReady, Message msg)
      throws ExoPlaybackException {
    ...
    if (codec != null) {
      ...
      if (feedInputBuffer(positionUs, true, msg)) {
        while (feedInputBuffer(positionUs, false, msg)) {}
      }
      TraceUtil.endSection();
    }
   ...
   */
  private boolean feedInputBuffer(long positionUs, boolean firstFeed, Message msg) throws ExoPlaybackException {
    if (inputStreamEnded
    ...
    if (inputIndex < 0) {
      if (mSetCallback) {
        ...
        if (firstFeed && (msg.arg1 == 1/*input buffer*/) && ((msg.arg2 & 0x80) > 0)) {
          inputIndex = msg.arg2 & 0x7F;
          //sampleHolder.data = codec.getInputBuffer(inputIndex);
        } else {
          return false;
        }
      } else {
