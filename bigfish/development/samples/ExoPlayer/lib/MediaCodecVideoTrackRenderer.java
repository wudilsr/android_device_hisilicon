...
public class MediaCodecVideoTrackRenderer extends MediaCodecTrackRenderer {
  ...
  protected void renderOutputBufferV21(MediaCodec codec, int bufferIndex, long releaseTimeNs) {
    maybeNotifyVideoSizeChanged();
    TraceUtil.beginSection("releaseOutputBuffer");
    codec.releaseOutputBuffer(bufferIndex, true/*releaseTimeNs*/);
    TraceUtil.endSection();
    codecCounters.renderedOutputBufferCount++;
    renderedFirstFrame = true;
    maybeNotifyDrawnToSurface();
  ...
  private void maybeSetMaxInputSize(android.media.MediaFormat format, boolean codecIsAdaptive) {
    ...
    if (mimetype.equals(MimeTypes.VIDEO_H263) || mimetype.equals(MimeTypes.VIDEO_MP4V)) {
        maxPixels = maxWidth * maxHeight;
        minCompressionRatio = 2;
    } else if (mimetype.equals(MimeTypes.VIDEO_H264)) {
        ...
        minCompressionRatio = 2;
    } else if (mimetype.equals(MimeTypes.VIDEO_VP8)) {
        // VPX does not specify a ratio so use the values from the platform's SoftVPX.cpp.
        maxPixels = maxWidth * maxHeight;
        minCompressionRatio = 2;
    } else if (mimetype.equals(MimeTypes.VIDEO_H265) || mimetype.equals(MimeTypes.VIDEO_VP9)) {
        maxPixels = maxWidth * maxHeight;
