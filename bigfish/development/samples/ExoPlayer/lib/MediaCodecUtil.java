...
public final class MediaCodecUtil {
  ...
  private static boolean requireTunneled = false;
  private static boolean mSoftwareCodec = false;

  private MediaCodecUtil() {}

  public static void setRequiresTunneled(boolean requires) {
      //Log.i(TAG, "setRequiresTunneled requires: " + requires);
      requireTunneled = requires;
  }

  public static void setSoftwareCodec(boolean software) {
      //Log.i(TAG, "setSoftwareCodec software: " + software);
      mSoftwareCodec = software;
  }
  ...
  public static DecoderInfo getDecoderInfo(String mimeType, boolean secure)
    ...
    Pair<String, CodecCapabilities> info = getMediaCodecInfo(mimeType, secure, requireTunneled);
    if (info == null) {
      return null;
    }
  ...
  public static synchronized void warmCodec(String mimeType, boolean secure) {
    try {
      getMediaCodecInfo(mimeType, secure, false);
  ...
  public static synchronized Pair<String, CodecCapabilities> getMediaCodecInfo(
      String mimeType, boolean secure, boolean requireTunneled) throws DecoderQueryException {
    if (secure || requireTunneled) {
      Log.i(TAG, "getMediaCodecInfo, " + mimeType + " secure: " + secure + " tunneled: " + requireTunneled);
    }

    CodecKey key = new CodecKey(mimeType, secure, requireTunneled, mSoftwareCodec);
      ...
      if (isCodecUsableDecoder(info, codecName, secureDecodersExplicit)) {
        ...
        for (int j = 0; j < supportedTypes.length; j++) {
          ...
          if (supportedType.equalsIgnoreCase(mimeType)) {
            if (mSoftwareCodec && !codecName.startsWith("OMX.google")) {
                Log.i(TAG, "Use software codec, skip " + codecName);
                continue;
            }
            CodecCapabilities capabilities = info.getCapabilitiesForType(supportedType);
            boolean tunneledSupported = false;
            if (key.requireTunneled && MimeTypes.isVideo(mimeType)) {
                tunneledSupported = mediaCodecList.isTunneledPlaybackSupported(key.mimeType, capabilities);
                if (!tunneledSupported) {
                    Log.i(TAG, "Tunneled is required but not supported, skip " + codecName);
                    continue;
                }
            }
            ...
            if (!secureDecodersExplicit) {
              ...
              codecs.put(key.secure ? new CodecKey(mimeType, false, tunneledSupported, mSoftwareCodec) : key,
                  Pair.create(codecName, capabilities));
              if (secure) {
                codecs.put(key.secure ? key : new CodecKey(mimeType, true, tunneledSupported, mSoftwareCodec),
              ...
              codecs.put(key.secure == secure ? key : new CodecKey(mimeType, secure, tunneledSupported, mSoftwareCodec),
                  Pair.create(codecName, capabilities));
            }
            if (codecs.containsKey(key)) {
              return codecs.get(key);
  ...
  public static boolean isSizeSupportedV21(String mimeType, boolean secure, int width,
    ...
    return false/*videoCapabilities != null && videoCapabilities.isSizeSupported(width, height)*/;
  }
  ...
  public static boolean isSizeAndRateSupportedV21(String mimeType, boolean secure,
    ...
    Assertions.checkState(Util.SDK_INT >= 21);
    //MediaCodecInfo.VideoCapabilities videoCapabilities = getVideoCapabilitiesV21(mimeType, secure);
    return false/*videoCapabilities != null
  ...
  public static boolean isH264ProfileSupported(int profile, int level)
      throws DecoderQueryException {
    Pair<String, CodecCapabilities> info = getMediaCodecInfo(MimeTypes.VIDEO_H264, false, false);
    if (info == null) {
      return false;
    }
  ...
  /*private static MediaCodecInfo.VideoCapabilities getVideoCapabilitiesV21(String mimeType,
    ...
    Pair<String, CodecCapabilities> info = getMediaCodecInfo(mimeType, secure, false);
    if (info == null) {
      return null;
    }
  ...
  private interface MediaCodecListCompat {
    ...
    public boolean isTunneledPlaybackSupported(String mimeType, CodecCapabilities capabilities);
  ...
  private static final class MediaCodecListCompatV21 implements MediaCodecListCompat {
    ...
    public MediaCodecListCompatV21(boolean includeSecure) {
      codecKind = 0/*includeSecure ? MediaCodecList.ALL_CODECS : MediaCodecList.REGULAR_CODECS*/;
    }
    ...
    @Override
    public boolean isSecurePlaybackSupported(String mimeType, CodecCapabilities capabilities) {
      boolean supported = false/*capabilities.isFeatureSupported(CodecCapabilities.FEATURE_SecurePlayback)*/;
      //Log.i(TAG, mimeType + " isSecurePlaybackSupported " + supported);
      return supported;
    }

    private void ensureMediaCodecInfosInitialized() {
      if (mediaCodecInfos == null) {
        mediaCodecInfos = null/*new MediaCodecList(codecKind).getCodecInfos()*/;
      }
    }
	...
	public boolean isTunneledPlaybackSupported(String mimeType,
        ...
        boolean supported = false/*capabilities.isFeatureSupported(CodecCapabilities.FEATURE_TunneledPlayback)*/;
        //Log.i(TAG, mimeType + " isTunneledPlaybackSupported " + supported);
        return supported;
	}
  ...
  private static final class MediaCodecListCompatV16 implements MediaCodecListCompat {
    ...
    public boolean isSecurePlaybackSupported(String mimeType, CodecCapabilities capabilities) {
      ...
      // H264 decoder exists.
      return true;//MimeTypes.VIDEO_H264.equals(mimeType);
    }
  ...
  private static final class CodecKey {
    ...
    public final boolean requireTunneled;
    public final boolean mSoftwareCodec;

    public CodecKey(String mimeType, boolean secure, boolean requireTunneled, boolean softwareCodec) {
      this.mimeType = mimeType;
      this.secure = secure;
      this.requireTunneled = requireTunneled;
      this.mSoftwareCodec = softwareCodec;
    }
    ...
    public boolean equals(Object obj) {
      ...
      return TextUtils.equals(mimeType, other.mimeType) && secure == other.secure
        && requireTunneled == other.requireTunneled && mSoftwareCodec == other.mSoftwareCodec;
    }
