...
public final class AudioTrack {
  ...
  private static boolean mIsTunneled = false;
  private static int mAvSyncHeaderRemaining = 0;
  ...
  public int initialize() throws InitializationException {
    return initialize(SESSION_ID_NOT_SET, false);
  }
   ...
   */
  public int initialize(int sessionId, boolean isTunneled) throws InitializationException {
    ...
    if (isTunneled) {
        Log.d(TAG, "channelConfig: " + channelConfig + " sampleRate: " + sampleRate + " bufferSize: " + bufferSize
            + " sessionId: " + sessionId + " encoding: " + encoding);

        mIsTunneled = isTunneled;

        // build AudioTrack using Audio Attributes and FLAG_HW_AV_SYNC
        /*AudioAttributes audioAttributes = nullnew AudioAttributes.Builder()
        ...
        AudioFormat audioFormat = (new AudioFormat.Builder())
                ...
                .build();*/
        audioTrack = null/*new android.media.AudioTrack(audioAttributes, audioFormat, bufferSize,
                android.media.AudioTrack.MODE_STREAM, sessionId)*/;
  ...
  public int handleBuffer(ByteBuffer buffer, int offset, int size, long presentationTimeUs)
    ...
    if (bufferBytesRemaining == 0) {
      ...
      if (Util.SDK_INT < 21) {
        ...
        Log.d(TAG, "bufferBytesRemaining: " + bufferBytesRemaining);
    }*/
    ...
    if (Util.SDK_INT < 21) { // passthrough == false
      ...
      bytesWritten = writeNonBlockingV21(audioTrack, buffer, bufferBytesRemaining, presentationTimeUs*1000);
    }
  ...
  private static int writeNonBlockingV21(
      android.media.AudioTrack audioTrack, ByteBuffer buffer, int size, long pts) {
    //Log.d(TAG, "mIsTunneled: " + mIsTunneled + " position: " + buffer.position() + " last: " + mLastDataWritten + " remaining: " + buffer.remaining() + " size: " + size + " pts: " + pts);
    if (mIsTunneled && buffer.position() == 0 && mLastDataWritten != 0) {
        final int headerSize = 16;
        if (mAvSyncHeaderRemaining == 0) {
            mAvSyncHeader = ByteBuffer.allocate(headerSize);;
            mAvSyncHeader.order(ByteOrder.BIG_ENDIAN);
            mAvSyncHeader.putInt(0x55550001);
            mAvSyncHeader.putInt(size);
            mAvSyncHeader.putLong(pts);
            mAvSyncHeader.position(0);

            mAvSyncHeaderRemaining = headerSize;
        }

        //int headerWritten = audioTrack.write(mAvSyncHeader, mAvSyncHeaderRemaining, android.media.AudioTrack.WRITE_NON_BLOCKING);
        //mAvSyncHeaderRemaining -= headerWritten;
        if (mAvSyncHeaderRemaining > 0) {
            //Log.d(TAG, "avSyncHeader remaining: " + mAvSyncHeaderRemaining);
            return 0;
        } /*else {
            Log.d(TAG, "avSyncHeader write success");
        }*/
    }

    //mLastDataWritten = audioTrack.write(buffer, size, android.media.AudioTrack.WRITE_NON_BLOCKING);
    //Log.d(TAG, "data write: " + mLastDataWritten);
    return mLastDataWritten;
  }
  ...
  private static int getEncodingForMimeType(String mimeType) {
    ...
    else if (mimeType == MimeTypes.AUDIO_E_AC3)
    {
        return C.ENCODING_E_AC3;
    }
    else if (mimeType == MimeTypes.AUDIO_DTS)
    {
        return C.ENCODING_DTS;
    }
    else if (mimeType == MimeTypes.AUDIO_DTS_HD)
    {
        return C.ENCODING_DTS_HD;
    }
    else
