...
public class DemoPlayer implements ExoPlayer.Listener, ChunkSampleSource.EventListener,
  ...
  Context mContext;
  private boolean mResizeWindow = false;
  ...
  public DemoPlayer(RendererBuilder rendererBuilder, Context context) {
    ...
    if (UserInfo.getDiableAudio(context)) {
       player.setSelectedTrack(TYPE_AUDIO, TRACK_DISABLED);
    }
    mContext = context;
    mResizeWindow = UserInfo.getResizeWindow(context);
  }
  ...
  public long getCurrentPosition() {
    ...
    if (mResizeWindow) {
        Point size1 = new Point(640, 480);
        Point size2 = new Point(1280, 720);
        Activity activity = (Activity)mContext;
        SurfaceView surfaceView = (SurfaceView) activity.findViewById(R.id.surface_view);
        ...
        if (position > 20000) {
            ...
            if (params.width == size1.x && params.height == size1.y) {
                params.width = size2.x;
                params.height = size2.y;

                setParams = true;
            }
        } else if (position > 5000) {
            if (params.width != size1.x && params.height != size1.y) {
                params.width = size1.x;
                params.height = size1.y;
        ...
        if (setParams) {
            Log.i(TAG, "Resize window to " + params.width + "x" + params.height);
            SurfaceHolder surfaceHolder = surfaceView.getHolder();
            surfaceHolder.setFixedSize(params.width, params.height);
