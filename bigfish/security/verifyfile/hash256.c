...
void hi_sha256_free( sha256_context *ctx )
{
    ...
    hi_zeroize( ctx, sizeof( sha256_context ) );
}
                    ...
                    size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        hi_sha256_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
...
void hi_sha256_finish( sha256_context *ctx, unsigned char output[32] )
{
    ...
    hi_sha256_update( ctx, sha256_padding, padn );
    hi_sha256_update( ctx, msglen, 8 );
             ...
             unsigned char output[32], int is224 )
{
    ...
    hi_sha256_init( &ctx );
    hi_sha256_starts( &ctx, is224 );
    hi_sha256_update( &ctx, input, ilen );
    hi_sha256_finish( &ctx, output );
    hi_sha256_free( &ctx );
}
                         ...
                         size_t keylen, int is224 )
{
    ...
    if( keylen > 64 )
    {
        hi_sha256( key, keylen, sum, is224 );
        keylen = ( is224 ) ? 28 : 32;
        key = sum;
    }
    ...
    hi_sha256_starts( ctx, is224 );
    hi_sha256_update( ctx, ctx->ipad, 64 );
...
void hi_sha256_hmac_finish( sha256_context *ctx, unsigned char output[32] )
{
    ...
    hi_sha256_finish( ctx, tmpbuf );
    hi_sha256_starts( ctx, is224 );
    hi_sha256_update( ctx, ctx->opad, 64 );
    hi_sha256_update( ctx, tmpbuf, hlen );
    hi_sha256_finish( ctx, output );
...
{
    hi_sha256_starts( ctx, ctx->is224 );
    hi_sha256_update( ctx, ctx->ipad, 64 );
}
                  ...
                  unsigned char output[32], int is224 )
{
    ...
    hi_sha256_init( &ctx );
    hi_sha256_hmac_starts( &ctx, key, keylen, is224 );
    hi_sha256_hmac_update( &ctx, input, ilen );
    hi_sha256_hmac_finish( &ctx, output );
    hi_sha256_free( &ctx );
}
...
int hi_sha256_self_test( int verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        if( j == 2 )
        {
            ...
            for( j = 0; j < 1000; j++ )
                hi_sha256_update( &ctx, buf, buflen );
        }
        else
            hi_sha256_update( &ctx, sha256_test_buf[j],
                                 sha256_test_buflen[j] );

        hi_sha256_finish( &ctx, sha256sum );
    ...
    for( i = 0; i < 14; i++ )
    {
        ...
        if( j == 5 || j == 6 )
        {
            ...
            hi_sha256_hmac_starts( &ctx, buf, buflen, k );
        }
        else
            hi_sha256_hmac_starts( &ctx, sha256_hmac_test_key[j],
                                      sha256_hmac_test_keylen[j], k );

        hi_sha256_hmac_update( &ctx, sha256_hmac_test_buf[j],
                                  sha256_hmac_test_buflen[j] );

        hi_sha256_hmac_finish( &ctx, sha256sum );
