...
HI_S32 main(HI_S32 argc,HI_CHAR**argv)
{
    ...
    HI_TRACE_LOG("\033[0;32m""Please Input the value you want to set:(MSID is 4 Bytes long. Note that every byte should be seperated by space)\n0x""\033[0m");
    
    Ret = getInputValue(4, msid);
    if (HI_SUCCESS != Ret)
    {
    ...
    HI_TRACE_LOG("You want to set MSID to 0x%02x%02x%02x%02x, Confirm?[yes/no]\n", msid[0], msid[1], msid[2], msid[3]);
    ...
    if (1 == userChoice)
    {
        Ret = csdSetMarketSegmentId(msid);
        if (HI_SUCCESS != Ret)
        {
            HI_TRACE_LOG("Set market segment id failed! Ret = %x\n", Ret);
        }
        else
        {
            HI_TRACE_LOG("Set market segment id success!\n");
        }
    }
