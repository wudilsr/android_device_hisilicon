...
//	sleep(1);
    ...
    Ret = HI_UNF_SND_CreateAef(HI_UNF_SND_0, HI_UNF_SND_AEF_TYPE_BASE, (HI_VOID *)(&stOpenParams), &hBaseAef);
    if (Ret != HI_SUCCESS)
    {
        sample_printf("call HI_UNF_SND_CreateAef failed.\n");
        goto TRACK_SLAVE_DESTROY;
    ...
    Ret = HI_UNF_SND_SetAefEnable(hBaseAef, HI_TRUE);
    if (Ret != HI_SUCCESS)
    {
        sample_printf("call HI_UNF_SND_SetAefEnable failed.\n");
        goto TRACK_SLAVE_DESTROY;
      ...
      case '1':
			{
				...
				HI_UNF_SND_SetAefConfig(hBaseAef, HI_AEF_BASE_CONFIG_TYPE_MASTERVOL, &stVol);
				if (Ret != HI_SUCCESS)
				{
			...
			case '3':
			{
				...
				HI_UNF_SND_SetAefConfig(hBaseAef, HI_AEF_BASE_CONFIG_TYPE_BALANCE, &stBalance);
				if (Ret != HI_SUCCESS)
				{
			...
			case '5':
			{
				...
				sample_printf("stVol.bEnable:%d\n",stVol.bEnable);
				break;
			}

			case '6':	
			{		
				  stBalance.bEnable = !stBalance.bEnable;
			    HI_UNF_SND_SetAefConfig(hBaseAef, HI_AEF_BASE_CONFIG_TYPE_BALANCE, &stBalance);
			    if (Ret != HI_SUCCESS)
			    {
			        sample_printf("call HI_UNF_SND_SetAefEnable failed.\n");
				  ...
				  sample_printf("stBalance.bEnable:%d\n",stBalance.bEnable);
				break;
			}	
