...
void* SampleSendFrameRoutine(void* args)
{
    ...
    HI_U32 queue_id  = 0;
    HI_U32 readlen = 0;
    HI_U32 u32Vir2PhyOffset = 0;
    //====================================== MMZ申请内存
    HI_MMZ_BUF_S stBuf;

    //HI_U8 u32TmpData[1920*1088*3/2];
    HI_U8* u32TmpData = malloc(1920 * 1088 * 3);
    if(YUV_PLANNER_420 == YUV_store_type)
    {
        perSize = (ALIGN_UP(pArgs->u32width, 16) + ALIGN_UP(pArgs->u32width / 2, 16) ) * pArgs->u32height;
    }
    else if (YUV_SEMIPLANNER_420_UV == YUV_store_type || YUV_SEMIPLANNER_420_VU ==
        YUV_store_type)
    {
        perSize =  ALIGN_UP(pArgs->u32width, 16) * pArgs->u32height * 3 / 2;
    }
    else if (YUV_PACKAGE_422_YUYV == YUV_store_type || YUV_PACKAGE_422_YVYU ==
             YUV_store_type || YUV_PACKAGE_422_UYVY == YUV_store_type)
    {
        perSize =  ALIGN_UP(pArgs->u32width, 16) * pArgs->u32height * 2;
    }
    else if (YUV_SEMIPLANNER_422_UV == YUV_store_type || YUV_SEMIPLANNER_422_VU ==
             YUV_store_type)
    {
        perSize = ALIGN_UP(pArgs->u32width, 16) * pArgs->u32height * 2;
    }
    ...
    sprintf(stBuf.bufname, "%s", MMZ_name_table[g_MmzID]);
    ...
    g_MmzID ++;
    u32Vir2PhyOffset = (HI_U32)(stBuf.user_viraddr - stBuf.phyaddr);
    //======================================
    ...
    while (!g_bSampleSendFrameStop)
    {
        ...
        if (readlen > 0)
        {
            memcpy((HI_VOID*)(stFrameinfo_release.stVideoFrameAddr[0].u32YAddr + u32Vir2PhyOffset),
                   u32TmpData , perSize);
...
#if 1     /* change the channel attribute activity*/
    HI_S32 channel_id ;
    HI_CHAR inputCmd;
...
#else    /* don't do any change during venc encode*/
    while (cmd_error == 0)
    {
        printf("\nplease input 0 to quit!!\n");
        fflush(stdin);
        scanf("%d", &cmd_id);
...
HI_S32 main(HI_S32 argc, HI_CHAR* argv[])
{
    ...
    if ( 2 == argc )
    {
        if (!strcmp("user-defined", argv[1]))
        {
            ...
            for (i = 0; i < chn_num; i++ )
            {
                printf("input the channel of type[%d] (0->h264 1->mjpeg 2->jpeg)    :", i);
                fflush(stdin);
    ...
    else if (argc == 6)
    {
        ...
        chn_width[0]               = strtol(argv[4], NULL, 0);
        chn_height[0]              = strtol(argv[5], NULL, 0);
        chn_priority[0]            = 0;
        g_JpegType[0]              = 0;
...
error_0:
    printf("\n      Usage1: %s ./filename.yuv  Picture_Width  Picture_Height  Encode_Width  Encode_Height\n", argv[0]);
