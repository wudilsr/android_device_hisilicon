...
static void HelpMsg(void)
{
    ...
    Printf("   -dm, --demo     <type> <OnOff>       Change the demo type(0:sharpness/1:dci/2:color/3:sr/4:all),OnOff(0:off/1:on)\n");
    Printf("   -pe, --pqenable <type> <OnOff>       Enable the pq module(0:sharpness/1:dci/2:color/3:sr/4:all),OnOff(0:off/1:on)\n");
...
HI_S32 setDemo(HI_U32 val, HI_U32 u32OnOff)
{
    ...
    Printf("Set Demo type:%d (0:sharpness/1:dci/2:color/3:sr/4:all)  %s\n", val, u32OnOff ? "on" : "off");
    return s32Ret ;
...
HI_S32 setPQModule(HI_U32 val, HI_U32 u32OnOff)
{
    ...
    Printf("Set PQ Module Enable:%d (0:sharpness/1:dci/2:color/3:sr/4:all)  %s\n", val, u32OnOff ? "on" : "off");
    return s32Ret ;
...
HI_S32 main(int argc, char* argv[])
{
    ...
    if (0 == gSetType)
    {
        ...
        else if (strncmp (gpGetOption, "-a", strlen(gpGetOption)) == 0 || strncmp (gpGetOption, "--all", strlen(gpGetOption)) == 0)
        {
            ...
            //Printf("color temperature:  %d (0:cold/1:middle/2:warm)\n", getColorTemperature((HI_UNF_DISP_E)gChan));
            if (HI_UNF_DISPLAY1 == gChan)
            {
                Printf("dynamic contrast:   %d\n", getDynamicContrast((HI_UNF_DISP_E)gChan));
                //Printf("intelligent color:  %d (0:blue/1:green/2:BG/3:skin/4:vivid/5:disable)\n", getIntelligentColor((HI_UNF_DISP_E)gChan));
            }
            Printf("fleshtone:          %d (0:off/1:low/2:mid/3:high)\n", getfleshtone((HI_UNF_DISP_E)gChan));
            Printf("colormode:          %d (0:recommend/1:blue/2:green/3:BG)\n", getColorEnhanceMode((HI_UNF_DISP_E)gChan));
            Printf("sharpness:          %d\n",  getSharpness());
