...
HI_S32 DOLBY_DrawChnBar(HI_U32 u32WindHandle,CHNBAR_S  stChnbar)
{
	...
	(HI_VOID)DOLBY_ShowText(u32WindHandle, "Mono:", 340, 660, WHITE_COLOR);
	(HI_VOID)DOLBY_ShowText(u32WindHandle, stChnbar.acMonoString, 410, 660, WHITE_COLOR);

	/* display mono track */
	(HI_VOID)DOLBY_ShowText(u32WindHandle, pcTrackModeString[stChnbar.enTrackMode], 500, 660, WHITE_COLOR);

	/* display volume */
	(HI_VOID)DOLBY_ShowText(u32WindHandle, "Volume:", 850, 660, WHITE_COLOR);
	memset(u8VolBuf, 0, 7);
	sprintf(u8VolBuf, "%03d", stChnbar.u32Volume);
...
HI_VOID HI_DOLBY_CreatPassThrough_WIN(HI_U32 u32WindHandle,PASSTHROUGH_S *pstPassProp)
{
	...
	(HI_VOID)DOLBY_ShowText(u32WindHandle, "config pass through:", 400, 100, BACK_COLOR);
	
	(HI_VOID)DOLBY_ShowText(u32WindHandle, "output port", 430, 200, BACK_COLOR);
	(HI_VOID)DOLBY_ShowText(u32WindHandle, " <|", 550, 200, BACK_COLOR);
	(HI_VOID)DOLBY_ShowText(u32WindHandle, " |>", 730, 200, BACK_COLOR);
	
	(HI_VOID)DOLBY_ShowText(u32WindHandle, "audio type:", 430, 300, BACK_COLOR);
	(HI_VOID)DOLBY_ShowText(u32WindHandle, " <|", 550, 300, BACK_COLOR);
	(HI_VOID)DOLBY_ShowText(u32WindHandle, " |>", 730, 300, BACK_COLOR);
	
	stPos.u32x = 580;
	stPos.u32y = 200;
	(HI_VOID)DOLBY_CreateWindMember(u32WindHandle, APP_WMEMB_TRANS, stPos, pstPassProp->acAOPort0, 13);

	stPos.u32x = 580;
	stPos.u32y = 300;
	(HI_VOID)DOLBY_CreateWindMember(u32WindHandle, APP_WMEMB_TRANS_2, stPos, pstPassProp->acPassThrough0, 13);
