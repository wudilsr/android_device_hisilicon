...
static HI_S32 getPlayChnState(int chn, int type)
{
    ...
    sample_common_printf("PVR state:%d, speed:%d\n", status.enState, status.enSpeed);
    sample_common_printf("pos:\n frame/PES:%u, time:%ums, size:%lluByte.\n",
...
{
    sample_common_printf("----------Disk Full while recording!- arg:%p---------\n", args);
}

static void PVR_EventCB_recError(HI_U32 chn, HI_UNF_PVR_EVENT_E type, HI_S32 value, void *args)
{
    sample_common_printf("----------Error occurred while recording!--arg:%p--------\n", args);
}
...
static void PVR_EventCB_playEndOfFile(HI_U32 chn, HI_UNF_PVR_EVENT_E type, HI_S32 value, void *args)
{
    ...
    getPlayChnState(0, 2);
    sample_common_printf("----- time use: %d -----\n", (int)((g_tv1.tv_sec - g_tv0.tv_sec) * 1000000 + (g_tv1.tv_usec - g_tv0.tv_usec)));
    sample_common_printf("----------End of file while playing!---arg:%p-------\n", args);
}
...
static void PVR_EventCB_playStartOfFile(HI_U32 chn, HI_UNF_PVR_EVENT_E type, HI_S32 value, void *args)
{
    ...
    getPlayChnState(0, 2);
    sample_common_printf("----- time use: %d -----\n", (int)((g_tv1.tv_sec - g_tv0.tv_sec) * 1000000 + (g_tv1.tv_usec - g_tv0.tv_usec)));
    sample_common_printf("----------Start of File while playing!----args:%p------\n", args);
}

static void PVR_EventCB_playError(HI_U32 chn, HI_UNF_PVR_EVENT_E type, HI_S32 value, void *args)
{
    getPlayChnState(0, 2);
    sample_common_printf("-----XXXXXXX-----Error occurred while playing!-----XXXXX:%#x, arg:%p-----\n", value, args);
}
...
HI_S32 PVR_Dmx_GetProgram(HI_S32 prognum, PVR_USERDATA_S *puserinfo)
{
    ...
    {
        sample_common_printf("point zero error\n");
        return HI_FAILURE;
    }
    ...
    sample_common_printf( " GetProgram Audio Stream PID = 0x%x[%d]", puserinfo->AElementPid, puserinfo->AElementPid);
    ...
    case HA_AUDIO_ID_DOLBY_PLUS:
        ...
        break;
    default:
        sample_common_printf(" Stream Type Unknow\n");
    }

    sample_common_printf(" GetProgram Video Stream PID = 0x%x[%d] ", puserinfo->VElementPid, puserinfo->VElementPid);
...
HI_S32 PVR_Dmx_AddPid(HI_U32 u32DmxId, int pid, HI_UNF_DMX_CHAN_TYPE_E chnType)
{
    ...
    if ((PVR_DEMUX_PLAY == u32DmxId) || (PVR_DEMUX_PLAYBACK == u32DmxId))
    {
        if (chnType == HI_UNF_DMX_CHAN_TYPE_AUD)
        {
            ...
            if (s32Ret == HI_SUCCESS)
            {
                ...
                sample_common_printf("AUDIO PID USE BY DEMUX %d CHANNEL %x\n", u32DmxId, hChannelHandle);
            }
        ...
        if (chnType == HI_UNF_DMX_CHAN_TYPE_VID)
        {
            ...
            if (s32Ret == HI_SUCCESS)
            {
                HI_UNF_DMX_CloseChannel(hChannelHandle);
                HI_UNF_DMX_DestroyChannel(hChannelHandle);
                sample_common_printf("VIDEO PID USE BY DEMUX %d CHANNEL %x\n", u32DmxId, hChannelHandle);
            }
    ...
    else
    {
        ...
        if (HI_SUCCESS == s32Ret)
        {
            ...
            {
                sample_common_printf("(%d):Pid %d already open for PLAY, can NOT recorde\n", __LINE__, pid);
            }
        }
...
HI_S32 PVR_Dmx_AttachPort(HI_U32 u32DmxId, HI_U32 u32Port , HI_U32 u32BufSize)
{
    ...
    {
        sample_common_printf(" PVR_Dmx_AttachPort input parameter u32DmxId = %d u32Port = %d error \n", u32DmxId, u32Port);
        return HI_FAILURE;
    }
    ...
    sample_common_printf(" +++ demux %d attach to port %d +++ \n", u32DmxId, u32Port);
...
HI_S32 PVR_AvPlay_Create(HI_U32 u32DemuxId)
{
    ...
    sample_common_printf("demux %d create avplay %d video channelid 0x%x , audio channelid 0x%x \n", u32DemuxId, avhandle,
...
static HI_S32 PVR_AvPlay_Destroy(HI_U32 u32DemuxId)
{
    ...
    {
        sample_common_printf("AVPLAY is used by demux %d\n", u32DemuxId);
        return HI_FAILURE;
    }
    ...
    sample_common_printf("destroy pvr avplay %d\n", pvrinfo[u32DemuxId].avHandle);
...
static HI_S32 PVR_Pvr_WriteUserDate(const HI_CHAR *pszFileName, HI_U8 *pInfo, HI_U32 u32UsrDataLen)
{
    ...
    {
        sample_common_printf(" write user data len error \n");
        return HI_FAILURE;
    }
...
static HI_S32 PVR_Pvr_ReadUserDate(const HI_CHAR *pszFileName, HI_U8 *pInfo, HI_U32 u32UsrDataLen)
{
    ...
    if (u32UsrDataLen != sizeof(PVR_USERDATA_S))
    {
        sample_common_printf(" read user data len error \n");
        return HI_FAILURE;
    }
                                  ...
                                  HI_BOOL                 cipher)
{
    ...
    {
        sample_common_printf("FileName len is longer than standard \n");
        return HI_FAILURE;
    }
                                 ...
                                 HI_U32                 usrDataSize)
{
    ...
    sample_common_printf("bRewind:%d\n", pattr->bRewind);
    sample_common_printf("maxsize:%llu\n", pattr->u64MaxFileSize);
...
static HI_S32 PVR_Pvr_GetInfo(HI_S32 demuxId, HIPVR_INFO **ppvrinfo)
{
    ...
    {
        sample_common_printf("demux id error %d \n", demuxId);
        return HI_FAILURE;
    }
...
HI_S32  PVR_Comm_Init(void)
{
    ...
    if (s32Ret != HI_SUCCESS)
    {
        sample_common_printf("call SndInit failed.\n");
        HIADP_Snd_DeInit();
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        sample_common_printf("call DispInit failed.\n");
        HIADP_Snd_DeInit();
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        sample_common_printf("call MPI_AVPlayInit failed.\n");
...
static HI_S32 PVR_USB_Mount(HI_CHAR *devicepathname, HI_CHAR *filepathname, HI_U32 FileSystemType)
{
    ...
    switch(FileSystemType)
    {
        ...
        {
            sample_common_printf("mount usb file system is NTFS \n");

            HIAPI_RUN_RETURN(PVR_Detect_FileExist(devicepathname));

            sample_common_printf("mount %s %s \n", devicepathname, filepathname);
        ...
        case FILESYSTEM_FAT:
        {
            sample_common_printf("mount usb file system is FAT \n");
        ...
        {
            sample_common_printf("mount usb file system is TMPFS \n");

            memset(cmd, 0, sizeof(cmd));
            sprintf(cmd, "mount -t tmpfs tmpfs %s -o size=64m", filepathname);
            sample_common_printf("PVR_USB_Mount %s\n", cmd);
        ...
        default:
        {
            sample_common_printf("mount usb file system type is %d not supported!\n", FileSystemType);
            sRet = HI_FAILURE;
        }
        break;
    }
    ...
    if (HI_SUCCESS == sRet)
    {
        sample_common_printf("mount success \n");
    }
    else
    {
        sample_common_printf("mount failure!\n");
    }
...
HI_VOID PVR_Operate_StopPlay(HIPVR_INFO *ppvrinfo)
{
    ...
    {
        sample_common_printf("play demux is not use \n");
        return;
    }
    ...
    HIAPI_RUN(HI_UNF_SND_Detach(g_hSoundTrack, ppvrinfo->avHandle), s32Ret);

    HI_UNF_SND_DestroyTrack(g_hSoundTrack);
...
HI_S32 PVR_Operate_StartPlay(PVR_USERDATA_S *puserinfo, HIPVR_INFO *ppvrinfo)
{
    ...
    {
        sample_common_printf("not acquirepmt , please <search> program first\n");
        return HI_FAILURE;
    }
    ...
    Ret = HI_UNF_SND_CreateTrack(HI_UNF_SND_0,&stTrackAttr,&g_hSoundTrack);
    if (Ret != HI_SUCCESS)
    {
        return Ret;
    }

    HIAPI_RUN_RETURN_FN(HI_UNF_SND_Attach(g_hSoundTrack, ppvrinfo->avHandle), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
    HIAPI_RUN_RETURN_FN(HI_UNF_VO_AttachWindow(gs_WinMainHandle, ppvrinfo->avHandle), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
    HIAPI_RUN_RETURN_FN(HI_UNF_VO_SetWindowEnable(gs_WinMainHandle, HI_TRUE), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
    sample_common_printf("puserinfo->VElementNum = %d, puserinfo->AElementNum=%d\n", puserinfo->VElementNum, puserinfo->AElementNum);

    if (puserinfo->VElementNum > 0)
    {
        u32Pid = puserinfo->VElementPid;
        HIAPI_RUN_RETURN_FN(HIADP_AVPlay_SetVdecAttr(ppvrinfo->avHandle,puserinfo->VideoType,HI_UNF_VCODEC_MODE_NORMAL), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
        HIAPI_RUN_RETURN_FN(HI_UNF_AVPLAY_SetAttr(ppvrinfo->avHandle,HI_UNF_AVPLAY_ATTR_ID_VID_PID,&u32Pid), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
        if (puserinfo->PcrPid != 0)
        {
            HIAPI_RUN_RETURN_FN(HI_UNF_AVPLAY_GetAttr(ppvrinfo->avHandle, HI_UNF_AVPLAY_ATTR_ID_SYNC, &synctype), HI_UNF_SND_DestroyTrack(g_hSoundTrack));

            synctype.enSyncRef = HI_UNF_SYNC_REF_NONE;

            HIAPI_RUN_RETURN_FN(HI_UNF_AVPLAY_SetAttr(ppvrinfo->avHandle, HI_UNF_AVPLAY_ATTR_ID_SYNC, &synctype), HI_UNF_SND_DestroyTrack(g_hSoundTrack));

            HIAPI_RUN_RETURN_FN(HI_UNF_AVPLAY_SetAttr(ppvrinfo->avHandle, HI_UNF_AVPLAY_ATTR_ID_PCR_PID,
                                                   &puserinfo->PcrPid), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
        }

        HIAPI_RUN_RETURN_FN(HI_UNF_AVPLAY_Start(ppvrinfo->avHandle, HI_UNF_AVPLAY_MEDIA_CHAN_VID, NULL), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
    }

    if (puserinfo->AElementNum > 0)
    {
        u32Pid = puserinfo->AElementPid;
        HIAPI_RUN_RETURN_FN(HIADP_AVPlay_SetAdecAttr(ppvrinfo->avHandle,puserinfo->AudioType,HD_DEC_MODE_RAWPCM,1), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
        HIAPI_RUN_RETURN_FN(HI_UNF_AVPLAY_SetAttr(ppvrinfo->avHandle,HI_UNF_AVPLAY_ATTR_ID_AUD_PID,&u32Pid), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
        HIAPI_RUN_RETURN_FN(HI_UNF_AVPLAY_Start(ppvrinfo->avHandle, HI_UNF_AVPLAY_MEDIA_CHAN_AUD, NULL), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
    }

    ppvrinfo->useflag = HI_TRUE;
    sample_common_printf(" +++ PVR_Operate_StartPlay +++\n");
    return HI_SUCCESS;
}
                                   ...
                                   const HI_CHAR * pszFileName)
{
    ...
    {
        sample_common_printf(" demux %d is working \n", ppvrinfo->DemuxID);
        return HI_SUCCESS;
    }
    ...
    ppvrinfo->useflag = HI_TRUE;
    sample_common_printf("Rec chn OK:%d\n", recChn);
...
HI_S32 PVR_Operate_StopRec(HIPVR_INFO *ppvrinfo)
{
    if (ppvrinfo == HI_NULL)
    {
        sample_common_printf("pvr info error\n");
        return HI_FAILURE;
    }
    ...
    sample_common_printf("Rec chn Stop OK:%d\n", ppvrinfo->recchannelID);
    ppvrinfo->useflag = HI_FALSE;
...
HI_S32 PVR_Operate_StartRePlay(PVR_USERDATA_S *puserinfo, HIPVR_INFO *ppvrinfo, const HI_CHAR *pszFileName)
{
    ...
    {
        sample_common_printf(" point zero \n");
        return HI_FAILURE;
    }

    /*if (ppvrinfo->useflag == HI_TRUE)
    {
        sample_common_printf(" replay is now working \n");
        return HI_FAILURE;
    }*/
    ...
    if (puserinfo->VElementNum > 0)
    {
        sample_common_printf("====vid pid=0x%x\n", puserinfo->VElementPid);
        u32Pid = puserinfo->VElementPid;
    ...
    if (puserinfo->AElementNum > 0)
    {
        sample_common_printf("====aus pid=0x%x\n", puserinfo->AElementPid);
        u32Pid = puserinfo->AElementPid;
    ...
    s32Ret = HI_UNF_SND_CreateTrack(HI_UNF_SND_0,&stTrackAttr,&g_hSoundTrack);
    if (s32Ret != HI_SUCCESS)
    {
        return s32Ret;
    }

    HIAPI_RUN_RETURN_FN(HI_UNF_SND_Attach(g_hSoundTrack, hAv), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
    HIAPI_RUN_RETURN_FN(HI_UNF_VO_AttachWindow(gs_WinMainHandle, hAv), HI_UNF_SND_DestroyTrack(g_hSoundTrack));
    HIAPI_RUN_RETURN_FN(HI_UNF_VO_SetWindowEnable(gs_WinMainHandle, HI_TRUE), HI_UNF_SND_DestroyTrack(g_hSoundTrack));

    //create pvr replay channel
    PVR_Pvr_SetPlayAttr(&attr, pszFileName, puserinfo->scrambled, puserinfo->cipher);
    HIAPI_RUN_RETURN_FN(HI_UNF_PVR_PlayCreateChn(&playChn, &attr, hAv, portinfo[ppvrinfo->PortID].hTsHandle), HI_UNF_SND_DestroyTrack(g_hSoundTrack));

    //sample_common_printf("new play chn%d ok\n", playChn);

    /**/

    //HIAPI_RUN(HI_UNF_PVR_PlayGetFileAttr(playChn, &fileAttr),s32Ret);
    //sample_common_printf("file attr: frame/PES:%u, time:%ums, size:%lluByte.\n",
    //       fileAttr.u32FrameNum, fileAttr.u32PlayTimeInMs, fileAttr.u64ValidSizeInByte);

    /**/

    //HIAPI_RUN(HI_UNF_PVR_PlayGetFileAttr(playChn, &fileAttr),s32Ret);
    //sample_common_printf("play size:%llu \n", fileAttr.u64ValidSizeInByte);

    /* start to play channel */
    s32Ret = HI_UNF_PVR_PlayStartChn(playChn);
    if (s32Ret != HI_SUCCESS)
    {
        HI_UNF_SND_DestroyTrack(g_hSoundTrack);
        return HI_FAILURE;
    }
    sample_common_printf("start play chn%d ok\n", playChn);
    sample_common_printf("attr: clr:%d, cipher:%d\n", attr.bIsClearStream, attr.stDecryptCfg.bDoCipher);
...
HI_S32 PVR_Operate_PlayResumeChn(HIPVR_INFO *ppvrinfo)
{
    ...
    if (ppvrinfo->useflag != HI_TRUE)
    {
        sample_common_printf("pvr channel %d not use \n", ppvrinfo->DemuxID);
        return HI_FAILURE;
    }

    if ((ppvrinfo->DemuxID != PVR_DEMUX_PLAY) && (ppvrinfo->DemuxID != PVR_DEMUX_PLAYBACK))
    {
        sample_common_printf("pvr channel not play channel %d\n", ppvrinfo->DemuxID);
        return HI_FAILURE;
    }

    if (ppvrinfo->playchannelID == INVAILD_HANDLE_VALUE)
    {
        sample_common_printf("pvr channel %d not replay file\n", ppvrinfo->DemuxID);
        return HI_FAILURE;
    }
...
HI_S32 PVR_Operate_PlaySeek(HIPVR_INFO *ppvrinfo, HI_S32 type, HI_S32 offset, HI_S32 whence)
{
    ...
    sample_common_printf("-----------------------------begin seek play ------------------------------\n");
    sample_common_printf("HI_UNF_PVR_PlaySeek ok: whence:%d, offset:%d\n", whence, offset);

    /* find out the playing position */
    HIAPI_RUN(HI_UNF_PVR_PlaySeek(ppvrinfo->playchannelID, &stCurrentPlayPos), s32Ret);
    getPlayChnState(0, 1);
    sample_common_printf("-----------------------------end seek play ------------------------------\n");
    return HI_SUCCESS;
}
...
HI_S32 PVR_Operate_TrickMode(HIPVR_INFO *ppvrinfo, HI_S32 speed)
{
    ...
    sample_common_printf("-----------------------------begin modify tick ------------------------------\n");

    /* get the player state */
    HIAPI_RUN(HI_UNF_PVR_PlayGetStatus(ppvrinfo->playchannelID, &status), s32Ret);
    sample_common_printf("now play status %d , speed %d \n", status.enState, status.enSpeed);

    mode.enSpeed = speed;
    sample_common_printf("modify to speed %d \n", speed);
...
HI_S32 ParseArg_Rec(HI_S32 argc, HI_BOOL bStartStop)
{
    ...
    if (bStartStop == HI_TRUE)
    {
        ...
        {
            sample_common_printf("only demux 2 and 3 support to pvr rec ,please modify demux id \n");
            return HI_FAILURE;
        }
    ...
    else
    {
        ...
        if ((demuxid != PVR_DEMUX_REC_0) && (demuxid != PVR_DEMUX_REC_1))
        {
            printf("only demux 2 and 3 support to pvr rec ,please modify demux id \n");
            return HI_FAILURE;
        }
...
HI_S32 ParseArg_Timethift(HI_S32 argc, HI_BOOL bStartStop)
{
    ...
    if (bStartStop == HI_TRUE)
    {
        ...
        {
            sample_common_printf("playback is used , time thift start failure \n");
            return HI_FAILURE;
        }
...
HI_S32 ParseArg_RePlay(HI_S32 argc, HI_BOOL bStartStop)
{
    ...
    if (bStartStop == HI_TRUE)
    {
        ...
        {
            sample_common_printf( " %s Audio Stream PID = 0x%x[%d] ", args[1], usrdata.AElementPid, usrdata.AElementPid);
            ...
            sample_common_printf(" %s Video Stream PID = 0x%x[%d] ", args[1], usrdata.VElementPid, usrdata.VElementPid);
            ...
            case HI_UNF_VCODEC_TYPE_H264:
                ...
                break;
            case HI_UNF_VCODEC_TYPE_MPEG2:
                sample_common_printf(" Stream Type MP2\n");
                break;
            case HI_UNF_VCODEC_TYPE_MPEG4:
                sample_common_printf(" Stream Type MP4\n");
                break;
            default:
                sample_common_printf(" Stream Type Unknow\n");
            }
...
static void ParseArg(int argc)
{
    ...
    for (i = 0; i < argc; i++)
    {
        sample_common_printf("argv[%d]:%s, ", i, args[i]);
    }

    sample_common_printf("argc:%d\n", argc);
...
HI_S32 main_argparse(int argc, char *argv[])
{
    ...
    if (m == HI_TRUE)
    {
        ...
        {
            printf("ip port select error %d  please input 3~4\n", u32TunerPort);
            return HI_FAILURE;
        }
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        printf("PVR# ");
#ifndef ANDROID
        if (!SAMPLE_GET_INPUTCMD(s8szPVRBuf))
        {
            sleep(1);
            continue;
        }
#else
        SAMPLE_GET_INPUTCMD(s8szPVRBuf);
#endif        

        pszPVRCmd = (HI_S8 *)s8szPVRBuf;
