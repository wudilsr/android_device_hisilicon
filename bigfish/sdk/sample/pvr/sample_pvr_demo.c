...
#define HISILICON_LOGO_PATH "./res/jpg/logo.jpg"
...
HI_S32 PVR_DrawGraphic(const HI_CHAR *pszFileName, HI_RECT_S *pstRect)
{
    ...
    stRectTmp.x = pstRect->s32X + 5;
    stRectTmp.y = pstRect->s32Y + 5;
    stRectTmp.w = pstRect->s32Width - 10;
    stRectTmp.h = pstRect->s32Height - 10;
...
HI_S32 PVR_DrawSelectRect(HI_RECT_S *pstRect, HI_BOOL bVisible, HI_BOOL bVedioExist)
{
    ...
    {
        PVR_DrawGraphic(HISILICON_LOGO_PATH, pstRect);
    }
...
HI_S32 PVR_AddAPlayBack(const HI_CHAR *pcFileName)
{
    ...
    {
        (void)pthread_mutex_lock(&g_stDrawRectMutex);
        PVR_DrawGraphic(HISILICON_LOGO_PATH, &pstPlayChnAttr->stRect);
        (void)pthread_mutex_unlock(&g_stDrawRectMutex);
    }
...
HI_S32 PVR_AddALivePlay(HI_U32 u32ProgramNum)
{
    ...
    {
        pstTimeshiftChnAttr->u32DmxPort = HI_UNF_DMX_PORT_IF_0;
    }
    else
    {
    ...
    {
        (void)pthread_mutex_lock(&g_stDrawRectMutex);
        PVR_DrawGraphic(HISILICON_LOGO_PATH, &pstTimeshiftChnAttr->stRect);
        (void)pthread_mutex_unlock(&g_stDrawRectMutex);
    }
                            ...
                            PVR_TEST_MODE enTestMode)
{
    ...
    if(0 == g_hEthSocketRecvTsBuff)
    {
        s32Ret = HI_UNF_DMX_AttachTSPort(u32RecDemuxId, HI_UNF_DMX_PORT_IF_0);
    }
    else
    {
...
HI_S32 PVR_ProcZoominScreen(HI_HANDLE hWindow)
{
    ...
    for(i = 0; i < PVR_MAX_PLAY_CHN; i++)
    {
        ...
        if (0 != g_stTimeShiftChnAttr[i].hWindow)
        {
        ...
        if (0 != g_stPlayChnAttr[i].hWindow)
        {
    ...
    switch(u32TestMode)
    {
        case PVR_TEST_MODE_TIMESHIFT:
            ...
            else
            {
                ...
                s32Ret = HI_UNF_DMX_AttachTSPort(DEMUX_ID_0, HI_UNF_DMX_PORT_IF_0);
                if (s32Ret != HI_SUCCESS)
                {
                    sample_common_printf("call HI_UNF_DMX_AttachTSPort failed. ret=%#x dmxid=%d port=%d\n", s32Ret, DEMUX_ID_0, HI_UNF_DMX_PORT_IF_0);
                    return -1;
                }
