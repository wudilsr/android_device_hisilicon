...
static HI_S32 HIADP_Dma_Open(HI_VOID)
{
    ...
    if (g_fdDma < 0)
    {
        sample_common_printf("can't open DMA\n");
        return -1;
    }
...
static HI_S32 HIADP_TLV320_Open(HI_U32 I2S_Mode)
{
    ...
    {
        sample_common_printf("can't open AIC31\n");
        return -1;
    }
...
HI_S32 HIADP_SLIC_Open(HI_VOID)
{
    ...
    if (g_fdSlic < 0)
    {
        sample_common_printf("can't open SLIC le89116\n");
        return -1;
    }
...
static HI_S32 HIADP_AiAo_Open( HI_UNF_SAMPLE_RATE_E enSamplerate, HI_U32 u32SamplePerFrame)
{
	...
	if (HI_SUCCESS != nRet)
    {
        sample_common_printf("chip not support or GetVersion err:0x%x\n", nRet);
        return nRet;
    }
#ifdef SLIC_AUDIO_DEVICE_ENABLE
#define ENA_PRIVATE_CLK
    ...
    if (HI_SUCCESS != nRet)
    {
        sample_common_printf("set ai %d attr err:0x%x\n", g_AiaoDevId, nRet);
        return nRet;
    }
    ...
    if (HI_SUCCESS != nRet)
    {
        sample_common_printf("enable ai dev %d err:0x%x\n", g_AiaoDevId, nRet);
        return nRet;
    }
    ...
    if (HI_SUCCESS != nRet)
    {
        sample_common_printf("enable ai chn %d err:0x%x\n", g_AiCh, nRet);
        return nRet;
    }
    ...
    if (HI_SUCCESS != nRet)
    {
        sample_common_printf("set ao %d attr err:0x%x\n", g_AiaoDevId, nRet);
        return nRet;
    }
    ...
    if (HI_SUCCESS != nRet)
    {
        sample_common_printf("enable ao dev %d err:0x%x\n", g_AiaoDevId, nRet);
        return nRet;
    }
    ...
    if (HI_SUCCESS != nRet)
    {
        sample_common_printf("enable ao chn %d err:0x%x\n", g_AoCh, nRet);
        return nRet;
    }
                       ...
                       HI_U32 u32SamplePerFrame)
{
    ...
    if (HI_SUCCESS != Ret)
    {
        sample_common_printf("TLV320 open err:0x%x\n", Ret);
        return Ret;
    }
#endif
    ...
    if (HI_SUCCESS != Ret)
    {
        sample_common_printf("dma open err:0x%x\n", Ret);
        return Ret;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        sample_common_printf(" aiao(%d) open err:0x%x\n", g_AiaoDevId, Ret);
        return Ret;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        sample_common_printf("Le89116 open err:0x%x\n", Ret);
        return Ret;
    }
#endif


    sample_common_printf("HIADP_AIAO_Init OK\n");
...
#ifdef SLIC_AUDIO_DEVICE_ENABLE
	sample_common_printf("SLIC Reset\n");
#else
	sample_common_printf("TLV320 Reset\n");
#endif
