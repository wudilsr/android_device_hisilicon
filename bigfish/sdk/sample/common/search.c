...
HI_S32 GetAllPmtTbl(HI_U32 u32DmxId, PMT_COMPACT_TBLS **pProgTbl)
{
    ...
    {
        sample_common_printf("PSISI Error: Fail to get ALL PMT table, get PAT table fail!\n");
        return s32Ret;
    }
    ...
    if (g_prog_tbl.proginfo == HI_NULL_PTR)
    {
        ...
        sample_common_printf("PSISI Error: Fail to get ALL PMT table, ALL PMT table alloc fail!\n");
        return HI_FAILURE;
    }
    ...
    for (i = 0; i < pat_tbl->ProgNum; i++)
    {
        ...
        if (s32Ret != HI_SUCCESS)
        {
            sample_common_printf("TS Error: Fail to get PMT table of program 0x%x!\n", prog_info->ProgID);
            continue;
        }
        ...
        while (pmt_stream)
        {
            ...
            case VIDEO_STREAM_HEVC:
                ...
                {
                    g_prog_tbl.proginfo[u32Promnum].VElementPid = pmt_stream->ElementPid;
                    g_prog_tbl.proginfo[u32Promnum].VideoType = HI_UNF_VCODEC_TYPE_HEVC;
                }
            ...
            default:
                sample_common_printf("Unsuport stream type %d !!!\n", pmt_stream->StreamType );
                break;
            }
    ...
    {
        sample_common_printf("no Program searched!\n");
        return -1;
    }
    ...
    {
        sample_common_printf("Channum = %d, Program ID = %d, Channel Num = %d\n", u32DmxId, g_prog_tbl.proginfo[i].ProgID, i + 1);
