...
3、注意事项
	...
	3、关于高安CA不能烧写的问题工具端已做了兼容性修改更新，但必须附带BurnConfig.ini(其内配置项BoardType配置为1)和ProgrammerforCA.bin（由客户提供有烧录命令的boot版本，默认是Hi3716CV110_tst板的样例）两个文件。其它烧写不需要ProgrammerforCA.bin文件，BurnConfig.ini文件必须将其内配置项BoardType配置为0。
