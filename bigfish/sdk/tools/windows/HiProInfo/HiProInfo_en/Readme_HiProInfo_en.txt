...
3¡¢Notes	
	...
	5. In order to support CA boards, users must provide two files named "BurnConfig.ini" (with the BoardType parameter set to 1) and "ProgrammerforCA.bin". "ProgrammerforCA.bin" file is not needed when burning NON-CA boards, and if users provide "BurnConfig.ini" file, the BoardType parameter must be set to 0.
