...
static HI_U32  Loader_ReadUpgrdTag(UPGRD_TAG_PARA_S *pstTagParam);
static HI_U32  Loader_WriteUpgrdTag(UPGRD_TAG_PARA_S *pstTagParam);
...
int Customer_Boot(HI_FLASH_TYPE_E enFlashType, HI_CHAR *pPartName, HI_U64 Address, HI_U64 Len)
{
	...
	do
	{
		...
		HI_OSAL_Snprintf(cmdStr, sizeof(cmdStr), "bootm 0x%x", (HI_U32)pImage);
...
static int LoaderCopyPartition(HI_HANDLE hFrom, HI_HANDLE hTo, unsigned long long len, int withOOB)
{
	...
	HI_Flash_Erase(hTo, 0, u32EraseSize);
	HI_Flash_Write(hTo, 0, (HI_U8*)g_buffer, len, u32FlashMode);
...
{
	uImage_header_t* hdr1, *hdr2;
	unsigned int c1, c2, cc1, cc2;
	char* buf1, *buf2;
	...
	do
	{
		...
		if (bakupExist)
		{
			if ((r1 < 0) || (r2 < 0))
			{
				...
				u32MainLoaderRWSize = ((hdr1->ih_size + IMAGE_HEAD_SIZE) & 0xfffc0000) + 0x40000;
				u32BakLoaderRWSize = ((hdr2->ih_size + IMAGE_HEAD_SIZE) & 0xfffc0000) + 0x40000;
...
HI_S32 Loader_Main()
{
	...
	pRet = Loader_Check();
	if ((HI_NULL != pRet) && (0 == HI_OSAL_Strncmp(pRet, "loader", sizeof("loader"))))
	{
...
HI_U32 Loader_ReadUpgrdTag(UPGRD_TAG_PARA_S *pstTagParam)
{
	...
	else
	{
		...
		HI_Flash_Read(hMainHandle, 0, au8Buf1, u32RegionSize1, HI_FLASH_RW_FLAG_RAW);
	...
	else
	{
		...
		/* NOT check return value for badblock */
		HI_Flash_Read(hBakHandle, 0, au8Buf2, u32RegionSize2, HI_FLASH_RW_FLAG_RAW);
		...
		if (LOADER_MAGIC_NUMBER == pstParamHead->u32MagicNumber)
		{
			if (pstParamHead->u32Length < u32RegionSize2)
			{
				...
				if (u32Crc == pstParamHead->u32Crc)
				{
					...
					HI_Flash_Write(hMainHandle, 0, au8Buf1, u32RegionSize1, HI_FLASH_RW_FLAG_ERASE_FIRST);
