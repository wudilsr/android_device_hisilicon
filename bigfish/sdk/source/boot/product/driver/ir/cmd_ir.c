...
int do_ir(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (!strncmp(cmd, "start", 5)) {
		printf("Trying open ir module!\n");
		return HI_UNF_IR_Init();
	}
	else if (!strncmp(cmd, "stop", 4)) {
		printf("Trying close ir module!\n");
		return HI_UNF_IR_DeInit();
	}
	else if (!strncmp(cmd, "reset", 5)) {
		printf("Trying reset ir module!\n");
		...
		if (!strncmp(argv[2], "up", 2)) {
			...
			if (val !=0 && val != 1)
				goto usage;
			printf("Trying set up event to %lu.\n",val);
			...
			if (val !=0 && val != 1)
				goto usage;
			printf("Trying set hold event to %lu.\n",val);
			return HI_UNF_IR_EnableRepKey(val);
		}
		else if (!strncmp(argv[2], "hi", 2)) {
			val = simple_strtoul(argv[3], NULL, 16);
			printf("Trying set hold interval to %lu ms\n", val);
		...
		if (HI_UNF_IR_GetProtocolEnabled(name, &enable)) {
			printf("Fail to get enable status of '%s'\n",name);
			return 1;
		}
		printf("Get status of %s is %s\n", name, enable? "Enable" : "Disable");
		return 0;
	}
		...
		for (i = 0, received = 0; i < cnt; i++) {
			...
			if (!ret) {
				printf("Received key: 0x%.08llx, %4s,\tprotocol: %s.\n",
			...
			if (tstc()) {
				if (getc()  == 0x3) {
					printf("^C detected, abort!\n");
					return 0;
				}
			}
		}
		printf("Total receive %lu keys!\n", received);
		return 0;
	}
usage:
	cmd_usage(cmdtp);
