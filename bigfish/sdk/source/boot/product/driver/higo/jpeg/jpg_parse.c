...
extern HI_VOID JPGSetFactor(JPG_COMPONENT_INFO *compinfo);
#endif
...
STATIC_FUNC HI_VOID  JPGPARSEReleaseTable(JPG_PICPARSEINFO_S* pPicInfo)
{
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        {
            //VCOS_free(pPicInfo->pQuantTbl[i]);
            //s_ParseMem.Offset -= sizeof(JPG_QUANT_TBL);
            pPicInfo->pQuantTbl[i] = NULL;
        }

        if(NULL != pPicInfo->pDcHuffTbl[i])
        {
            //VCOS_free(pPicInfo->pDcHuffTbl[i]);
            //s_ParseMem.Offset -= sizeof(JPG_HUFF_TBL);
            pPicInfo->pDcHuffTbl[i] = NULL;
        }

        if(NULL != pPicInfo->pAcHuffTbl[i])
        {
            //VCOS_free(pPicInfo->pAcHuffTbl[i]);
            //s_ParseMem.Offset -= sizeof(JPG_HUFF_TBL);
...
HI_VOID JPGPARSEUpdateExif(JPG_CYCLEBUF_S *pParseBuf, JPGPARSE_APP1_S* pstAPP1, 
                           JPGPARSE_APP1_S* pstExif, HI_BOOL bExifEnd)
{
    ...
    if((u32ExifLen >= 64) || (HI_TRUE == bExifEnd))
    {
        ...
        else
        {
            ...
            VCOS_memcpy(pExifAddr + stData.u32Len[0], stData.pAddr[1], 
...
HI_S32 JPGPARSE_CreateInstance(JPG_HANDLE *pHandle, JPG_IMGTYPE_E ImgType)
{
    ...
    JPG_ASSERT_NULLPTR(pHandle) ;
        ...
        /*·µ»Ø HI_ERR_JPG_NO_MEM*/
    //pParseCtx = (JPGPARSE_CTX_S*)VCOS_malloc(sizeof(JPGPARSE_CTX_S) + JPG_JOINTSPACE_LEN);
...
* Description:   DQT SOS DHT SOF Mark
* Input:         pParseCtx 
*                pParseBuf BUF
*                Marker    Marker
*                Index     
    ...
    JPG_CYCLEBUF_S *pParseBuf, HI_U8 Marker, HI_U32 Index)
{
    ...
    {   
        ...
        VCOS_memcpy((HI_U8*)pParseCtx->pJointSpace + stData.u32Len[0], stData.pAddr[1],
                                          ...
                                          HI_U8 Marker, HI_U32 Index)
{
    ...
    switch (Marker)
    {
        ...
        {   
            ...
            //pPicInfo = (JPG_PICPARSEINFO_S *)VCOS_malloc(sizeof(JPG_PICPARSEINFO_S));
    ...
    {     
            ...
            && (HI_FALSE == pstExif->bActive))
        {
            ...
            if(ExifRead == JPG_EXIF)
            {
                ...
                //pExifAddr = VCOS_malloc(pstAPP1->APP1MarkLen + 2);
