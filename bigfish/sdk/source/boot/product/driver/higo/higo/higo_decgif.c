...
******************************************************************************
   ...
   Created                    :   2008/05/29
   Last Modified            :
   Description               :   T-Bag is cool man! HAHO!Break Prision!
...
#ifdef TEST_IN_ROOTBOX    
    gifIoInfo.IoInfo.pFileName = (const HI_CHAR *)pSrcDesc->SrcInfo.pFileName;
#endif    
            ...
            case 0xf9:         
                ...
                while (pGifInstance->gifImageCount < GIF_MAX_IMAGE)
                {
                    ...
                    if (be == 4)
                    {
                        ...
                        GIF_READ(pGifInstance->gifIo, (HI_U8 *)&(pf->ctrlExt.delayTime), 2);
                        GIF_READ(pGifInstance->gifIo, &be, 1);
...
static HI_S32 gif_ExtractData( GIF_DECODER_INSTANCE *pGifInstance, HI_U32 image_index )
{
    ...
    len = f->imageWidth;    
#ifdef TEST_IN_ROOTBOX
    f->dataBuf = (HI_U8 *)gif_Malloc(GIF_MALLOC_SYS, (HI_U32)(len * height));
#else
...
static HI_VOID* gif_Malloc( HI_U32 flag, HI_U32 size )
{
    ...
    {
#ifdef TEST_IN_ROOTBOX
        p = HIGO_MMZ_Malloc(size, HIGO_MMZ_DEFAULT);
#else
