...
static int check_gpio_recovery(void)
{
    ...
    switch(chipid){
        case _HI3716C_V200:
            ...
            /* get gpio5_1 buttom */
            while((((REG_VAL(0xF80043FC) & 0x02)>>1) == 0) && (count > 0)){
                udelay(1000*1000);
                count--;
            }
#endif
	...
	else 
	{
		...
		switch (pFlashInfo.FlashType)
		{
			...
			case HI_FLASH_TYPE_NAND_0:
				...
				buf1 = (unsigned char *)malloc(len * sizeof(unsigned char));
				if (buf1 == NULL) {
				...
				ret = HI_Flash_Read(flashhandle, FASTBOOT_MISC_OFFSET, buf, len, HI_FLASH_RW_FLAG_RAW);
				if(ret == HI_FAILURE) {
				...
				ret = HI_Flash_Read(flashhandle, erasesize, buf1, len, HI_FLASH_RW_FLAG_RAW);
				if(ret == HI_FAILURE) {
					printf("HI_Flash_Read partion %s error\n", FASTBOOT_MISC_NAME);
					free(buf);
					free(buf1);
...
int load_recovery_image(void)
{
	...
	ret = HI_Flash_Read(flashhandle, FASTBOOT_RECOVERY_OFFSET, addr, pFlashInfo.PartSize, HI_FLASH_RW_FLAG_RAW);
	if(ret == HI_FAILURE) {
		printf("HI_Flash_Read partion %s error\n", FASTBOOT_RECOVERY_NAME);
		HI_Flash_Close(flashhandle);
		return HI_FAILURE;
...
void load_recovery(void)
{
    ...
    if(getenv("recoverybootargs")) {
        recovery_boot_args = (char *)malloc(strlen(getenv("recoverybootargs"))+1);
