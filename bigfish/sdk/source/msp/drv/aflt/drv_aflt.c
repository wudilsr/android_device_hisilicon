...
static HI_S32 AFLT_Chan_AllocHandle(HI_HANDLE *phAflt, struct file *pstFile, AFLT_CREATE_ATTR_S  *pstCreateAttr)
{
    ...
    for (i = 0; i < AFLT_MAX_CHAN_NUM; i++)
    {
        if (0 == atomic_read(&s_stAfltDrv.astChanEntity[i].atmUseCnt))
        {
            ...
            s_stAfltDrv.astChanEntity[i].u32File = (HI_U32)pstFile;
            s_stAfltDrv.u32ChanNum++;
...
static HI_S32 AFLT_Chan_FreeHandle(HI_HANDLE hAflt)
{
    ...
    s_stAfltDrv.astChanEntity[u32AfltId].u32File = (HI_U32)HI_NULL;
    s_stAfltDrv.u32ChanNum--;
...
static HI_S32 AFLT_SetParam(HI_HANDLE hAflt, AFLT_PARAM_S stParam)
{
    ...
    s32Ret = copy_from_user((HI_VOID *)pstChan->stMsgPoolMmz.u32StartVirAddr, stParam.pParamStruct, stParam.u32ParamStructSize);
...
static HI_S32 AFLT_GetParam(HI_HANDLE hAflt, AFLT_PARAM_S *pstParam)
{
    ...
    s32Ret = copy_to_user(pstParam->pParamStruct, (HI_VOID *)pstChan->stMsgPoolMmz.u32StartVirAddr, pstParam->u32ParamStructSize);
...
static HI_S32 AFLT_SetConfig(HI_HANDLE hAflt, AFLT_CONFIG_S stConfig)
{
    ...
    s32Ret = copy_from_user((HI_VOID *)pstChan->stMsgPoolMmz.u32StartVirAddr, stConfig.pConfigStruct, stConfig.u32ConfigStructSize);
...
static HI_S32 AFLT_GetConfig(HI_HANDLE hAflt, AFLT_CONFIG_S *pstConfig)
{
    ...
    s32Ret = copy_to_user(pstConfig->pConfigStruct, (HI_VOID *)pstChan->stMsgPoolMmz.u32StartVirAddr, pstConfig->u32ConfigStructSize);
...
static HI_S32 AFLT_InitInbuf(HI_HANDLE hAflt, AFLT_INBUF_ATTR_S *pstAttr)
{
    ...
    if(HI_TRUE == s_stAfltDrv.bAfltSwFlag)
    {
        pstChan->stInBufAttr.u32BufAddr = pstChan->stInBufMmz.u32StartVirAddr;
    }
    else
    {
        pstChan->stInBufAttr.u32BufAddr = pstChan->stInBufMmz.u32StartPhyAddr;
    }
...
err1:
    HI_DRV_MMZ_UnmapAndRelease(&pstChan->stInBufMmz);
    pstChan->stInBufMmz.u32StartPhyAddr = 0;
    pstChan->stInBufMmz.u32StartVirAddr = 0;
...
err1:
    HI_DRV_MMZ_UnmapAndRelease(&pstChan->stOutBufMmz);
    pstChan->stOutBufMmz.u32StartPhyAddr = 0;
    pstChan->stOutBufMmz.u32StartVirAddr = 0;
...
static HI_S32 AFLTAcquireFrame(HI_U32 u32AfltId, AFLT_FRAME_INFO_S* pstFrameInfo)
{
    ...
    HI_U32 u32PeriodBufSize = 0, u32ReadPtr = 0;
    ...
    u32ReadPtr = pstChan->stOutBufMmz.u32StartVirAddr + pstChan->stOutBufAttr.u32PeriodBufSize * u32ReadIdx;
    pstAdspFrameInfo = (AFLT_ADSP_FRAME_INFO_S *)u32ReadPtr;
...
static HI_S32 AFLT_OpenDev(HI_VOID)
{
    ...
    for (i = 0; i < AFLT_MAX_CHAN_NUM; i++)
    {
        ...
        s_stAfltDrv.astChanEntity[i].u32File = (HI_U32)HI_NULL;
    ...
    AFLT_HAL_Init( AFLT_COM_REG_BASE );
...
static HI_VOID AFLTFlushOutBufData(AFLT_CHANNEL_S *pstChan)
{
    ...
    memset((HI_VOID *)pstChan->stOutBufMmz.u32StartVirAddr, 0, pstChan->stOutBufAttr.u32BufSize);
...
HI_S32 AFLT_DRV_Release(struct inode *inode, struct file  *filp)
{
    ...
    if (atomic_dec_return(&s_stAfltDrv.atmOpenCnt) != 0)
    {
        for (i = 0; i < AFLT_MAX_CHAN_NUM; i++)
        {
            if (s_stAfltDrv.astChanEntity[i].u32File == ((HI_U32)filp))
            {
                if (atomic_read(&s_stAfltDrv.astChanEntity[i].atmUseCnt))
                {
