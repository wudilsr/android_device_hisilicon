...
#define INPUT_BYTE_DATA(handle,code,byte)\
        {\
            IMAG_INFO_S *pstImgInfo = (IMAG_INFO_S*)handle;\
            if(1 == byte)\
            {\
...
* others:		: NA
***************************************************************************************/
static HI_BOOL parse_fist_marker(HI_S32 s32DecHandle)
{
    HI_S32 s32Code  = 0;
    HI_S32 s32Code1 = 0;
...
* others:		: NA
***************************************************************************************/
static HI_BOOL parse_dri_marker(HI_S32 s32DecHandle)
{
      HI_S32 s32Len  = 0;
      HI_S32 s32Code = 0;
      IMAG_INFO_S *pstImgInfo = (IMAG_INFO_S*)s32DecHandle;
...
* others:		: NA
***************************************************************************************/
static HI_BOOL parse_sofn_marker(HI_S32 s32DecHandle)
{
    ...
    for (s32Cnt = 0; s32Cnt < pstImgInfo->s32NumComponent; s32Cnt++)
    {
        ...
        INPUT_BYTE_DATA(s32DecHandle,pstImgInfo->stComponentInfo[s32Cnt].s32ComponentId,1);
        INPUT_BYTE_DATA(s32DecHandle,s32Code,1);
        pstImgInfo->stComponentInfo[s32Cnt].u8HorSampleFac = (s32Code >> 4) & 15;
        pstImgInfo->stComponentInfo[s32Cnt].u8VerSampleFac = (s32Code     ) & 15;
        INPUT_BYTE_DATA(s32DecHandle,pstImgInfo->stComponentInfo[s32Cnt].s32QuantTblNo,1);
    }
...
* others:		: NA
***************************************************************************************/
static HI_BOOL parse_sos_marker(HI_S32 s32DecHandle)
{
    HI_S32 s32Len = 0;
...
* others:		: NA
***************************************************************************************/
static HI_BOOL parse_dht_marker(HI_S32 s32DecHandle)
{
...
* others:		: NA
***************************************************************************************/
static HI_BOOL parse_dqt_marker(HI_S32 s32DecHandle)
{
      HI_S32 s32Len  = 0;
      HI_S32 s32Code = 0;
      HI_S32 s32Cnt  = 0;
      ...
      while (s32Len > 0)
      {
            ...
            for (s32Cnt = 0; s32Cnt < MAX_DCT_SIZE; s32Cnt++)
            {
                  if(s32Precision)
                  {
                      INPUT_BYTE_DATA(s32DecHandle,tmp,2);
                   }
                  else
                   {
                      INPUT_BYTE_DATA(s32DecHandle,tmp,1);
                   }
...
HI_S32 jpg_dec_parse(HI_DRV_JPEG_INMSG_S *pstInMsg)
{
    ...
    pstImgInfo->pDataVir[1]   = pstInMsg->pSaveVir[1];
