...
HI_S32 HI_DRV_DMX_GetPoolBufAddr(DMX_MMZ_BUF_S *PoolBuf)
{
    ...
    return DMX_OsiGetPoolBufAddr(&VirAddr, &PoolBuf->u32BufPhyAddr, &PoolBuf->u32BufSize);
}
...
HI_S32 HI_DRV_DMX_TSIAttachTSO(HI_UNF_DMX_TSI_ATTACH_TSO_S *pstTSIAttachTSO)
{
    ...
    Id = (HI_U32)pstTSIAttachTSO->enTSI - (HI_U32)HI_UNF_DMX_PORT_TSI_0;
        ...
        if (0 == down_interruptible(&SaveEsMutex))
        {
                if (SaveEsFlag && ((HI_UNF_DMX_CHAN_TYPE_VID == type) || (HI_UNF_DMX_CHAN_TYPE_AUD == type)))
                {
                    if (HI_NULL == DmxEsHandle[chnid])
                    {
                        ...
                        {
                            snprintf(str, sizeof(str),"%s/dmx_vid_%u-%02u_%02u_%02u.es", path, chnid, now.tm_hour, now.tm_min, now.tm_sec);
                        }
                        else
                        {
                            snprintf(str,sizeof(str), "%s/dmx_aud_%u-%02u_%02u_%02u.es", path, chnid, now.tm_hour, now.tm_min, now.tm_sec);
                        }
...
static HI_S32 DMX_OsrSaveALLTs_Start(HI_U32 PortId)
{
    ...
    else
    {
        ...
        if(PortId >= HI_UNF_DMX_PORT_TSI_0)
        {
            PortId = PortId - HI_UNF_DMX_PORT_TSI_0 + DMX_IFPORT_CNT;
        }
    }
