...
HI_U32 DmxHalGetDmxClk(HI_VOID)
{
    ...
    if ((HI_CHIP_TYPE_HI3716M == ChipType) && (HI_CHIP_VERSION_V300 == Version))
    {
        ...
        Clk    = reg_mv300.bits.dmx_clk_sel ? 172 : 198;
    }
    else
    {
        U_PERI_CRG25 reg;
        reg.value = SYS_READ_REG(PERI_CRG25);
        Clk    = reg.bits.dmx_clk_sel ? 198 : 148;
    }

    switch ( reg.bits.dmx_clk_scale )
    {
        case 0x0 :
            break;
        case 0x1 :
...
#elif defined(CHIP_TYPE_hi3716h) || defined(CHIP_TYPE_hi3716c)
        U_PERI_CRG25 reg;
        reg.value = SYS_READ_REG(PERI_CRG25);
        Clk    = reg.bits.dmx_clk_sel ? 198 : 148;
        switch ( reg.bits.dmx_clk_scale )
        ...
        U_PERI_CRG25 reg;
        reg.value = SYS_READ_REG(PERI_CRG25);
        Clk    = reg.bits.dmx_clk_sel ? 182 : 214;
        switch ( reg.bits.dmx_clk_scale )
        {
            case 0x0 :
                break;
...
HI_VOID DmxHalIPPortGetBPStatus(HI_U32 PortId, DMX_Proc_RamPort_BPStatus_S *BPStatus)
{
    ...
    BPStatus->OverflowBp        =  (PortId == OverflowBp)? HI_TRUE:HI_FALSE;
    SwitchBufferBp              =   (DMX_READ_REG(IP_CHN_BP_STA) >> 8 ) & 0x7;
    BPStatus->SwitchBufferBp    =  (PortId == SwitchBufferBp)? HI_TRUE:HI_FALSE;
...
HI_VOID DmxHalIPPortGetDescInfo(HI_U32 PortId, DMX_Proc_RamPort_DescInfo_S *DescInfo)
{
...
HI_VOID DmxHalGetChannelDescrambleFlag(HI_U32 ChanId, HI_BOOL *pEnable)
{
    U_DMX_PID_CTRL dmx_pidctrl;
...
HI_VOID DmxHalGetRecTsCnt(HI_U32 ScdId, HI_U64 *TsCnt)
{
    HI_U64 TsCntLow;
    HI_U64 TsCntHigh;

    TsCntLow = DMX_READ_REG(TS_CNT0_L(ScdId - 1));
    TsCntHigh = DMX_READ_REG(TS_CNT0_H(ScdId - 1)) & 0xff;
 ...
 */
HI_S32 DmxHalSetTagDealCtl(HI_U32 PortId, HI_BOOL bEnable, HI_U32 u32SyncMode, HI_U32 u32TagLen)
{
    U_MUX_SOURCE mux_source;
    U_MUX_CONFIG mux_config;
    U_DBG_DVBT dbg_config;
    HI_U32 TSPortId = 0; /* no mux source */

    if (HI_TRUE == bEnable)
    {
        if (DMX_INVALID_PORT_ID == PortId)
        {
            HI_ERR_DEMUX("invalid TS port(%d).\n", PortId);
        ...
        dbg_config.u32 = DMX_READ_REG(TS_DBG_DVBT_INTERFACE(PortId));
        dbg_config.bits.dbg_ser_len_bypass = 1;
        DMX_WRITE_REG(TS_DBG_DVBT_INTERFACE(PortId), dbg_config.u32);
    }
    else
    {
        if (DMX_INVALID_PORT_ID == PortId )
 ...
 */
HI_VOID DmxHalSetDemuxTagPortId(HI_U32 DmxId, HI_U32 u32TagPortId)
{
    HI_U32          Id = 0;
    HI_SIZE_T u32LockFlag;
 ...
 */
HI_VOID DmxHalEnableRamClkAutoCtl(HI_VOID)
{
    HI_U32 value;
