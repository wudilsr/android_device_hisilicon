...
static HI_S32 Jpge_ChkChnCfg_toVenc( Jpge_EncCfg_S *pEncCfg )
{
    HI_S32 CfgErr = 0;
    CfgErr |= (pEncCfg->FrameWidth  < 16) | (pEncCfg->FrameWidth  > 4096);
    CfgErr |= (pEncCfg->FrameHeight < 16) | (pEncCfg->FrameHeight > 4096);
...
static HI_S32 Jpge_StartOneFrame( HI_U32 EncHandle, Jpge_EncIn_S *pEncIn )
{
    ...
    if((ret = copy_to_user((HI_U32 *)(pEncPara->StrmBufRpAddr + pEncPara->Vir2BusOffset), &value, sizeof(HI_U32))) < 0)
        {
            return -EFAULT;
        }
    if((ret = copy_to_user((HI_U32 *)(pEncPara->StrmBufWpAddr + pEncPara->Vir2BusOffset), &value, sizeof(HI_U32))) < 0)
        {
            return -EFAULT;
        }
...
static HI_S32 Jpge_StartOneFrame_toVenc( HI_U32 EncHandle, Venc2Jpge_EncIn_S *pEncIn )
{
    Jpge_EncPara_S  *pEncPara  = (Jpge_EncPara_S *)EncHandle;
    ...
    pEncPara->Vir2BusOffset = (HI_U32)pEncIn->pOutBuf - pEncIn->BusOutBuf;
	*(HI_U32 *)(pEncPara->StrmBufRpAddr + pEncPara->Vir2BusOffset) = ;
    *(HI_U32 *)(pEncPara->StrmBufWpAddr + pEncPara->Vir2BusOffset) = ;
#else

    memcpy(pEncPara->pJpgeYqt,pEncIn->pJpgeYqt,sizeof(HI_U8)*64);
    memcpy(pEncPara->pJpgeCqt,pEncIn->pJpgeCqt,sizeof(HI_U8)*64);

    pEncPara->StrmBufAddr   = pEncIn->StrmBufAddr;      /* 64-byte aligned */
    pEncPara->StrmBufRpAddr = pEncIn->StrmBufRpAddr;    /* 16-byte aligned */
    pEncPara->StrmBufWpAddr = pEncIn->StrmBufWpAddr;    /* 16-byte aligned */
    
    pEncPara->StrmBufSize   = pEncIn->StrmBufSize;
    pEncPara->Vir2BusOffset = pEncIn->Vir2BusOffset;

	pEncPara->YuvSampleType = pEncIn->YuvSampleType;
    pEncPara->YuvStoreType  = pEncIn->YuvStoreType;
    pEncPara->RotationAngle = pEncIn->RotationAngle;
    pEncPara->PackageSel    = pEncIn->PackageSel;

	pEncPara->PicWidth      = pEncIn->FrameWidth;
	pEncPara->PicHeight     = pEncIn->FrameHeight;
...
static HI_VOID Jpge_CfgReg( HI_U32 EncHandle )
{
    ...
    {   //pic cfg
		...
		/** revise by y00181162 **/
		D32.bits.memClkGateEn = 1;  //打开mem 门控
		D32.bits.clkGateEn    = 2;  //同时打开帧级和宏块级时钟门控
	...
	pAllReg->JPGE_PTS0 = pEncPara->PTS0;

	/** revise by y00181162 **/
	pAllReg->JPGE_OUTSTD.u32    = 2;  //可以解决性能问题
    // start HW to encoder
...
static HI_S32 Jpge_EndOneFrame( HI_U32 EncHandle, Jpge_EncOut_S *pEncOut )
{
    ...
    if ((ret = copy_from_user(&pNaluHdr,(Jpge_NaluHdr_S *)(pEncPara->StrmBufAddr + pEncPara->Vir2BusOffset), sizeof(Jpge_NaluHdr_S))) < 0)
        {
            return -EFAULT;
        }
   // Jpge_NaluHdr_S  *pNaluHdr = (Jpge_NaluHdr_S *)(pEncPara->StrmBufAddr + pEncPara->Vir2BusOffset);
    
    JfifHdrLen = pEncPara->SlcSplitEn ? 698 : 698 - 6;
    ...
    */
    ...
    if((ret = copy_to_user(pEncOut->pStream, pEncPara->pJfifHdr, JfifHdrLen)) < 0)
        {
            return -EFAULT;
        }
...
HI_S32 Jpge_Create_toVenc( HI_U32 *pEncHandle, Jpge_EncCfg_S *pEncCfg )
{
    ...
    /* check channel config */
    if( HI_FAILURE == Jpge_ChkChnCfg_toVenc( pEncCfg ) )
    {
        return HI_FAILURE;
    }
...
HI_S32 Jpge_Encode_toVenc( HI_U32 EncHandle, Venc2Jpge_EncIn_S *pEncIn,HI_BOOL *pBufferFull )
{
    ...
    Jpge_StartOneFrame_toVenc( EncHandle, pEncIn );    
    JpgeOsal_MutexLock( &JpgeIpCtx.ChnMutex ); /* Multi-Chn Mutex */    
    JpgeIpCtx.CurrHandle = EncHandle;
    Jpge_CfgReg( EncHandle );
    JpgeOsal_MutexLock( &pEncPara->IsrMutex );  /* wait isr */
