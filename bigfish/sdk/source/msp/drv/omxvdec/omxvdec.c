...
HI_VOID omxvdec_release_mem(MMZ_BUFFER_S *pMMZ_Buf)
{
    if (HI_NULL != pMMZ_Buf)
    {
        HI_DRV_MMZ_UnmapAndRelease(pMMZ_Buf);
        memset(pMMZ_Buf, 0, sizeof(MMZ_BUFFER_S));
    }
    else
    {
        OmxPrint(OMX_FATAL, "%s invalid param pMMZ_Buf = NULL\n", __func__);
    }
...
static HI_S32 omxvdec_setup_cdev(OMXVDEC_ENTRY *omxvdec, const struct file_operations *fops)
{
	...
	g_OmxVdecClass = class_create(THIS_MODULE, "hivdec_class");
...
static long omxvdec_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	if (code != VDEC_IOCTL_CHAN_CREATE)
    {
        ...
        pchan = channel_find_inst_by_channel_id(omxvdec, chan_num);
        if (NULL == pchan)
        {
	 ...
	 & player & driver, here we use a input/output buf table to record its info */
    	...
    	case VDEC_IOCTL_CHAN_GET_MSG:
    	{
            ...
            if (ret != HI_SUCCESS)
            {
                if (ret == -EAGAIN)
                {
                    OmxPrint(OMX_WARN, "%s, L%d: no msg found, try again.\n", __func__, __LINE__);
                    return -EAGAIN;
                }
                else
                {
    	...
    	case VDEC_IOCTL_CHAN_CREATE:
    	{
    		...
    		channel_id = channel_create_inst(fd, &chan_cfg);
    		if (channel_id < 0)
            {
                OmxPrint(OMX_FATAL, "%s, l%d: case call channel_create_inst failed!\n", __func__, __LINE__);
    			return -EFAULT;
    		}

    		if (copy_to_user(vdec_msg.out, &channel_id, 4))
            {
                OmxPrint(OMX_FATAL, "%s, l%d: case call copy_from_user failed!\n", __func__, __LINE__);
                return -EIO;
            }
...
HI_VOID omxvdec_help_proc(HI_VOID)
{
    ...
    HI_DRV_PROC_EchoHelper(
    "cmd = set_print,        para = value        :set TraceOption = value\n"
