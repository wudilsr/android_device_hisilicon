...
UINT8 R840_Xtal_Pwr = XTAL_GM3V_HIGHEST;
UINT8 R840_Xtal_Pwr_tmp = XTAL_GM3V_HIGHEST;
...
Sys_Info_Type R840_Sys_Sel(R840_Standard_Type R840_Standard)
{
	...
	case R840_DVB_T2_1_7M_IF_5M: 
		...
		R840_Sys_Info.FILT_EXT_ENA=0x10;      //R11[4]=0, ext disable
		R840_Sys_Info.FILT_EXT_WIDEST=0x00;//R30[2]=0, ext normal
		R840_Sys_Info.FILT_EXT_POINT=0x00;   //R30[1:0]=0, lna=max-1 & Buf 4, hpf+1
		break;
	...
	R840_Sys_Info.RF_DISCHARGE = 0x60;//3 
	R840_Sys_Info.NA_DISCHARGE = 0x04;  
...
SysFreq_Info_Type R840_SysFreq_Sel(R840_Standard_Type R840_Standard,UINT32 RF_freq)
{
	...
	case R840_DVB_T2_1_7M_IF_5M:
		...
		if((R840_Standard==R840_DVB_T_7M) || (R840_Standard==R840_DVB_T2_7M))
		{
...
R840_ErrCode R840_InitReg(void)
{
	...
	if(Rafael_I2C_Write_Len(&R840_I2C_Len) != RT_Success)
		return RT_Fail;
...
R840_ErrCode R840_TF_Check(void)
{
	...
	else  //24M
	{
	 ...
	 if(Rafael_I2C_Write(&R840_I2C) != RT_Success)
		return RT_Fail;
...
R840_ErrCode R840_Xtal_Check(void)
{
	...
	for(i=0; i<XTAL_CHECK_SIZE; i++)
	{
		...
		R840_I2C_Len.RegAddr = 0x00;
		R840_I2C_Len.Len = 3;
		if(Rafael_I2C_Read_Len(&R840_I2C_Len) != RT_Success)
		{
			if(Rafael_I2C_Read_Len(&R840_I2C_Len) != RT_Success)
			   return RT_Fail;
		}
...
R840_ErrCode R840_PLL(UINT32 LO_Freq, R840_Standard_Type R840_Standard)
{
	...
	for(VCO_current_trial=0; VCO_current_trial<3; VCO_current_trial++)
	{
		...
		if(Rafael_I2C_Read_Len(&R840_I2C_Len) != RT_Success)
		{
		    Rafael_I2C_Read_Len(&R840_I2C_Len);
		}		
	...
	if(VCO_current_trial==2)
	{
		...
		{
		        R840_I2C.RegAddr = 0x0F;
				R840_Array[15] = (R840_Array[15] & 0xE7) | 0x18;  //3V
				R840_I2C.Data = R840_Array[15];
				if(Rafael_I2C_Write(&R840_I2C) != RT_Success)
			return RT_Fail;
		}
...
{ 
	...
	rem = rf_rssi - (rf_rssi/1000)*1000;
	if(rem<500)
        *RfLevelDbm = (UINT32)(rf_rssi/1000);
	else
		*RfLevelDbm = (UINT32)(rf_rssi/1000)+1;
