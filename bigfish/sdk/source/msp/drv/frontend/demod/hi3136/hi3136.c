...
HI_VOID hi3136_vcm_ts_clk(HI_U32 u32TunerPort,HI_U32 fs_cur)
{
    ...
    if(!temp1 && temp3 )
    {
      ...
      qam_write_byte(u32TunerPort,0x9a,(HI_U8)u32Temp);
...
HI_S32  hi3136_get_total_isi_number(HI_U32 u32TunerPort,HI_U32 *u32TotalStream)
{
    ...
    //printk("===u8CCMMode:%d===\n",u8CCMMode);
    qam_read_byte(u32TunerPort, LOCK_FLAG, &u8Temp);
    if(u8Temp & 0x8)   //dvb-s2 and VCM/ACM,select stream
    {
...
HI_S32  hi3136_get_isi_id(HI_U32 u32TunerPort,HI_U8 u8Stream, HI_U8 *u8IsiId)
{
    ...
    if(u8Temp & 0x8)   //dvb-s2 and VCM/ACM,select stream
    {
        qam_write_byte(u32TunerPort, TS_CTRL3, u8Stream);
        tuner_mdelay(5);
...
HI_VOID hi3136_read_tuner_registers(HI_U32 u32TunerPort, struct seq_file * p)
{
    HI_U8 i;
    HI_U8 u8Data = 0;
    ...
    if (hi3136_Read_Lock_Status(u32TunerPort) >= SYNC_LOCKED)
    {
        ...
        case 0:
            {   //DVBS
                ...
                temp3 = (temp1 << 8) + temp2;
                ErrorRate = 1000000000/ total_num * temp3;
                //printk("\na:%d--total_num:%d--temp3:%d--ErrorRate:%d\n",a,total_num,temp3,ErrorRate);
            }
            break;
        ...
        case 2:
            {   //DVBS2
                ...
                temp3 = (temp1 << 8) + temp2;
                ErrorRate = 1000000000/ total_num * temp3;
                //printk("\na:%d--b:%d--total_num:%d--temp3:%d--ErrorRate:%d\n",a,b,total_num,temp3,ErrorRate);
                break;
            }
        }
...
static HI_S32 hi3136_set_symrate(HI_U32 u32TunerPort, HI_U32 u32SymRate)  /*u32SymRate unit hz*/
{
    ...
    else
    {
        ...
        qam_write_byte(u32TunerPort, TS_CTRL4,temp);
        temp = 0;
        qam_write_byte(u32TunerPort, 0x99,temp);
        qam_write_byte(u32TunerPort, 0x9a,temp);
    }
    ...
    if(u32SymRate <= 3500)
    {
    	/*tp symbrate <=3.5Mhz,max blindscan carrier error set 3MHz,add by wangjq 20150311*/
		qam_write_byte(u32TunerPort, 0xa0, 0x34);
	}
	else
	{
		/*tp symbrate >3.5Mhz,max blindscan carrier error set 5MHz,add by wangjq 20150311*/
		qam_write_byte(u32TunerPort, 0xa0, 0xb4);
	}
...
HI_S32 hi3136_get_signal_info(HI_U32 u32TunerPort, HI_UNF_TUNER_SIGNALINFO_S *pstInfo)
{
    ...
    HI_U8   u8CCMMode = 0,u8IsiNum = 0,i;
    ...
    if (hi3136_Read_Lock_Status(u32TunerPort) == FEC_LOCKED)
    {
        ...
        switch (temp1)
        {
            ...
            case 2:
            {
                ...
                else
                {
                    ...
                    hi3136_set_isi_id(u32TunerPort,g_u8IsiID);
                }
		...
		*/ /*codecc, cannot reach here,the value of "temp1" must be between 0 and 2.*/
        }
    }
    else
    {
