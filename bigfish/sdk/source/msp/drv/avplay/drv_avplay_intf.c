...
static HI_S32 AVPLAY_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    {
        PROC_PRINT(p, "FrameChanID           :vpss_port%04x->win%04x(master)\n",
    ...
    {
        PROC_PRINT(p, "FrameChanID           :vpss_port%04x->win%04x(slave%02d)\n",
    ...
    {
        PROC_PRINT(p, "FrameChanID           :vpss_port%04x->win%04x(virtual%02d)\n",
...
{
    HI_DRV_PROC_EchoHelper("echo FrcEnable=true|false > /proc/msp/avplayxx, enable or disable frc\n"
          );
...
HI_S32 AVPLAY_SetCpuFreq(struct file *file)
{
    HI_U32  i;
    HI_S32  Ret;
    ...
    if (i == AVPLAY_MAX_NUM)
    {
        HI_ERR_AVPLAY("call AVPLAY_SetCpuFreq fail.\n");
        return HI_FAILURE;
    }
    ...
    if (!strnicmp(cur_policy.governor->name, "interactive", CPUFREQ_NAME_LEN))
    {
        HI_INFO_AVPLAY("------------boost--------------\n");
