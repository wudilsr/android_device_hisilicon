...
static void HDMI_SPD_OnReceiving(HI_U32 packet[IF_BUFFER_LENGTH])
{
	...
	else
	{
		...
		// Check VSIF now.
		//printk("VSIF state will change to VSIF_SPD_ANY_VSIF after 4 .. 1\n");
...
static void HDMI_INFO_PacketProc(HI_U32 u32Addr)
{
    ...
    if (u32Type == HDMI_IF_AVI) {
        ...
        //printk("avi length = %d bCheckSum = %d \n", u32Length, bCheckSum);
...
{
    printk("HDMI_INFO_ResetAudInfoFrameData \n");
...
void HDMI_VSIF_Mloop(void)
{
    ...
    if(hdmi_ctx.StrInfo.u16ElapseTime[0] > hdmi_ctx.StrInfo.u16VsifTimer) {
        switch (hdmi_ctx.StrInfo.u8VsifState) {
            default:
            case VSIF_SPD_INIT:
                //printk("VSIF STATE is VSIF_SPD_INIT \n");
                HDMI_HAL_RegWrite(SPD_DEC, HDMI_IF_VSIF);
                HDMI_HAL_RegWriteFldAlign(RX_INTR3,intr_new_spd, HI_TRUE);
                //printk("vsif state will change to VSIF_SPD_ANY_VSIF after 40 \n");
             ...
             case VSIF_SPD_ANY_VSIF:
                //printk("VSIF STATE is VSIF_SPD_ANY_VSIF \n");
                ...
                //printk("vsif state will change to VSIF_SPD_ANY_SPD after 40 \n");
                break;
             case VSIF_SPD_ANY_SPD:
                //printk("VSIF STATE is VSIF_SPD_ANY_SPD \n");
                HDMI_VSIF_ChangeState(VSIF_SPD_ANY_VSIF, (200/MLOOP_TIME_UNIT)/INFO_TIME_UNIT);
                HDMI_HAL_RegWrite(SPD_DEC, HDMI_IF_VSIF);
                HDMI_HAL_RegWriteFldAlign(RX_INTR3,intr_new_spd, HI_TRUE);
                //printk("vsif state will change to VSIF_SPD_ANY_VSIF after 4 .. 2 \n");
                break;
             case VSIF_SPD_DELAY:
                //printk("VSIF STATE is VSIF_SPD_DELAY \n");
                HDMI_VSIF_ChangeState(VSIF_SPD_ANY_VSIF, (200/MLOOP_TIME_UNIT)/INFO_TIME_UNIT);
                HDMI_HAL_RegWrite(SPD_DEC, HDMI_IF_VSIF);
                HDMI_HAL_RegWriteFldAlign(RX_INTR3,intr_new_spd, HI_TRUE);
                HDMI_ISR_SpdIntOnOff(HI_TRUE);
                //printk("vsif state will change to VSIF_SPD_ANY_VSIF after 4 .. 3 \n");
                break;
        }
