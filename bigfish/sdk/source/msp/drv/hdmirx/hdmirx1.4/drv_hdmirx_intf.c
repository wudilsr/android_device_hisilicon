...
HI_S32 HDMIRX_Create(HI_VOID)
{
    ...
    sprintf(ProcName, "%s", HI_MOD_HDMIRX);
    pProcItem = HI_DRV_PROC_AddModule(ProcName, HI_NULL, HI_NULL);
    if (!pProcItem)
    {
        printk("add %s proc failed.\n", ProcName);
        return HI_FAILURE;
    }
...
HI_S32 HDMIRX_Ioctl(struct inode *inode, struct file *file, unsigned int cmd, HI_VOID *arg)
{
    ...
    {
        printk("Acquire HdmiRx mutex failed!\n");
        return s32Ret;
    }
...
HI_S32 HDMIRX_DRV_ModInit(HI_VOID)
{
    ...
    if(HI_SUCCESS != Ret)
    {
        printk("ERR: HI_DRV_MODULE_Register, Ret = %#x!\n", Ret);
        return HI_FAILURE;
    }

    sprintf(g_HdmirxRegisterData.devfs_name, UMAP_DEVNAME_HDMIRX);
    ...
    if (Ret != HI_SUCCESS)
    {
        printk("HDMIRX PROC Create Fail!\n");
        goto err_proc;
    }

    HDMI_Connect();

    printk("[%s] module initialized\n", HI_MOD_HDMIRX);
