...
#if defined(CHIP_TYPE_hi3751v100) ||  defined(CHIP_TYPE_hi3751v200)  
#define TIANLAI_CODEC_TV
#else //3798c
#define TIANLAI_DAC_STB
#endif

static HI_S32 HAL_TIANLAI_InitBaseAddr(HI_VOID)
{
#if defined(CHIP_TYPE_hi3796cv100) || defined(CHIP_TYPE_hi3798cv100)||defined (CHIP_TYPE_hi3751v100b)
    g_pstRegTianlaiBase = (S_Audio_Codec_REGS_TYPE*)IO_ADDRESS(TIANLAI_BASE_ADDR_V100);
#else
    g_pstRegTianlaiBase = (S_Audio_Codec_REGS_TYPE *)IO_ADDRESS(TIANLAI_BASE_ADDR_V100B);
#endif
...
#ifdef TIANLAI_DAC_STB
#ifdef HI_SND_MUTECTL_SUPPORT
HI_VOID ADAC_FastPowerEnable(HI_BOOL bEnable)
{	
    U_ANA_DAC_CTRL unAnaDacCtl;

    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.fs  = (HI_U32)bEnable; 
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;
}
#endif
#endif
...
static HI_U32 HAL_TIANLAI_Poweup(HI_VOID)
{
    ...
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.popfreer  = 1;
    unAnaDacCtl.bits.popfreel  = 1;
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;
    
#ifdef HI_SND_MUTECTL_SUPPORT 
    //设置fs为快速上电模式
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.fs  = 1; 
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;
#else
    //设置fs为正常上电模式
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.fs  = 0;
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;
#endif

    /* step 2: 将pd_vref置0 */
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.pd_vref  = 0;
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;

    /* step 4: 将相应通道的PD_DAC置0*/
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.pd_dacr  = 0;
    unAnaDacCtl.bits.pd_dacl  = 0;
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;

    /* step 5: 将popfree控制置为0*/
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.popfreer  = 0;
    unAnaDacCtl.bits.popfreel  = 0;
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;
    
    /* step6: 先将dac的模拟mute控制设为0*/ 
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.mute_dacr = 0; 
    unAnaDacCtl.bits.mute_dacl = 0;
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;
#endif
...
static HI_U32 HAL_TIANLAI_Powedown(HI_VOID)
{
...
#ifdef TIANLAI_DAC_STB
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.mute_dacr = 1; 
    unAnaDacCtl.bits.mute_dacl = 1;
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;
    ...
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.pd_dacr = 1; 
    unAnaDacCtl.bits.pd_dacl = 1;
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;
#endif  
...
HI_VOID HAL_TIANLAI_DacSetParam(S5_TIANLAI_DAC_NUM_E enDACNum,S5_TIANLAI_SAMPLERATE_E enSampleRate,HI_U32 u32LeftVol,HI_U32 u32RightVol)
{

    /******************0xff28001c: 0x00F35A00 --> 0xFF035A00******************/
    HAL_TIANLAI_DacSetBitWidth(enDACNum, S5_TIANLAI_BIT_DEPTH_16);

    /******************0xff280020: 0xC0000001 --> 0x08780001******************/
    HAL_TIANLAI_DacSetSoftMuteEn(enDACNum, HI_FALSE);
    HAL_TIANLAI_DacSetSoftUnMuteEn(enDACNum, HI_FALSE);
    HAL_TIANLAI_DacSetI2SSel(enDACNum, 0, 0);
    HAL_TIANLAI_DacSeLRSel(enDACNum, HI_FALSE, HI_TRUE);    
    HAL_TIANLAI_DacSetDeepFs(enDACNum, S5_TIANLAI_DAC_DEEPFS_NULL);
    HAL_TIANLAI_DacVolUpdate(enDACNum, HI_TRUE);

    /*set sample rate*/      
    HAL_TIANLAI_DacSampleRate(enDACNum, enSampleRate);  
    
    /*set volume*/
    HAL_TIANLAI_DacSetVolume(enDACNum,u32LeftVol,u32RightVol);

}

#ifdef TIANLAI_DAC_STB
static HI_VOID HAL_TIANLAI_ADacSetReset(HI_BOOL bReset)
{
    U_ANA_DAC_CTRL unAnaDacCtl;
    
    unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
    unAnaDacCtl.bits.rst       = bReset;
    g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;

    if(bReset == HI_TRUE)
    {
        unAnaDacCtl.u32 = g_pstRegTianlaiBase->ANA_DAC_CTRL.u32;
        unAnaDacCtl.bits.clksel2   = 0;
        g_pstRegTianlaiBase->ANA_DAC_CTRL.u32 = unAnaDacCtl.u32;
    }
}

#endif
static HI_VOID HAL_TIANLAI_SetReset(HI_VOID)
{
    HAL_TIANLAI_DacSetReset(S5_TIANLAI_DAC_NUM_0, HI_FALSE);
#ifdef TIANLAI_CODEC_TV  
    HAL_TIANLAI_DacSetReset(S5_TIANLAI_DAC_NUM_1, HI_FALSE);
    HAL_TIANLAI_DacSetReset(S5_TIANLAI_DAC_NUM_2, HI_FALSE);
    HAL_TIANLAI_DacSetReset(S5_TIANLAI_DAC_NUM_3, HI_FALSE);
    HAL_TIANLAI_AdcSetReset(HI_FALSE);
#endif   
    
#ifdef TIANLAI_DAC_STB
      HAL_TIANLAI_ADacSetReset(HI_TRUE); //to check
#endif
...
#ifdef TIANLAI_DAC_STB
    HAL_TIANLAI_ADacSetReset(HI_FALSE); //to check
#endif

}

static HI_VOID HAL_TIANLAI_SetEnable(HI_VOID)
{   
...
static HI_VOID HAL_TIANLAI_SetDisable(HI_VOID)
{   
    HAL_TIANLAI_DacSetEn(S5_TIANLAI_DAC_NUM_0, HI_FALSE);
#ifdef TIANLAI_CODEC_TV
    HAL_TIANLAI_DacSetEn(S5_TIANLAI_DAC_NUM_1, HI_FALSE);
    HAL_TIANLAI_DacSetEn(S5_TIANLAI_DAC_NUM_2, HI_FALSE);
    HAL_TIANLAI_DacSetEn(S5_TIANLAI_DAC_NUM_3, HI_FALSE);
#endif

}

static HI_VOID HAL_TIANLAI_SetParam(S5_TIANLAI_SAMPLERATE_E enSampleRate)
{    
#ifdef TIANLAI_CODEC_TV  
    HAL_TIANLAI_AdcSetParam(enSampleRate);
    HAL_TIANLAI_DacSetParam(S5_TIANLAI_DAC_NUM_0,enSampleRate,0x0,0x0);
    HAL_TIANLAI_DacSetParam(S5_TIANLAI_DAC_NUM_1,enSampleRate,0x0,0x0);
    HAL_TIANLAI_DacSetParam(S5_TIANLAI_DAC_NUM_2,enSampleRate,0x0,0x0);
    HAL_TIANLAI_DacSetParam(S5_TIANLAI_DAC_NUM_3,enSampleRate,0x0,0x0);  
#endif
    
#ifdef TIANLAI_DAC_STB 
    HAL_TIANLAI_DacSetParam(S5_TIANLAI_DAC_NUM_0,enSampleRate,0x6,0x6);
#endif
}
...
{    
    ...
    HAL_TIANLAI_SetReset(); 
    HAL_TIANLAI_SetEnable();// 
    msleep(1); 
    HAL_TIANLAI_Poweup();   
    HAL_TIANLAI_SetParam(enSampleRate);
...
HI_U32 HAL_TIANLAI_V510_DeInit(HI_VOID)
{
    ...
    HAL_TIANLAI_DacSetReset(S5_TIANLAI_DAC_NUM_2, HI_FALSE);
    HAL_TIANLAI_DacSetReset(S5_TIANLAI_DAC_NUM_3, HI_FALSE);  
    HAL_TIANLAI_AdcSetReset(HI_FALSE);//for stb no use for compile warning
#endif   
