...
extern MMZ_BUFFER_S g_stKernelOmxmmz[OMX_OUTPUT_PORT_BUFFER_MAX];
...
HI_S32 VENC_DRV_EflStartOneFrameVenc( VeduEfl_EncPara_S* EncHandle, VeduEfl_EncIn_S* pEncIn )
{
    ...
    {
        if ((pEncPara->OMXChn != HI_TRUE) || (pEncPara->stStat.GetFrameNumOK == 1))
        {
            VeduEfl_NaluHdr_S NaluHdr;
...
HI_S32 VENC_DRV_EflCreateVenc( VeduEfl_EncPara_S** pEncHandle, VeduEfl_EncCfg_S* pEncCfg )
{
    ...
    s32Ret = HI_DRV_MMZ_AllocAndMap("VENC_SteamBuf", HI_NULL, BitBufSize + RcnBufSize, 64, 
    &pEncPara->StreamMMZBuf);
    if (HI_SUCCESS == s32Ret)
    {
        BusBitBuf = pEncPara->StreamMMZBuf.u32StartPhyAddr;
        pVirBit = (HI_VOID*)(pEncPara->StreamMMZBuf.u32StartVirAddr);
    }
	else
	{
    ...
    pEncPara->StrmBufRpVirAddr = (HI_U32*)pEncPara->StreamMMZBuf.u32StartVirAddr;
...
error_2:	
    HI_DRV_MMZ_UnmapAndRelease(&pEncPara->StreamMMZBuf);
error_1:	
    HI_KFREE(HI_ID_VENC, pEncPara);
error_0:
    return HI_FAILURE;
...
HI_S32 VENC_DRV_EflDestroyVenc( VeduEfl_EncPara_S* EncHandle )
{
    ...
    if (pEncPara->OMXChn)
    {
	    ...
	    {
            if ( 0 != g_stKernelOmxmmz[i].u32StartVirAddr)
	        {
               sMBufVenc_temp.u32StartVirAddr = g_stKernelOmxmmz[i].u32StartVirAddr;
...
HI_S32 VENC_DRV_EflAllocBuf2Jpge( VeduEfl_EncPara_S* EncHandle, HI_U32 CapLevel)
{
    ...
    s32Ret = HI_DRV_MMZ_AllocAndMap("Venc_JpegOutBuf", HI_NULL, u32Size, 64, &pEncPara->JpgMMZBuf);
...
HI_S32 VENC_DRV_EflFreeBuf2Jpge( VeduEfl_EncPara_S* EncHandle )
{
    ...
    HI_DRV_MMZ_UnmapAndRelease(&pEncPara->JpgMMZBuf);
...
#if ((defined __VENC_3716CV200_CONFIG__)|(defined __VENC_S5V100_CONFIG__)|(defined __VENC_98M_CONFIG__)) 
		...
		while (!VeduIpCtx.StopTask)
        {
...
#ifdef __VENC_SUPPORT_JPGE__
			{
				EncHandle = VENC_DRV_EflQueryChn_JPEG( &EncIn );
            if (EncHandle != NULL)
                {
                    D_VENC_GET_CHN(u32ChnID,EncHandle); 
					if (u32ChnID >= VENC_MAX_CHN_NUM)
                    ...
                    if (HI_INVALID_HANDLE != g_stVencChn[u32ChnID].hJPGE)  /*JPGE CHANNEL*/
                    {
						...
						{
						    VENC_DRV_EflEndOneFrameJpge(EncHandle);
						}
            ...
            if (!g_stVencChn[i].bWithoutVPSS)
            {
                if ((g_stVencChn[i].bOMXChn) && g_stVencChn[i].bEnable && (g_stVencChn[i].hVEncHandle != NULL))
                {
                    ...
                    do
                    {
                        s32Ret = (pVpssFunc->pfnVpssGetImage)(g_stVencChn[i].hVPSS, &stVpssFrame);
                        if (s32Ret == HI_SUCCESS)
                        {
                            pEncPara->stStat.VpssQueueBufNum--;
                            pstFramePriv = (HI_DRV_VIDEO_PRIVATE_S*)stVpssFrame.u32Priv;
...
HI_S32 VENC_DRV_EflGetBitStream_X( VeduEfl_EncPara_S* EncHandle, VeduEfl_NALU_S* pNalu )
{
    ...
    bit_offset = (HI_U32)((HI_U8*)pNalu->pVirtAddr[0] - (HI_U8*)pEncPara->StreamMMZBuf.u32StartVirAddr);
    bit_offset1 = (HI_U32)((HI_U8*)pNalu->pVirtAddr[1] - (HI_U8*)pEncPara->StreamMMZBuf.u32StartVirAddr);
