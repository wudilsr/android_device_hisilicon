    ...
    for (enPort = 0; enPort < MAX_VI_PORT; enPort++)
    {
        for (u32Chn = 0; u32Chn < MAX_VI_CHN; u32Chn++)
        {
            if (HI_INVALID_HANDLE != g_ViDrv[enPort][u32Chn].hVi)
            {
                ...
                if (HI_TRUE == g_ViDrv[enPort][u32Chn].bStarted)
                {
                    HI_DRV_VI_Start(g_ViDrv[enPort][u32Chn].hVi);
                }
            }
        }
    }
...
#ifdef HI_HDMI_RX_SUPPORT
        HI_S32 Ret;
        Ret = HI_PHY_VICAP_Start(g_ViDrv[enPort][u32Chn].hVicap);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_VI("HI_PHY_VICAP_Start failed ret = 0x%08x\n",Ret);
            return HI_FAILURE;
...
HI_S32 HI_DRV_VI_Stop(HI_HANDLE hVi)
{
    ...
    VI_DRV_BufReset(&g_ViDrv[enPort][u32Chn].stFrameBuf, &stFbAttr);
    ...
    Ret = (pVpssFunc->pfnVpssSendCommand)(g_ViDrv[enPort][u32Chn].hVpss, HI_DRV_VPSS_USER_COMMAND_RESET, HI_NULL);
    if (HI_SUCCESS != Ret)
    {
    ...
    for (i = 0; i < VI_MAX_VPSS_PORT; i++)
    {
        if (HI_TRUE == g_ViDrv[enPort][u32Chn].stPortParam[i].bEnable)
        {
            switch ((g_ViDrv[enPort][u32Chn].stPortParam[i].hDst >> 16) & 0xff)
            {
                case HI_ID_VO:
                {
                    Ret = (pWinExtFunc->pfnWinReset)(g_ViDrv[enPort][u32Chn].stPortParam[i].hDst, HI_DRV_WIN_SWITCH_BLACK);
                    if (HI_SUCCESS != Ret)
                    {
