...
#elif defined(HI_DONGLE_MODE_NORMAL)
...
//    { 800000, HI_VDD_MPU_OPP3_HPM, HI_VDD_MPU_OPP2_VMIN, HI_VDD_MPU_OPP3_UV},
...
static int cpu_init_hpm(unsigned long rate)
{
    ...
    for (i = 0; i < MAX_FREQ_NUM; i++)
    {
        ...
        {
            cur_cpu_hpm  = cpu_freq_hpm_table[i].hpmrecord + g_u8CpuHpmOffset;
...
static int mpu_clk_set_rate(struct clk *clk, unsigned rate)
{
    ...
    unTmpValueCRG155.bits.cpll_ssmod_ctrl &= 0xffd; //bit[1] to 0;enable ssmod
    ...
    {
    case 1200000:
        sel = 0; // CPLL need to check, now 1.2G
        break;
    case 1500000:
        sel = 3;
        break;
