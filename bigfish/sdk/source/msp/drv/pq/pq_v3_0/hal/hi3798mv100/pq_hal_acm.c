...
HI_S32 PQ_HAL_SetACMParam(COLOR_ACM_LUT_S* pstColorData)
{
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        for (j = 0; j < 7; j++)
        {
            ...
            for (k = 0; k < 29; k++)
            {
                printk("%i, ", sg_pstAcmLut->as16Luma[i][j][k]);
                //printk("%i, ", sg_pstAcmLut->as16Hue[i][j][k]);
                //printk("%i, ", sg_pstAcmLut->as16Sat[i][j][k]);
            }
            printk("},\n");
        }
        printk("    },\n");
    ...
    memcpy((HI_U8*)sg_stACMBuffer.u32StartVirAddr, pAcmBitArray->stBit, pAcmBitArray->u32Size);
