...
HI_S32 VPSS_IN_VMALLOC_V3(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    if (HI_NULL == pstEntity->pstCcclCntInfo[0])
    {
    ...
    if (HI_NULL == pstEntity->pstCcclCntInfo[1])
    {
        VPSS_ERROR("malloc VPSS_CCCLCNTINFO_S failed\n");
        goto V3_VMALLOC_ERROR;
    }
...
{
    if((pstInEntity->stStreamInfo.u32StreamW <= 1920) && (pstInEntity->stStreamInfo.u32StreamH <= 1080))
    {
        return HI_TRUE;
    }
...
HI_VOID VPSS_IN_ClearAllAlgBuffer(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    (HI_VOID)VPSS_STTINFO_CcclDeInit(pstEntity->pstCcclCntInfo[1]);
    ...
    switch(enType)
    {
        ...
        case VPSS_IN_INFO_WBCREG_VA:
                ...
                || HI_DRV_BUF_ADDR_RIGHT == enLR )
            {
                ...
                *pu32VirAddr = pstStWbc->stMMZBuf.u32StartVirAddr;
