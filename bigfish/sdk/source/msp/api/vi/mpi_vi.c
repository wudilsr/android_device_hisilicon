...
static HI_S32 VI_AttachToVpss(HI_HANDLE hVi, HI_HANDLE hDst)
{
    ...
    case HI_ID_VO:
    {
        ...
        stWinSrc.pfRlsFrame = (PFN_PUT_FRAME_CALLBACK)g_Vi[enPort][u8Chn].stPortParam[i].pfRlsImage;
        stWinSrc.pfSendWinInfo = (PFN_GET_WIN_INFO_CALLBACK)g_Vi[enPort][u8Chn].stPortParam[i].pfChangeWinInfo;
    ...
    case HI_ID_VENC:
    {
        ...
        stVencSrc.pfRlsFrame   = (FN_VENC_PUT_FRAME)g_Vi[enPort][u8Chn].stPortParam[i].pfRlsImage;
        stVencSrc.pfChangeInfo = (FN_VENC_CHANGE_INFO)g_Vi[enPort][u8Chn].stPortParam[i].pfChangeVencInfo;
