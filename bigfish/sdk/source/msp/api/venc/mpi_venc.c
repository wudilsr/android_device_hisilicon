...
HI_S32 HI_MPI_VENC_Create(HI_HANDLE* phVencChn, const HI_UNF_VENC_CHN_ATTR_S* pstAttr)
{
    ...
    if (pstAttr->bSlcSplitEn)
    {
       patchBufSize = VENC_ALIGN_UP((pstAttr->u32Width *pstAttr->u32Height*3/2/4), VEDU_MMZ_ALIGN);
    }
    else
    {
       patchBufSize = VENC_ALIGN_UP((pstAttr->u32Width *pstAttr->u32Height*3/2), VEDU_MMZ_ALIGN);
    }
    s_asrVeChnVirAddr[i].pStrmBufVirAddr = HI_MMAP(s_asrVeChnInfo[i].u32StrmBufPhyAddr, s_asrVeChnInfo[i].u32BufSize + patchBufSize );    //liminqi
    if (NULL == s_asrVeChnVirAddr[i].pStrmBufVirAddr)
    {
...
HI_S32 HI_MPI_VENC_Destroy(HI_HANDLE hVencChn)
{
    ...
    s32Ret = HI_MUNMAP(s_asrVeChnVirAddr[i].pStrmBufVirAddr);
    if (HI_SUCCESS != s32Ret)
    {
        return s32Ret;
    }
