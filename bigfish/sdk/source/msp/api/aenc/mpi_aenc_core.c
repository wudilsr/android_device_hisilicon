...
static HI_S32 AENCInitInBuf(AENC_CHAN_S *pstAencChan, HI_U32 u32Size, HI_U32 u32FrameSize)
{
    ...
    {   
#ifdef AENC_MMZ_BUF_SUPPORT
        strcpy(pstInBuf->sMMzBuf.bufname, g_szAencInBuf_MMzName[pstAencChan->u32ChID]);
...
static HI_VOID AENCInitProcInfo(AENC_CHAN_S *pstAencChan)
{
    ...
    pstAencInfo->bAencWorkEnable = HI_FALSE;
...
//    CHECK_AENC_TYPE(pstAencAttr->enAencType);
    ...
    pstAencChan->pstAencInfo->bAencWorkEnable = HI_TRUE;
...
HI_S32 AENCHwInitInbuf(AENC_CHAN_S *pstAencChan, HI_U32 u32BufSize)
{
    ...
    pstAencChan->sInPcmBuf.pRealAddr = (HI_VOID *)stInbufAttr.u32BufAddr;
...
HI_S32 AENCHwDeInitInbuf(AENC_CHAN_S *pstAencChan)
{
    ...
    hHaEntry = (HI_HA_ENCODE_S *)(pstAencChan->hHaEntry);
    if(HA_ErrorNone != hHaEntry->EncodeHwDeInitInbuf(pstAencChan->hEncoder, (HI_U32)pstAencChan->sInPcmBuf.pRealAddr))
    {
        return HI_FAILURE;
    }
...
HI_S32 AENCHwInitOutbuf(AENC_CHAN_S *pstAencChan, HI_U32 u32BitsFrameSize, HI_U32 u32FrameNum)
{
    ...
    pstAencChan->sOutStreamBuf.pBaseAddr = (HI_VOID *)stOutbufAttr.u32BufAddr;
...
HI_S32 AENCHwDeInitOutbuf(AENC_CHAN_S *pstAencChan)
{
    ...
    hHaEntry = (HI_HA_ENCODE_S *)(pstAencChan->hHaEntry);
    if(HA_ErrorNone != hHaEntry->EncodeHwDeInitOutbuf(pstAencChan->hEncoder, (HI_U32)pstAencChan->sOutStreamBuf.pBaseAddr))
    {
        return HI_FAILURE;
    }
...
HI_S32 AENCHwReceiveStream(AENC_CHAN_S * pstAencChan, AENC_STREAM_S *pstStream, HI_U32 u32TimeoutMs)
{
    ...
    pstStream->pu8Data = pstAencChan->sOutStreamBuf.pBaseAddr + (HI_U32)sAOut.ps32BitsOutBuf;  //TODO ps32BitsOutBuf is offset addr for hw encoude
