...
{
    return s_SysUnicable.u32UBInfo[s_SysUnicable.u32UBIndex].s32CenterFreq;
...
static int hi3136_SearchTone(HI_U32 u32TunerId,HI_U32 CenterFreq,HI_U32 *pu32ToneTrue, HI_U32 *peakAgc)
{
    ...
    s32Ret = TUNER_Get_Agc(u32TunerId,CenterFreq,&u32PeakAgc);
    ...
    *peakAgc = u32PeakAgc;
    ...
    if(u32ToneTrue)
    {
        ...
        else
        {
            ...
            tsAvgToneCount++;
            //printf("Get Average agc value %d\n", tsAvgToneAgc);
        }
...
static HI_S32 Scr_EnableTone(HI_U32 u32TunerId)
{
    ...
    //printf("enter Scr_EnableTone\n");
    //printf("@@@@@@@@@@@@Send the tone enable cmd!\n");
...
static HI_S32 Scr_OffTones(HI_U32 u32TunerId, HI_U32 SCRBPF)
{
    ...
    //printf("@@@@@@@@@@@@Send the scrdrv off cmd!\n");
    //printf("enter Scr_OffTones, u32TunerId is 0x%x, UB is %d\n", u32TunerId, SCRBPF+1);
...
HI_S32 Scr_SetFrequency(HI_U32 u32TunerId,  HI_U32 Frequency, HI_U32 LNBIndex, HI_U32 SCRBPF)
{
    ...
    printf("@@@@@@@@@@@@Send the scrdrv setfreq cmd!");
    printf("enter SetFrequency Scr set Freq %d, LNBIndex %d, SCRBPF %d\n", Frequency, LNBIndex, SCRBPF);
...
static HI_S32 Scr_UBxDeAllocation(HI_U32 u32TunerId, HI_U8 SCRBPF)
{
    ...
    printf("@@@@@@@@@@@@Send the scrdrv deallocation cmd!");
    printf("enter Scr_UBxDeAllocation,UB is %d\n",SCRBPF);
...
static HI_S32 Scr_UBxPresent(HI_U32 u32TunerId, HI_U32 SCRBPF)
{
    ...
    printf("@@@@@@@@@@@@Send the scrdrv ub present cmd!");
    printf("enter Scr_UBxPresent,UB is %d\n",SCRBPF);
...
TONE_SEARCH_UB_ACCEPT_E Scr_CheckToneOnFreq(HI_U32 u32TunerId, HI_U32 ulCenterFreq)
{
    ...
    printf("(%s,%d): Start tone signal scan. Freq = %d!\n", __FUNCTION__, __LINE__, ulCenterFreq);
    ...
    do
    {
        ...
        printf("[%d]Search tone yes %d   %d", center_freq, center_freq,  power_yes);
        printf("[%d]Search tone no  %d   %d", center_freq, center_freq + HON_D_FREQ_OFFSET_NO,   power_no);

        if (u32ToneTrueYes)
        {
            printf("(%s,%d): [YES] tone detected. tone_freq = %d!\n", __FUNCTION__, __LINE__, center_freq);
            enAccept = TONE_SEARCH_UB_NOT_ASSIGNED;
            break;
        }
        else if (u32ToneTrueNo)
        {
            printf("(%s,%d): [NO] tone detected. tone_freq = %d!\n", __FUNCTION__, __LINE__, center_freq + HON_D_FREQ_OFFSET_NO);
    ...
    printf("(%s,%d): Stop tone signal scan. center_freq = %d!\n", __FUNCTION__, __LINE__, center_freq);
    return enAccept;
}
...
TONE_SEARCH_UB_ACCEPT_E Scr_CheckToneOnUB(HI_U32 u32TunerId,HI_U32 SCRBPF)
{
    ...
    printf("(%s,%d): Start tone signal scan. UB = %d!\n", __FUNCTION__, __LINE__, SCRBPF+1);
...
HI_U32 Scr_GetToneAgc(HI_U32 u32TunerId, HI_U32 u32CenterFreq)
{
    ...
    printf("(%s,%d): Start tone signal scan. ulCenterFreq = %d!\n", __FUNCTION__, __LINE__, u32CenterFreq);
    ...
    {
        printf("(%s,%d): There is no valid tone detected. \n", __FUNCTION__, __LINE__);
    }
...
HI_S32 Scr_CheckOffValid(HI_U32 u32TunerId, HI_U32 u32Index, HI_U32 u32CentralFreq)
{
    ...
    printf("Check off valid index[%d]  centralfreq[%d]", u32Index, u32CentralFreq);
    ...
    for (i=0; i<3; i++)
    {
        ...
        printf("+++tsResultAgc:%d+++tsAvgToneAgc:%d\n",tsResultAgc,tsAvgToneAgc);

        if ( s32Agc > tsAvgToneAgc + 300)
        {
            printf("UB:%d off valid.\n",u32Index+1);
            s32Ret = HI_SUCCESS;
            break;
        }
...
static TONE_SEARCH_UB_ACCEPT_E Scr_MDUAccept(HI_U32 u32TunerId, HI_U32 SCRBPF, HI_U16 SCRCenterFrequency)
{
    ...
    printf("@@@@@@@@@@@@Send the scrdrv ub accept cmd!");
    printf("Accept the ub %d !", SCRBPF);
...
HI_U8 Scr_GetAverageAGC(HI_U8 *pcAGC, HI_S32 iLength)
{
    HI_S32     iLoop       = 0;
    HI_S32     iIndex      = 0;
    HI_U8      cAgcTemp    = 0;
...
HI_S32 Scr_BlindScanTone(HI_VOID *pstScanPara)
{
    ...
    for(u32UBCount = 0; u32UBCount < SCR_USER_BAND_NUMBERS;u32UBCount++)
    {
        ...
        printf("++++sMinAgc.u32Freq:%d+++sMinAgc.u32Agc:%d\n",sMinAgc[u32UBCount].s32Freq,sMinAgc[u32UBCount].s32Agc);
        printf("freq start:%dMHz ",sSCRArray[s32MinAgcIndex - 50].s32Freq);
        for(s32Index = s32MinAgcIndex -50; s32Index < s32MinAgcIndex + 50; s32Index++)
        {
            printf("%d ",sSCRArray[s32Index].s32Agc);
        }
	...
	for(u32UBCount = 0; u32UBCount < u32RealUBNumber;u32UBCount++)
	{
		...
		//printf("After scr off,tone freq %d agc value %d\n",sMinAgc[u32UBCount].s32Freq,s32AgcValue);
	...
	if(s32Count == 0)
	{
		printf("There is no tone,please check the unicable!\n");
	...
	for(u32UBCount = 0; u32UBCount < u32RealUBNumber;u32UBCount++)
	{
		...
		//printf("After scr on,tone freq %d agc value %d\n",sMinAgc[u32UBCount].s32Freq,s32AgcValue);
	...
	{	
        ...
        //printf("<<<<<<<<<====dToneFreq:%f======s32ToneFreucncy:%d\n",dToneFreq,s32ToneFreucncy);
...
HI_S32 HI_UNF_TUNER_UNICABLE_ScanUserBands(HI_U32 u32TunerId, HI_UNF_TUNER_UNICABLE_SCAN_PARA_S stScanPara)
{
    ...
    /*全频段扫描疑似tone信号，并保存*/
    s32Result = pthread_create(pScanUB, 0, Scr_BlindScanTone, &stLocalScanPara);
