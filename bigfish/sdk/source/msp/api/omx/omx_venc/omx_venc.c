...
static const struct codec_profile_level avc_profile_level_list[] =
{
    ...
    //{OMX_VIDEO_AVCProfileHigh, OMX_VIDEO_AVCLevel1},        //not support High profile now!
...
static int write_output(void)
{
    ...
    printf("\n/******************************* BEGIN ****************************************/\n");
    printf(" FrameID         diff1   diff2   |   diff3    diff4  |  [diff5]   ||total_diff||\n");
    for (i = 0; i < 100; i++)
    {
        ...
        printf(" %03d  %8d   %8d  |  %8d  %8d  | [%8d]   ||%8d||\n",
               i, diff1, diff2, diff3, diff4, diff5, total);
    }
    printf("\n/*******************************  END *******************************************/\n");
        ...
        OMX_IN OMX_U8* buffer)
{
    ...
    pvenc_buf->user_buf.client_data    = (OMX_U32)pomx_buf;
...
#if 0 /*now not support set level*/
    switch (pVencChnCfg->h264Level)
    {
        case 10:
            profileLevelType->eLevel = OMX_VIDEO_AVCLevel1;
    ...
    OMX_IN OMX_BUFFERHEADERTYPE* pomx_buf)
{
    ...
    if (pcom_priv->m_store_metadata_buf != HI_TRUE)
    {
        ...
        if ((port_priv->port_def.format.video.eColorFormat == OMX_COLOR_FormatYUV420SemiPlanar)
            || (port_priv->port_def.format.video.eColorFormat == OMX_COLOR_FormatYVU420SemiPlanar))   //QS!!
        {
        ...
        /*else   //for OMX_COLOR_FormatAndroidOpaque
        {
           puser_buf->strideY      = ALIGN_UP(port_priv->port_def.format.video.nFrameWidth,16);
           puser_buf->strideC      = ALIGN_UP(port_priv->port_def.format.video.nFrameWidth,16);
           puser_buf->offset_YC    = puser_buf->strideY * port_priv->port_def.format.video.nFrameHeight;
        }*/
    }
    ...
    OMX_STATETYPE state, OMX_U32* sem_posted)
{
    ...
    if (pcom_priv->m_state == OMX_StateLoaded)
    {
        ...
        {
#if 0
            HI_UNF_VENC_CHN_ATTR_S* pVencUnfAttrs = &pcom_priv->drv_ctx.venc_chan_attr.chan_cfg.VencUnfAttr;

            DEBUG_PRINT("encoder width:%d,height:%d,\n",
                        pVencUnfAttrs->u32Width, pVencUnfAttrs->u32Height);
            DEBUG_PRINT("encoder CapLevel:%d,VencProfile:%d,protocol:%d,rotation_angle:%d,streamBufSize:%d,SlcSplitEn:%d\n",
                        pVencUnfAttrs->enCapLevel, pVencUnfAttrs->enVencProfile,
                        pVencUnfAttrs->enVencType,
                        pVencUnfAttrs->u32RotationAngle,
                        pVencUnfAttrs->u32StrmBufSize,
                        pVencUnfAttrs->bSlcSplitEn);
            DEBUG_PRINT("encoder Gop:%d,QuickEncode:%d,u32DriftRateThr = %d,TargetBitRate:%d,TargetFrmRate:%d,InputFrmRate:%d\n",
                        pVencUnfAttrs->u32Gop,
                        pVencUnfAttrs->bQuickEncode,
                        pVencUnfAttrs->u32DriftRateThr,
                                    ...
                                    OMX_INOUT OMX_PTR param_data)
{
    ...
    switch (param_index)
    {
...
#if 0
            DEBUG_PRINT("get_param: OMX_IndexParamPortDefinition,PortIndex(%d),state(%d),nBufferCountActual(%d),nBufferCountMin(%d),nBufferSize(%d)\n",
                        (OMX_S32)portDefn->nPortIndex, (OMX_S32)pcomp_priv->m_state,
                        portDefn->nBufferCountActual,
                        portDefn->nBufferCountMin,
                        portDefn->nBufferSize);
            DEBUG_PRINT("get_param: OMX_IndexParamPortDefinition,bEnabled(%d),bPopulated(%d),bBuffersContiguous(%d),nBufferAlignment(%d)\n",
                        (OMX_S32)portDefn->bEnabled, portDefn->bPopulated,
                        portDefn->bBuffersContiguous,
                        portDefn->nBufferAlignment);
            DEBUG_PRINT("get_param: OMX_IndexParamPortDefinition,nFrameWidth(%d),nFrameHeight(%d),nStride(%d),nSliceHeight(%d),nBitrate(%d),eCompressionFormat(%d),eColorFormat(%d)\n",
        ...
        {
            OMX_VIDEO_PARAM_AVCTYPE* pAvcType = (OMX_VIDEO_PARAM_AVCTYPE*)param_data;
            HI_UNF_VENC_CHN_ATTR_S* pVencUnfAttrs = &pcomp_priv->drv_ctx.venc_chan_attr.chan_cfg.VencUnfAttr;
        ...
        case OMX_IndexParamVideoPortFormat:
        {
            ...
            if ( 0  == portFmt->nIndex )       //输入&输出端口一致
            {
                ...
                DEBUG_PRINT("get_param: OMX_IndexParamVideoPortFormat,PortIndex(%lu),eColorFormat = %d,eCompressionFormat = %d\n",
                            portFmt->nPortIndex, portFmt->eColorFormat, portFmt->eCompressionFormat);
            }
            else
            {
        ...
        case OMX_SkyworthIndexGetParameterSet:
        {
            OMX_VIDEO_PPS_SPS_DATA* pParameterSet =
                (OMX_VIDEO_PPS_SPS_DATA*)param_data;

            DEBUG_PRINT("get_param: OMX_SkyworthIndexGetParameterSet\n");
        ...
        case OMX_SkyworthIndexAutoRequestIFrm:
        {
            ...
            DEBUG_PRINT("get_parameter: OMX_SkyworthIndexAutoRequestIFrm\n");
    ...
    switch (param_index)
    {
        case OMX_IndexParamPortDefinition:
        {
            ...
            if (portDefn->nPortIndex == OUTPUT_PORT_INDEX)
            {
                ...
                pvenc_attr->VencUnfAttr.u32TargetFrmRate = pcomp_priv->m_port[0].port_def.format.video.xFramerate >> 16; //portDefn->format.video.xFramerate >> 16;
        ...
        case OMX_IndexParamVideoBitrate:
        {
            OMX_VIDEO_PARAM_BITRATETYPE* pVideoBitRate = (OMX_VIDEO_PARAM_BITRATETYPE*)param_data;
            HI_UNF_VENC_CHN_ATTR_S* pVencUnfAttrs = &pcomp_priv->drv_ctx.venc_chan_attr.chan_cfg.VencUnfAttr;
...
#if 0
            if (pVideoBitRate->eControlRate != OMX_Video_ControlRateConstant)
            {
                DEBUG_PRINT_ERROR("not support the ControlRate(%ld),just support ControlRate(%d)\n",
                                  (OMX_S32)pVideoBitRate->eControlRate, OMX_Video_ControlRateConstant);
                ret = OMX_ErrorBadParameter;
            }
#endif
...
#if 0
            DEBUG_PRINT("set_param : OMX_IndexParamVideoPortFormat PortIndex = %ld,state=%ld,\n",
                        (OMX_S32)portFmt->nPortIndex, (OMX_S32)pcomp_priv->m_state);
            DEBUG_PRINT("set_param: OMX_IndexParamVideoPortFormat,PortIndex(%ld),eColorFormat = %ld,eCompressionFormat = %ld\n",
                        portFmt->nPortIndex, portFmt->eColorFormat, portFmt->eCompressionFormat);
#endif
        ...
        case OMX_GoogleIndexStoreMetaDataInBuffers:
        {
            ...
            if (pcomp_priv->m_store_metadata_buf == HI_TRUE)
            {
                //HI_TDE2_Open();
                /*pcomp_priv->hTDE = HI_TDE2_BeginJob();
                if ((pcomp_priv->hTDE == HI_NULL)||(pcomp_priv->hTDE == HI_INVALID_HANDLE))
                {
                   DEBUG_PRINT_ERROR("set_parameter: StoreMetaDataInBuffersParams,TDE failed to get Handle(ret = 0x%x)%d\n", pcomp_priv->hTDE);
                   return OMX_ErrorHardware;
                }*/
            }
        ...
        case OMX_SkyworthIndexAutoRequestIFrm:
        {
            struct AutoRequestIFrmParams* penable = (struct AutoRequestIFrmParams*)param_data;
            venc_chan_cfg* pVencChnAttr = &pcomp_priv->drv_ctx.venc_chan_attr.chan_cfg;
            DEBUG_PRINT("set_parameter: OMX_SkyworthIndexAutoRequestIFrm-> enable(%d)\n",
                        (int)penable->bEnable);
            ...
            {
                DEBUG_PRINT_ERROR("ERROR: set_parameter: OMX_SkyworthIndexAutoRequestIFrm-> enable(%d) no use!\n", penable->bEnable);
            }
            break;
        }
        default:
        ...
        OMX_OUT OMX_INDEXTYPE* pindex_type)
{
    ...
    DEBUG_PRINT("get_extension_index : param_name = %s!\n", param_name);

    if (!strcmp(param_name,
                "OMX.Hisi.Param.Index.ChannelAttributes"))
    {
        *pindex_type = OMX_HisiIndexChannelAttributes;
        DEBUG_PRINT("get_extension_index : pindex_type = %d!\n", *pindex_type);
    }
    //#ifdef _ANDROID_
                     ...
                     "OMX.google.android.index.useAndroidNativeBuffer2"))
    {
        *pindex_type = OMX_GoogleIndexUseAndroidNativeBuffer;
        DEBUG_PRINT("get_extension_index : pindex_type = %d!\n", *pindex_type);
                     ...
                     "OMX.skyworth.android.index.getParameterSet"))
    {
        *pindex_type = OMX_SkyworthIndexGetParameterSet;
        DEBUG_PRINT("get_extension_index : pindex_type = %d!\n", *pindex_type);
    }
    else if (!strcmp(param_name,
                     "OMX.skyworth.android.index.autoRequestIFrmScnChg"))
    {
        *pindex_type = OMX_SkyworthIndexAutoRequestIFrm;
        DEBUG_PRINT("get_extension_index : pindex_type = %d!\n", *pindex_type);
    }
    //#endif
    else
