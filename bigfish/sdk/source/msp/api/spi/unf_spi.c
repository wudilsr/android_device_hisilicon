...
HI_S32 HI_UNF_SPI_SetAttr(HI_UNF_SPI_DEV_E enDev, HI_UNF_SPI_ATTR_S *pstAttr)
{
    ...
    /*Frame And Dss*/
    Ret = ioctl(g_SpiDevFd, CMD_SPI_SET_ATTR, (HI_U32)&SPI_FFORM);

    /*Big-Little End*/
    Ret |= ioctl(g_SpiDevFd, CMD_SPI_SET_BLEND, (HI_U32)&setBigEnd);
    if (HI_SUCCESS != Ret)
    {
...
HI_S32 HI_UNF_SPI_GetAttr(HI_UNF_SPI_DEV_E enDev, HI_UNF_SPI_ATTR_S *pstAttr)
{
    ...
    Ret = ioctl(g_SpiDevFd, CMD_SPI_GET_ATTR, (HI_U32)&SPI_FFORM);
    ...
    Ret |= ioctl(g_SpiDevFd, CMD_SPI_GET_BLEND, (HI_U32)&getBigEnd);
...
HI_S32 HI_UNF_SPI_ReadExt(HI_UNF_SPI_DEV_E enDev, HI_U8 *pu8Send, HI_U32 u32SendCnt, HI_U8 *pu8Read, HI_U32 u32ReadCnt)
{
	...
	ret = ioctl(g_SpiDevFd, CMD_SPI_READEX, (HI_U32)&DATA);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_SPI("read err.\n");
        return ret;
...
HI_S32 HI_UNF_SPI_Read(HI_UNF_SPI_DEV_E enDev, HI_U8 *pu8Read, HI_U32 u32ReadCnt)
{
    ...
    ret = ioctl(g_SpiDevFd, CMD_SPI_READ, (HI_U32)&SDATA);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_SPI("read err.\n");
        return ret;
...
HI_S32 HI_UNF_SPI_Write(HI_UNF_SPI_DEV_E enDev, HI_U8 *pu8Send, HI_U32 u32SendCnt)
{
    ...
    ret=ioctl(g_SpiDevFd, CMD_SPI_WRITE, (HI_U32)&SDATA);
    if (HI_SUCCESS != ret)
    {
