...
HI_S32 HI_MPI_VDEC_Init(HI_VOID)
{
    ...
    if (0 == s_stVdecParam.u8InitCount)
    {
        ...
        s32Ret |= HI_CODEC_Register(VDEC_VPU_Codec());
#endif
    }
...
static HI_VOID* VDEC_SoftCodec(HI_VOID* phVdec)
{
    ...
    HI_U32  u32Count = 0, i = 0, u32WaitEsTime = 0;
    HI_U32  u32DemuxFlag = 0;
    VPU_STREAM_BUF_STATUS_S stStreamBufStatus = {0};
    ...
    while (!pstVdec->bThreadStop)
    {
        ...
        if (2 != pstVdec->u32EosFlag)   /* If not end of stream */
        {
            ...
            else
            {
                if (HI_INVALID_HANDLE != pstVdec->hStreamBuf)
                {
                    ...
                    else
                    {
                        ...
                        if (1 == pstVdec->u32EosFlag)
                        {
                            ...
                            if (0 == strncmp("VPU", pstCodec->pszName, 3))
                            {
                                s32Ret = pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), VPU_CMD_SETSTREAMENDFLAG, (HI_VOID*)(&(pstVdec->u32EosFlag)));
                                if (HI_SUCCESS != s32Ret)
                                {
                                    HI_ERR_VDEC("VPU_SetStreamEndFlag fialed:%d.\n",s32Ret);
                                }
                            }
                        }
                    }
                ...
                {
                   u32DemuxFlag = 1;
                    pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), VPU_CMD_SETBITSTREAMMODE, (HI_VOID*)(&u32DemuxFlag));
                    pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), VPU_CMD_SETWAITESTIME, (HI_VOID*)(&u32WaitEsTime));
                    for (i = 0; i < u32Count; i++)
                    {
                        ...
                        if (HI_SUCCESS == s32Ret)
                        {
                            ...
                            if (0 == strncmp("VPU", pstCodec->pszName, 3))
                            {
                                s32Ret = pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), VPU_CMD_FIllRAWPACKAGE, (HI_VOID*)pstCodecStrm);
                                if (HI_SUCCESS == s32Ret)
                                {
                                    if (HI_INVALID_HANDLE != pstVdec->hDmxVidChn)
                                    {
                    ...
                    if (0 == strncmp("VPU", pstCodec->pszName, 3))
                    {
                        pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), VPU_CMD_GETSTREAMBUFSTATUS, (HI_VOID*)(&stStreamBufStatus));
                        if (stStreamBufStatus.bStreamBufSuccfient == HI_FALSE)
                        {
                            VDEC_UNLOCK(pstVdec->stMutex);
                            continue;
                        }
        ...
        if (HI_SUCCESS == s32Ret || VPU_STR_BUF_INSUFFICIENT_SUCESS == s32Ret)
        {
            bDecodeSucc = HI_TRUE;
...
HI_S32 HI_MPI_VDEC_CheckNewEvent(HI_HANDLE hVdec, VDEC_EVENT_S *pstNewEvent)
{
    ...
    if (HI_NULL != pstVdec->pstCodec)
    {
        ...
        else
        {
            ...
            if (0 == strncmp("VPU", pstVdec->pstCodec->pszName, 3))
            {
				s32Ret =  pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), VPU_CMD_CHECKEVT, (HI_VOID*)pstNewEvent);
...
HI_S32 HI_MPI_VDEC_ReadNewFrame(HI_HANDLE hVdec, HI_DRV_VIDEO_FRAME_S *pstNewFrame)
{
    ...
    if (HI_NULL != pstVdec->pstCodec)
    {
        ...
        else
        {
            if (0 == strncmp("VPU", pstVdec->pstCodec->pszName, 3))
            {
                pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), VPU_CMD_READNEWFRAME, (HI_VOID*)(pstNewFrame));
            }
            else
            {
...
HI_S32 HI_MPI_VDEC_SetChanFrmRate(HI_HANDLE hVdec, HI_UNF_AVPLAY_FRMRATE_PARAM_S *pstFrmRate)
{
    ...
    if (HI_NULL != pstVdec->pstCodec)
    {
        ...
        {
            return pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), VPU_CMD_SETFRAMERATE, (HI_VOID*)pstFrmRate);
        }
#endif
#if (HI_VDEC_REG_CODEC_SUPPORT == 1) || (HI_VDEC_MJPEG_SUPPORT == 1)
...
HI_S32 HI_MPI_VDEC_GetChanFrmRate(HI_HANDLE hVdec, HI_UNF_AVPLAY_FRMRATE_PARAM_S *pstFrmRate)
{
    ...
    if (HI_NULL != pstVdec->pstCodec)
    {
        ...
        {
            return pstVdec->pstCodec->Control(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), VPU_CMD_GETFRAMERATE, (HI_VOID*)pstFrmRate);
        }
#endif
#if (HI_VDEC_REG_CODEC_SUPPORT == 1) || (HI_VDEC_MJPEG_SUPPORT == 1)
        else if (HI_INVALID_HANDLE != pstVdec->hFrameBuf)
