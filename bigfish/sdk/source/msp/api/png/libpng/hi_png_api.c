...
HI_VOID PNG_UnMap_DecoderBuf(HI_PNG_HANDLE s32Handle)
{
    ...
    while (gs_PngApiInstance[s32Handle - 1].pMemHead != HI_NULL)
    {
		...
		HI_GFX_Unmap(gs_PngApiInstance[s32Handle - 1].pMemHead->stBuf.u32PhyAddr);
		HI_GFX_FreeMem(gs_PngApiInstance[s32Handle - 1].pMemHead->stBuf.u32PhyAddr);
#else
		...
		HI_GFX_FreeMem(g_s32MemFd, gs_PngApiInstance[s32Handle - 1].pMemHead->stBuf.u32PhyAddr, gs_PngApiInstance[s32Handle - 1].pMemHead->stBuf.pMemHandle);
#endif	
#endif
...
HI_S32 HI_PNG_AllocBuf(HI_PNG_HANDLE s32Handle, HI_PNG_BUF_S *pstBuf)
{
...
#ifdef HIPNG_COMMON_MMZ
	pstBuf->u32PhyAddr = (HI_U32)HI_GFX_AllocMem(u32Size, 16, NULL, "HIPNG_STREAM_BUF");
#else
	pstBuf->u32PhyAddr = (HI_U32)HI_GFX_AllocMem(g_s32MemFd, u32Size, 16, NULL, "HIPNG_STREAM_BUF", &pMemHandle);
#endif
...
#ifdef HIPNG_COMMON_MMZ
	pstBuf->pVir = HI_GFX_MapCached(pstBuf->u32PhyAddr);
#else
	...
	{
#ifdef HIPNG_COMMON_MMZ
		HI_GFX_FreeMem(pstBuf->u32PhyAddr);
#else
		HI_GFX_FreeMem(g_s32MemFd, pstBuf->u32PhyAddr, pMemHandle);
#endif
		return HI_ERR_PNG_NOMEM;
	}
...
#ifndef HIPNG_STREAMBUF_KERNEL
#ifdef HIPNG_COMMON_MMZ
		HI_GFX_Unmap(pstBuf->u32PhyAddr);
		HI_GFX_FreeMem(pstBuf->u32PhyAddr);
#else
