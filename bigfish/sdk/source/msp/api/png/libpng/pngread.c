...
png_configure_decoder(png_structp png_ptr, int *row, int pass)
{
   ...
   png_line_indexp line_index = index->pass_line_index[pass][n];
	...
	if (pstHwStruct)
	{
		pstHwStruct->u8DecMode = HIPNG_SW_DEC;
	}
#endif
   ...
   if (png_ptr->private_ptr)
   {
       ...
       if (!(pstHwStruct->u8DecMode & HIPNG_HW_DEC)
        ...
        && pstHwStruct->u32TmpPhyaddr)
        {
        	...
        	HI_GFX_Unmap(pstHwStruct->u32TmpPhyaddr);
...
#if defined(HIPNG_ACCELERATE) && !defined(CONFIG_GFX_ANDROID_SDK)
{
	 ...
	 if (pstHwStruct)
	 {
	     ...
	     u32Phyaddr = (png_uint_32)HI_GFX_AllocMem(u32RowBytes * info_ptr->height, 16, HI_NULL, HI_NULL);
	     if(u32Phyaddr)
	     {
	        pVir = HI_GFX_Map(u32Phyaddr);
	        if (NULL == pVir)
	        {
