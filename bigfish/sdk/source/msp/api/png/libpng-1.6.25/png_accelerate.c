...
#ifdef HIPNG_MMZ
#ifdef HIGFX_COMMON
            HI_GFX_FreeMem(pstHwctlStruct->pallateaddr);
#else
	    HI_MMZ_Delete(pstHwctlStruct->pallateaddr);
#endif
#else
            HI_GFX_FreeMem(pstHwctlStruct->s32MmzFd, pstHwctlStruct->pallateaddr, pstHwctlStruct->pPallateMemData);
#endif
...
#ifdef HIPNG_MMZ
#ifdef HIGFX_COMMON
                pu32Clut = (HI_U32 *)HI_GFX_MapCached(pstTransform->u32ClutPhyaddr);
#else
...
#ifdef HIPNG_MMZ
#ifdef HIGFX_COMMON
                    HI_GFX_FreeMem(pstTransform->u32ClutPhyaddr);
#else
		    HI_MMZ_Delete(pstTransform->u32ClutPhyaddr);
#endif
#else
                    HI_GFX_FreeMem(pstHwStruct->s32MmzFd, pstTransform->u32ClutPhyaddr, pstHwStruct->pPallateMemData);
#endif
		    return HI_FAILURE;
                }
...
#ifdef HIGFX_COMMON
		HI_GFX_Flush(0);
                HI_GFX_Unmap(pstTransform->u32ClutPhyaddr);
#else
...
#ifdef HIPNG_MMZ
#ifdef HIGFX_COMMON
		    s32Ret = HI_GFX_GetPhyaddr(image[0],  (HI_CHAR *)&u32Phyaddr, &u32Size);
#else
...
#else
		        pstHwStruct->u32TmpPhyaddr = (HI_U32)HI_GFX_AllocMem(pstHwStruct->s32MmzFd, u32MemSize, 
		            16, HI_NULL, HI_NULL, &(pstHwStruct->pTmpMemData));
...
#ifdef HIPNG_MMZ
#ifdef HIGFX_COMMON
					pstHwStruct->pu8TmpImage = (HI_U8 *)HI_GFX_MapCached(pstHwStruct->u32TmpPhyaddr);
#else
...
#ifdef HIPNG_MMZ
#ifdef HIGFX_COMMON
						HI_GFX_FreeMem(pstHwStruct->u32TmpPhyaddr);
#else
