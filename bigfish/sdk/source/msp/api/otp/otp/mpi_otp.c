...
{
    HI_S32 s32Ret = HI_SUCCESS;
    OTP_TEST_DATA_S OtpTest; 
    
    CHECK_OTP_INIT();
    HI_OTP_LOCK();
    
    memcpy(OtpTest.Reserved, u8TestValue, 16);
    s32Ret = ioctl(g_OtpDevFd, CMD_OTP_TEST, &OtpTest);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_OTP("Failed to invoke OTP_TEST\n");
        HI_OTP_UNLOCK();
        return HI_FAILURE;
    }
    memcpy(u8TestValue, OtpTest.Reserved, 16);
