...
static HI_S32 IrParseMessage(HI_CHAR *pMessage, LIRC_KEY_S *pLircKey)
{
    ...
    HI_SYS_GetTimeStampMs(&pLircKey->GotTime);
...
static HI_VOID *IrRecvThread(HI_VOID *args)
{
	...
	while(g_Lirc.bThreadRun)
	{
		...
		{			
			...
			{
		    	/* discard key up*/
			    if ((HI_FALSE == g_Lirc.bUpFlag) || (LastKey.KeyId != CurKey.KeyId)
			        || (HI_UNF_KEY_STATUS_UP == LastKey.KeyStatus))
			    {
			    	pToken = strtok(HI_NULL,"\n");
			    	continue;
			    }
			}
			else if (CurKey.KeyId == LastKey.KeyId)
			{
		    	/* discard key repeat or repeat too frequently*/
			    if ((HI_FALSE == g_Lirc.bRepFlag) 
			        || (CurKey.GotTime - LastKey.GotTime < g_Lirc.u32RepTime)
			        )
			    {
			    	pToken = strtok(HI_NULL,"\n");
					continue;
