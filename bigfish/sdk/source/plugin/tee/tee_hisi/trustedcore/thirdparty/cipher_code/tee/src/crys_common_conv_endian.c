					...
					DxUint32_t  sizeInBytes)
{
	 ...
	 *   buffer starting from non zero MSB */
	 if((DxUint32_t)out32_ptr != (DxUint32_t)in8_ptr){
		 ...
		 /* zeroe empty bytes */
		 out32_ptr[0] = 0;
		 memset((DxUint8_t*)out32_ptr + (sizeInBytes & 3UL), 0, sizeOutBuffBytes - (sizeInBytes & 3UL));
		 /* copy */
		 memcpy((DxUint8_t*)out32_ptr + ((4-(sizeInBytes & 3UL)) & 3UL), in8_ptr, sizeInBytes);
	 }
					...
					DxUint32_t  sizeInBytes)
{
	...
	if((DxUint32_t)out8_ptr != (DxUint32_t)in32_ptr){
		...
		memcpy(out8_ptr, (DxUint8_t*)in32_ptr + ((4-(sizeInBytes & 3UL)) & 3UL), sizeInBytes);
