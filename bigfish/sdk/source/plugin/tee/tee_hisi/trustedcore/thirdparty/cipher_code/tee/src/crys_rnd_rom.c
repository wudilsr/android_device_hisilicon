...
CEXPORT_C CRYSError_t CRYS_RND_GenerateVector( DxUint16_t  RndSize, DxUint8_t *Output_ptr )
{
    ...
    ret = CRYS_GetRandomNumber(Output_ptr, RndSize);
...
CEXPORT_C CRYSError_t CRYS_RND_GenerateVectorInRange(DxUint32_t  rndSizeInBits, DxUint8_t *maxVect_ptr, DxUint8_t *rndVect_ptr )
{
   /* FUNCTION DECLARATIONS */
	...
	DxUint32_t   maxVectSizeBits, maxVectSizeBytes;
    DxUint8_t   smin;
  ...
  /* calculate count of extra bits for shifting */
   ...
   while( k < 0xFFFF )
   {
       ...
       Error = CRYS_GetRandomNumber( rndVect_ptr, (DxUint16_t)rndSizeInBytes);	   
	   if( Error != CRYS_OK )    
		   goto End;
		  ...
		  /* step1 check high part of random */
		   if( checkingSizeBytes == CRYS_AES_BLOCK_SIZE_IN_BYTES )
		   {
			   while( 1 )
			   {
                   ...
                   Error = CRYS_GetRandomNumber( rndVect_ptr + rndSizeInBytes - CRYS_AES_BLOCK_SIZE_IN_BYTES, CRYS_AES_BLOCK_SIZE_IN_BYTES);				   
				   if( Error != CRYS_OK )    
					   goto End;
					...
					DxUint32_t  *tmp_ptr)
{
	...
	*  array							      */
	memcpy((DxUint8_t*)tmp_ptr, (DxUint8_t*)maxVect_ptr, rndSizeInWords*sizeof(DxUint32_t));
...
void test_rnd(void)
{
    ...
    CRYS_RND_GenerateVector(32, rand);

    HI_PRINT_HEX("RAND", rand, 32);
}
#endif
