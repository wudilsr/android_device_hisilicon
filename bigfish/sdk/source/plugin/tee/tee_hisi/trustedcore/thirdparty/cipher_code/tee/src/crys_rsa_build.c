...
//   PRINT_INFO("-->before if( ModulusSize > CRYS_RSA_MAX_VALID_KEY_SIZE_VALUE_IN_BYTES )1\n");//2012/11/17
   if( ModulusSize > CRYS_RSA_MAX_VALID_KEY_SIZE_VALUE_IN_BYTES )
	   return CRYS_RSA_INVALID_MODULUS_SIZE;
//    PRINT_INFO("-->after if( ModulusSize > CRYS_RSA_MAX_VALID_KEY_SIZE_VALUE_IN_BYTES )2\n");//2012/11/17
...
{				                   
   ...
   memset(PrivKey_ptr, 0, sizeof(CRYSRSAPrivKey_t));
    ...
    pu8N = (HI_U8*)PrivKey_ptr->n;
    pu8N += sizeof(PrivKey_ptr->n);
    if (DRV_CIPHER_RSA_CallRMul(pu8N, P_ptr, Q_ptr, 2*PSize, PSize, QSize) != HI_SUCCESS)
    {
        Error = CRYS_FATAL_ERROR;
        goto End;
    }
   Error = CRYS_COMMON_ConvertMsbLsbBytesToLswMswWords(PrivKey_ptr->n, 4*((2*PSize + 3)/4) + 4, pu8N, 2*PSize);
   if(Error)
       return Error;
...
//				Buffers_ptr);
   ...
   memset((DxUint8_t *)Buffers_ptr, 0, sizeof(CRYS_RSAConvertKeyToCrtBuffers_t));
