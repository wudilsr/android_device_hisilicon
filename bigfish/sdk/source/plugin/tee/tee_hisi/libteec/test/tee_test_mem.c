...
/* for test TEEC_MEMREF_PARTIAL_OUTPUT, [0] size > tee size, [1] size < tee size*/
    ...
    result = TEEC_InvokeCommand(
                 ...
                 &origin);
    if (result == TEEC_ERROR_SHORT_BUFFER) {
        TEEC_Debug("[0] : buffer=%s, size=%d\n", testdata, operation.params[0].tmpref.size);
        TEEC_Debug("[1] : buffer=%s, size=%d\n", testdata + 100, operation.params[1].tmpref.size);
        if (operation.params[0].tmpref.size != tee_output_len) {
            TEEC_Error("11: operation.params[0].tmpref.size=%d\n", operation.params[0].tmpref.size);
            goto error;
        } else if (strncmp(testdata, tee_output, tee_output_len) !=0) {
            TEEC_Error("12: testdata=%s\n", testdata);
            goto error;
        } else if (operation.params[1].tmpref.size != tee_output_len) {
            TEEC_Error("13: operation.params[1].tmpref.size=%d\n", operation.params[1].tmpref.size);
            goto error;
        }
    } else {
        TEEC_Error("14: invoke failed, codes=0x%x, origin=0x%x\n", result, origin);
...
/* for test TEEC_MEMREF_PARTIAL_INOUT, [0] size > tee size, [1] size < tee size*/
    ...
    if (result == TEEC_ERROR_SHORT_BUFFER) {
        TEEC_Debug("[0] : buffer=%s, size=%d\n", testdata, operation.params[0].tmpref.size);
        TEEC_Debug("[1] : buffer=%s, size=%d\n", testdata + 100, operation.params[1].tmpref.size);
        if (operation.params[0].tmpref.size != tee_inout_len) {
            TEEC_Error("21: operation.params[0].tmpref.size=%d\n", operation.params[0].tmpref.size);
            goto error;
        } else if (strncmp(testdata, tee_inout, tee_inout_len) !=0) {
            TEEC_Error("22: testdata=%s\n", testdata);
            goto error;
        } else if (operation.params[1].tmpref.size != tee_inout_len) {
            TEEC_Error("23: operation.params[1].tmpref.size=%d\n", operation.params[1].tmpref.size);
            goto error;
        }
    } else {
        TEEC_Error("24: invoke failed, codes=0x%x, origin=0x%x\n", result, origin);
...
/* for test , [0] is INPUT, [1] size > tee size, [2] size > tee size, [3] size < tee size */
    ...
    if (result == TEEC_ERROR_SHORT_BUFFER) {
        TEEC_Debug("[0] : buffer=%s, size=%d\n", testdata, operation.params[0].tmpref.size);
        TEEC_Debug("[1] : buffer=%s, size=%d\n", testdata + 100, operation.params[1].tmpref.size);
        TEEC_Debug("[2] : buffer=%s, size=%d\n", testdata + 200, operation.params[2].tmpref.size);
        TEEC_Debug("[3] : buffer=%s, size=%d\n", testdata + 300, operation.params[3].tmpref.size);
        if (operation.params[1].tmpref.size != tee_output_len) {
            TEEC_Error("31: operation.params[1].tmpref.size=%d\n", operation.params[1].tmpref.size);
            goto error;
        } else if (strncmp(testdata+100, tee_output, tee_output_len) !=0) {
            TEEC_Error("32: testdata+100=%s\n", testdata+100);
            goto error;
        } else if (operation.params[2].tmpref.size != tee_inout_len) {
            TEEC_Error("33: operation.params[2].tmpref.size=%d\n", operation.params[2].tmpref.size);
            goto error;
        } else if (strncmp(testdata+200, tee_inout, tee_inout_len) !=0) {
            TEEC_Error("34: testdata+200=%s\n", testdata+200);
            goto error;
        } else if (operation.params[3].tmpref.size != tee_inout_len) {
            TEEC_Error("35: operation.params[3].tmpref.size=%d\n", operation.params[3].tmpref.size);
            goto error;
        }
    } else {
        TEEC_Error("36: invoke failed, codes=0x%x, origin=0x%x\n", result, origin);
