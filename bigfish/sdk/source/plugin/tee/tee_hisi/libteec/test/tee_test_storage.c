...
int test_write_read_05_times()
{
    ...
    write_fd = TEEC_FOpen("/data/05.txt", TEE_DATA_FLAG_ACCESS_WRITE |TEE_DATA_FLAG_SHARE_READ|TEE_DATA_FLAG_CREATE);
    if (write_fd == -1) {
        TEEC_Error("%s: TEEC_FOpen failed, errno=0x%x\n", __FUNCTION__, TEEC_GetFErr());
        res = 1;
        goto error;
    ...
    read_fd = TEEC_FOpen("/data/05.txt", TEE_DATA_FLAG_ACCESS_READ|TEE_DATA_FLAG_SHARE_READ);
    if (read_fd == -1) {
        TEEC_Error("%s: TEEC_FOpen failed, errno=0x%x\n", __FUNCTION__, TEEC_GetFErr());
        res = 1;
        goto error;
...
int main()
{
    ...
    if (test_read_01() != 0) {
        ...
        TEEC_Debug("test test_read_01 success\n");
    }
    if (test_open_01() != 0) {
