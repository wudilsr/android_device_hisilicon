...
int main(void)
{
    ...
    result = TEEC_InitializeContext(
               ...
               &context);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_1param_memrefWhole_allocated: TEEC_InitializeContext failed\n");
        goto error;
    }
    ...
    result = TEEC_OpenSession(
        ...
        NULL);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_1param_memrefWhole_allocated: TEEC_OpenSession failed, result=0x%x\n", result);
        goto clean;
    }

    result = Invoke_Remember_Expected_ParamTypes(&session);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_1param_memrefWhole_allocated: Invoke_Remember_Expected_ParamTypes failed, result=0x%x\n", result);
        goto clean1;
    }

    result = Invoke_1param_and_check_expectedParameterType (&context, &session);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_1param_memrefWhole_allocated: Invoke_1param_and_check_expectedParameterType   failed, result=0x%x\n", result);
        goto clean1;
    }
    ...
    printf("Invoke_1param_memrefWhole_allocated success\n");
    return 0;
clean1:
    TEEC_CloseSession(&session);
clean:
