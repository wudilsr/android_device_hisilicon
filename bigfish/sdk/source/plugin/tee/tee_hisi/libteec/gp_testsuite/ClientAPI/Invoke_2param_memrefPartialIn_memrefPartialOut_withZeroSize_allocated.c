...
TEEC_Result Invoke_2param_Copy_ParamIn_to_ParamOut(TEEC_Context *context, TEEC_Session *session)
{
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_Copy_ParamIn_to_ParamOut: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_Copy_ParamIn_to_ParamOut: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto clean1;
    }
    ...
    *(uint32_t*)shrdMem01.buffer = VALUE01;
    ...
    operation.params[1].memref.size = ZERO;
...
int main(void)
{
    ...
    result = TEEC_InitializeContext(
               ...
               &context);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_memrefPartialIn_memrefPartialOut_withZeroSize_allocated: TEEC_InitializeContext failed\n");
        goto error;
    }
    ...
    result = TEEC_OpenSession(
        ...
        NULL);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_memrefPartialIn_memrefPartialOut_withZeroSize_allocated: TEEC_OpenSession failed, result=0x%x\n", result);
        goto clean;
    }

    result = Invoke_Remember_Expected_ParamTypes(&session);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_memrefPartialIn_memrefPartialOut_withZeroSize_allocated: Invoke_Remember_Expected_ParamTypes failed, result=0x%x\n", result);
        goto clean1;
    }

    result = Invoke_2param_Copy_ParamIn_to_ParamOut (&context, &session);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_memrefPartialIn_memrefPartialOut_withZeroSize_allocated: Invoke_2param_Copy_ParamIn_to_ParamOut  failed, result=0x%x\n", result);
        goto clean1;
    }
    ...
    printf("Invoke_2param_memrefPartialIn_memrefPartialOut_withZeroSize_allocated success\n");
    return 0;
clean1:
    TEEC_CloseSession(&session);
clean:
