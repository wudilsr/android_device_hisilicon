...
typedef struct SecureBoot_LockInfo_Tag {
    unsigned int user_password[4];      //user password, used as key, should compare to user password
    unsigned int unlock_flag[4];       //unlock flag
    unsigned int relock_flag[4];       //relock flag
    unsigned int encrypted_flag[4];    //flag which is encrypted with user password
