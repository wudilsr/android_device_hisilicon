        ...
        const char *loadfile)
{
    ...
    if ((ret != 1) ||
            (image_head.context_len + sizeof(TEEC_image_head) > MAX_IMAGE_LEN)) {
    ...
    TEEC_Debug("image_head manifest_str_len %d, cipher_bin_len %d\n",
            image_head.manifest_str_len, image_head.cipher_bin_len);

    /* allocate share memory */
    if (image_head.context_len + sizeof(TEEC_image_head) + LOAD_IMAGE_FLAG_OFFSET
            > MAX_SHARE_BUF_LEN) {
        sharedMem.size = MAX_SHARE_BUF_LEN;
    } else {
        sharedMem.size = sizeof(TEEC_image_head) + image_head.context_len
    ...
    total_img_len = image_head.context_len + sizeof(image_head);
    while (total_img_len) {
        ...
        *(int32_t*)file_buf_pt = load_flag;
        ...
        ret = fread(file_buf_pt, 1, send_img_len, fp);
        if (ret != send_img_len) {
