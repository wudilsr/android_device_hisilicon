...
DxStatus DxTzClient_InitSecureContentPath(Dx_Client_Tz_Session pSession){
    ...
    //multiple_size = (pSession->size/align_size + ((pSession->size%align_size)?1:0)) * align_size;
    ...
    TEEC_Debug("!!!!!!!!!!!!!!malloc success!addr 0x%x, size %dKB\n", (uint32_t)pSession->sharedMem->buffer, pSession->sharedMem->size/1024);
    ...
    //TEEC_Debug("111111111111111111111111111111111\n");
    TEEC_Debug("fd = %d, size = %d, offset = %d\n", pSession->session->context->fd,
        pSession->sharedMem->size, pSession->virtual_addr - pSession->base_addr);
    ...
    #endif
    //asm("B .");
    //TEEC_Debug("2222222222222222222222222222222222\n");
...
DxStatus DxTzClient_TerminateSecureContentPath(Dx_Client_Tz_Session pSession){
    ...
    #if 0
    if(pSession->virtual_addr){
        *pSession->virtual_addr = 0xdd;
        TEEC_Debug("addr: 0x%x = 0x%x\n", (uint32_t)pSession->virtual_addr, *pSession->virtual_addr);
    }
    #endif
    TEEC_ReleaseSharedMemory(sharedMem);
    TEEC_Debug("!!!!!!!!!!!!!!!!!DxTzClient_TerminateSecureContentPath ok\n");
    return ret;
}
