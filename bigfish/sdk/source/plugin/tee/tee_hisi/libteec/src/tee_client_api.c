...
//static pthread_mutex_t   mmzMutex = PTHREAD_MUTEX_INITIALIZER;
    ...
    uint32_t* returnOrigin)
{
    ...
    switch (connectionMethod) {
        ...
        case TEEC_LOGIN_IDENTIFY:
            ...
            char linkc[100], path[100];
            int32_t retz;

            memset(path, 0x00, 100);
            (void)snprintf(linkc,100,"/proc/%d/exe", getpid());
            retz = readlink(linkc,path,sizeof(path));
            if(retz == -1)
            {
            ...
            path[retz]='\0';
            retz = strncmp(path,"/system/bin/app_process",23);
            if(!retz)
            {
                TEEC_Debug("java app\n");
                FILE *fd;
                char linkj[100],pathj[100];
                (void)snprintf(linkj,100,"/proc/%d/cmdline", getpid());
                fd=fopen(linkj,"r");
                if(fd==NULL)
                {
                ...
                (void)fread(pathj,1,100,fd);
                fclose(fd);
                strncpy(path, pathj, 100);
            }
            else{
                TEEC_Debug("c app\n");
            }
            operation->params[3].tmpref.buffer = path;
            operation->params[3].tmpref.size = strlen(path)+1;
            break;
        default:
            goto error;
    }
    ...
    TEEC_Operation *operation)
{
    ...
    for (param_cnt = 0;param_cnt < 4;param_cnt++) {
        ...
        } else if (param_type[param_cnt] == TEEC_DX_SHARE_MEM) {
    ...
    TEEC_Operation* operation)
{
    ...
    for (param_cnt = 0;param_cnt < 4;param_cnt++) {
        ...
        if ((param_type[param_cnt] == TEEC_MEMREF_TEMP_INPUT)
            ...
            cli_context->params[param_cnt].memref.buffer
                    = (uint32_t)operation->params[param_cnt].memref.parent;
        } else {
            /* if type is none, ignore it */
        }
    }
