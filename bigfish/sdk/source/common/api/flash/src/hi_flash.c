...
HI_S32 HI_Flash_Erase(HI_HANDLE hFlash, HI_U64 u64Address, HI_U64 u64Len)
{
    ...
    ret = (HI_S32)gFlashInfo[hFlash].pFlashopt->raw_erase(gFlashInfo[hFlash].fd, (unsigned long long)StartAddr, u64Len, gFlashInfo[hFlash].OpenAddr, LimitLeng);
    HI_INFO_FLASH("end.\n");
    pthread_mutex_unlock(&gFlashMutex);
...
HI_S64 HI_Flash_Erase64(HI_HANDLE hFlash, HI_U64 u64Address, HI_U64 u64Len)
{
    ...
    ret = (HI_S64)gFlashInfo[hFlash].pFlashopt->raw_erase(gFlashInfo[hFlash].fd, (unsigned long long)StartAddr, u64Len, gFlashInfo[hFlash].OpenAddr, LimitLeng);
    HI_INFO_FLASH("end.\n");
    pthread_mutex_unlock(&gFlashMutex);
