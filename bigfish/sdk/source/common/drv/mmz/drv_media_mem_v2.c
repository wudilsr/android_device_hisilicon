...
int hil_mmb_force_put(hil_mmb_t *mmb)
{
    ...
    if (mmb->flags & HIL_MMB_RELEASED)
    {
        PRINTK_CA(KERN_WARNING "hil_mmb_force_put: amazing, mmb<%s> is released!\n", mmb->name);
    }
...
static void dump_mmz_mem(void)
{
	...
	HI_PRINT("|%-102s|\n", "Summary:");
	HI_PRINT(SPLIT_LINE);
	HI_PRINT("|  MMZ Total Size  |      Used      |     Idle        |  Zone Number  |   BLock Number                 |\n");
	HI_PRINT(SPLIT_LINE);
...
//EXPORT_SYMBOL(hil_mmb_force_put);
