...
static void dump_mmz_mem(void)
{
	...
	unsigned int ion_used_size = 0, total_used_size = 0;

	HI_PRINT( "|                   PHYS           |  ID  |   KVIRT   |    FLAGS    |  LENGTH(KB)  |       NAME        |\n");
	...
	list_for_each_entry(p, &mmz_list, list){
		...
		HI_PRINT("|ZONE[%d]: (0x%08lx, 0x%08lx)   %d                 0x%08lx      %-10lu   \"%s%-14s|\n",nZoneCnt, \
	...
	HI_PRINT("|%-102s|\n", "Summary:");
	HI_PRINT(SPLIT_LINE);
	HI_PRINT("|  MMZ Total Size  |  MMZ Used  | ION Used  |  Idle   |  Zone Number  |   BLock Number                 |\n");
	HI_PRINT(SPLIT_LINE);
	...
	if (zone_number == 1) {
		...
		if (p->cma_dev) {
			...
			if (cma && cma->count)
				total_used_size = bitmap_weight(cma->bitmap, cma->count) * 4;
		}
	}

	if(0 != mmz_total_size){
		free_size = mmz_total_size - total_used_size;
		ion_used_size = total_used_size - used_size;
		HI_PRINT("|       %d%-8s   %d%-8s    %d%-8s   %d%-6s         %d                 %d               |\n",
			 mmz_total_size/1024, "MB", used_size/1024, "MB", ion_used_size/1024, "MB", free_size/1024, "MB", zone_number, block_number);
		mmz_total_size = 0;
		zone_number = 0;
		block_number = 0;
	}
                  ...
                  int count, int *eof, void *data)
{
    ...
    len = snprintf(page , count, "|  MMZ Total Size  |  MMZ Used  | ION Used  |  Idle   |  Zone Number  |   BLock Number                 |\n");
    memcpy(p_mmz_info_buf, page, len);
    p_mmz_info_buf += len;
	...
	if(0 != mmz_total_size){
	    free_size = mmz_total_size - total_used_size;
	    ion_used_size = total_used_size - used_size;
	    len = snprintf(page, count, "|       %d%-8s   %d%-8s    %d%-8s   %d%-6s         %d                 %d               |\n",
				 mmz_total_size/1024, "MB", used_size/1024, "MB", ion_used_size/1024, "MB", free_size/1024, "MB", zone_number, block_number);
		CHECK_BUFF_OVERFLOW();
		memcpy(p_mmz_info_buf, page, len);
		p_mmz_info_buf += len;
...
int mmz_read_proc(struct seq_file *m, void *v) {
	...
	seq_printf(m, "|%-102s|\n", "Summary:");
	seq_puts(m, SPLIT_LINE);
	seq_puts(m, "|  MMZ Total Size  |  MMZ Used  | ION Used  |  Idle   |  Zone Number  |   BLock Number                 |\n");
	seq_puts(m, SPLIT_LINE);
	...
	if(0 != mmz_total_size){
		free_size = mmz_total_size - total_used_size;
		ion_used_size = total_used_size - used_size;
		seq_printf(m, "|       %d%-8s   %d%-8s    %d%-8s   %d%-6s         %d                 %d               |\n",
				 mmz_total_size/1024, "MB", used_size/1024, "MB", ion_used_size/1024, "MB", free_size/1024, "MB", zone_number, block_number);
		CHECK_BUFF_OVERFLOW();
