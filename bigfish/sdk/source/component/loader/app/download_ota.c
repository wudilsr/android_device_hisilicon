...
static HI_S32 DOWNLOAD_OTA_TUNER_Init(HI_U32 u32TunerPort, HI_LOADER_OTA_PARA_S *pstPara)
{
    ...
    HI_DBG_LOADER("TunerType:%d, DemoType:%d, I2cNo:%d\n", stTunerAttr.enTunerDevType,
                  stTunerAttr.enDemodDevType, stTunerAttr.enI2cChannel);
		...
		HI_UNF_TUNER_LOCK_STATUS_E *penStatus)
{
	...
	do
	{
			...
			case HI_UNF_TUNER_SIG_TYPE_SAT:
				{
					...
					stConnectParam.unConnectPara.stSat.u32ScrambleValue = 0;
					break;
				}
		...
		while (s32loop > 0)
		{
			...
			{
				HI_ERR_LOADER("tuner connect error: relock time %d\n", s32loop);
				continue;
			}
