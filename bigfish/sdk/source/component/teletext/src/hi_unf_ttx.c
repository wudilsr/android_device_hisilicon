...
static HI_U32 s_u32Handle   = 0;
static HI_BOOL s_bIsInitTTX = HI_FALSE;
static pthread_mutex_t s_mutex_interface = PTHREAD_MUTEX_INITIALIZER;

static HI_HANDLE aharrHandle[TTX_MAX_HANDLE] = {0};
static HI_S32 TTX_FindHandle(HI_HANDLE hTTX);
static HI_S32 TTX_StoreHandle(HI_HANDLE hTTX);
static HI_S32 TTX_ClearHandle(HI_HANDLE hTTX);
static HI_S32 TTX_Destroy(HI_HANDLE hTTX);

#define JUDGE_HANDLE(hTTX) \
    do {                                                 \
        if (HI_FAILURE == TTX_FindHandle(hTTX))         \
        {                                               \
            return HI_FAILURE;                          \
        }                                               \
    } while (0)
...
static HI_S32  TTX_PageContext_Init(HI_HANDLE  * phTtx)
{
    TTX_PAGE_CONTEXT_S * pstContext;
    HI_S32 i, j = 0;
    ...
    memset(pstContext->u8szInputNum, '\0', 4);
    ...
    if (0 != pthread_mutex_init(&pstContext->PAGEQueue.Queue_lock, NULL))
    {
        goto INIT_CONTEXTMUTEX_ERROR;
    }

    pstContext->PAGEQueue.pstSegmentFront = HI_NULL;
    pstContext->PAGEQueue.pstSegmentRear = HI_NULL;
    pstContext->PAGEQueue.bFull = HI_FALSE;
    pstContext->pPAGEQueue = &pstContext->PAGEQueue;

    *phTtx = (HI_HANDLE) pstContext;
...
static HI_S32  TTX_PageContext_Create(HI_HANDLE hTtx)
{
    TTX_PAGE_CONTEXT_S * pstContext;

    CHECK_PARAM(hTtx);

    pstContext = (TTX_PAGE_CONTEXT_S *) hTtx;

    /** Start  UpdateTask thread */
    if (0
        != pthread_create(&pstContext->PidUpdateTask, HI_NULL, (HI_VOID * (*)(HI_VOID *))TTX_Parse_UpdateTask,
                          (HI_VOID *)pstContext))
    {
...
static HI_S32 TTX_InitSubQueue(TTX_PAGE_CONTEXT_S *  pstContext, HI_BOOL bSubtitle)
{
    ...
    if (HI_NULL != pstContext->pu8PgQueueMemory)
    {
        ...
        {
            /*if (HI_FAILURE
                == TTX_PesQueue_Reset(pstContext->pPAGEQueue, pstContext->pu8PgQueueMemory, pstContext->u32QueueSize))
            {
                return HI_FAILURE;
            }*/
    ...
    if (HI_SUCCESS != TTX_PesQueue_Init(&pstContext->PAGEQueue, pstContext->pu8PgQueueMemory, u32MallocMemSize))
    {
        TTX_Mem_Free(pstContext->pu8PgQueueMemory);
...
static HI_S32 TTX_PageContext_DeInit(TTX_PAGE_CONTEXT_S * pstContextHead)
{
    ...
    if (0 != pthread_mutex_destroy(&pstContextHead->PAGEQueue.Queue_lock))
    {
        HI_ERR_TTX("TTX_PageContext_DeInit error\n");
    }
...
static HI_S32 TTX_InjectData(HI_HANDLE hTTX, HI_U8 *pu8Data, HI_U32 u32DataSize)
{
    ...
    s32Ret = TTX_Recv_ProcessData(hTTX, pu8Data, u32DataSize);
...
static HI_S32  TTX_CheckPage(HI_HANDLE hTTX, HI_UNF_TTX_CHECK_PARAM_S *  pstCheckParam)
{
    ...
    stPageAddr = pstCheckParam->stPageAddr;
    ...
    if (HI_SUCCESS
        == TTX_Data_GetValidPage(&stPageAddr.u8MagazineNum, &stPageAddr.u8PageNum,
...
static HI_S32 TTX_GetRawData(HI_HANDLE hTTX, TTX_SEGMENT_S_PTR pstSegment)
{
    TTX_PARAM_S *pstParam = HI_NULL;
...
static HI_S32  TTX_RequestRawData(HI_HANDLE hTTX, const HI_UNF_TTX_REQUEST_RAWDATA_S *pstRequestRawData)
{
    ...
    stTtxRequest.u32UserData = (HI_U32)hTTX;
...
HI_S32 HI_UNF_TTX_DeInit(HI_VOID)
{
    ...
    for (i = 0; i < TTX_MAX_HANDLE; i++)
    {
        if (0 != aharrHandle[i])
        {
            ...
            if (HI_SUCCESS != TTX_Destroy(aharrHandle[i]))
            {
...
HI_S32 HI_UNF_TTX_Create(HI_UNF_TTX_INIT_PARA_S* pstInitParam, HI_HANDLE* phTTX)
{
    ...
    HI_UNF_TTX_BUFFER_PARAM_S stBuffParam = {0};

    if ((s_u32Handle >= TTX_MAX_HANDLE) || (!s_bIsInitTTX))
    {
    ...
    pstCurrentPoint->hTTX = (HI_HANDLE)pstParam;
    ...
    *phTTX = (HI_HANDLE)pstParam;

    TTX_MUTEX_LOCK(&s_mutex_interface);
    if (HI_FAILURE == TTX_StoreHandle(*phTTX))
    {
        goto STOREHANDLE_ERROR;
    }
...
static HI_S32 TTX_Destroy(HI_HANDLE hTTX)
{
    ...
    if (HI_FAILURE == TTX_ClearHandle(hTTX))
    {
        HI_ERR_TTX("TTX_ClearHandle error!\n");
    }
...
static HI_S32  TTX_Handle_NumKey(HI_HANDLE hTTX, HI_UNF_TTX_KEY_E enKey)
{
    ...
    pstCurrentPoint->u8szInputNum[3]++;
    if (pstCurrentPoint->u8szInputNum[3] == 1)
    {
        if ((enKey > HI_UNF_TTX_KEY_0) && (enKey < HI_UNF_TTX_KEY_9))
        {
            pstCurrentPoint->u8szInputNum[0] = enKey + 0x30;
            pstCurrentPoint->u8szInputNum[1] = 0x2d;
            pstCurrentPoint->u8szInputNum[2] = 0x2d;
        }
        else
        {
            pstCurrentPoint->u8szInputNum[3] = 0;
        }
    }
    else if (pstCurrentPoint->u8szInputNum[3] == 2)
    {
        pstCurrentPoint->u8szInputNum[1] = enKey + 0x30;
    }
    else  if (pstCurrentPoint->u8szInputNum[3] == 3)
    {
        pstCurrentPoint->u8szInputNum[3] = 0;
        pstCurrentPoint->u8szInputNum[2] = enKey + 0x30;

        u8MagazineNum = (pstCurrentPoint->u8szInputNum[0] - 0x30) % 8;
        u8PageNum = (HI_U8)((pstCurrentPoint->u8szInputNum[1] - 0x30) * 10 + enKey);
...
static HI_S32  TTX_Handle_DirectionKey(HI_HANDLE hTTX, HI_UNF_TTX_KEY_E enKey)
{
    ...
    TTX_PAGE_CONTEXT_S  * pstCurrentPoint = NULL;
    TTX_Filter_S stTtxRequest;
    TTX_SEARCH_DIRECTION_E enSeaDirection;
...
static HI_S32 TTX_Handle_ControlKey(HI_HANDLE hTTX, HI_UNF_TTX_KEY_E enKey)
{
    ...
    {
        pstCurrentPoint->bAutoPlay = pstCurrentPoint->bAutoPlay ? HI_FALSE : HI_TRUE;
        if (HI_SUCCESS != TTX_Recv_AutoPlay(pstParam->hDataRecv, 2))
        {
            HI_WARN_TTX("TTX_Recv_AutoPlay failure!\n");
        }
...
HI_S32 HI_UNF_TTX_ExecCmd(HI_HANDLE hTTX, HI_UNF_TTX_CMD_E enCMD, HI_VOID *pvCMDParam)
{
    ...
    case HI_UNF_TTX_CMD_SETREQUEST:
    {
        ...
        {
            HI_WARN_TTX("pstRawData is null!\n");
            return HI_FAILURE;
        }
#endif
