...
HI_S32 CC608_XDS_Create(HI_U8 u8ModuleID)
{
    ...
    // initial XDS mutex
    (HI_VOID)pthread_mutex_init(&pstObject->XDSMutex,NULL);
...
HI_S32 CC608_XDS_Destroy(HI_U8 u8ModuleID)
{
    ...
    if (HI_NULL != pstObject)
    {
        (HI_VOID)pthread_mutex_destroy(&pstObject->XDSMutex);
    }
    return HI_SUCCESS;
}
...
void CC608_XDS_Reset(HI_U8 u8ModuleID)
{
    ...
    pthread_mutex_lock(&pstObject->XDSMutex);
    ...
    pthread_mutex_unlock(&pstObject->XDSMutex);
}
