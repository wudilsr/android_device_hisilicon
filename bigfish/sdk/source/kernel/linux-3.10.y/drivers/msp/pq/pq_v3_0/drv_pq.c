...
HI_S32 DRV_PQ_SetDemoMode(HI_PQ_DEMO_E enFlags, HI_BOOL bOnOff)
{
    ...
    else if (enFlags == HI_PQ_DEMO_TNR)
    {
#if defined(CHIP_TYPE_hi3716mv410) || defined(CHIP_TYPE_hi3716mv420)
    ...
    else if (enFlags == HI_PQ_DEMO_DEI)
    {
...
HI_S32 DRV_PQ_UpdateVpssPQ(HI_U32 u32HandleNo, HI_VPSS_PQ_INFO_S* pstTimingInfo, PQ_VPSS_CFG_REG_S* pstVPSSReg, PQ_VPSS_WBC_REG_S* pstWbcReg, HI_PQ_VPSS_MODULE_S* pstPQModule)
{
    ...
    {
        pqprint(PQ_PRN_VPSS, "[%s]: HandleNo:%d, InputSrc:%d, Width:%d, Height:%d, FrameRate:%d, Interlace:%d, ColorSys:%d\n",
                __FUNCTION__, u32HandleNo, pstTimingInfo->enInputSrc, pstTimingInfo->u32Width, pstTimingInfo->u32Height,
                pstTimingInfo->u32FrameRate, pstTimingInfo->bInterlace, pstTimingInfo->enColorSys);
        ...
        PQ_MNG_EnableDBDemo(stPqParam.bDemoOnOff[HI_PQ_DEMO_DBM]);
        PQ_MNG_EnableDMDemo(stPqParam.bDemoOnOff[HI_PQ_DEMO_DBM]);
        ...
        PQ_MNG_EnableTnrDemo(stPqParam.bDemoOnOff[HI_PQ_DEMO_TNR]);
        ...
        DRV_PQ_SetDemoDispMode(stPqParam.enDemoMode);
#endif
    }
...
HI_S32 DRV_PQ_UpdateVdpPQ(HI_U32 u32DisplayId, HI_VDP_PQ_INFO_S* pstTimingInfo, S_VDP_REGS_TYPE* pstVDPReg)
{
    ...
    if (pstVDPReg != HI_NULL && pstTimingInfo != HI_NULL)
    {
        ...
        pqprint(PQ_PRN_VDP, "[%s]: DisplayId:%d, Width: %d, Height: %d, OutWidth:%d, OutHeight:%d, bSRState: %d\n",
                __FUNCTION__, u32DisplayId, pstTimingInfo->u32Width, pstTimingInfo->u32Height,
        ...
        s32Ret = PQ_MNG_LoadPhyList(REG_TYPE_VDP, sg_u32SourceMode, sg_u32OutputMode, PQ_BIN_MODULE_ALL);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_PQ("PQ_MNG_LoadPhyList VDP failed!\n");
            return s32Ret;
        ...
        PQ_MNG_EnableDCIDemo(stPqParam.bDemoOnOff[HI_PQ_DEMO_DCI]);
        PQ_MNG_EnableACMDemo(stPqParam.bDemoOnOff[HI_PQ_DEMO_COLOR]);
...
{
    ZME_LAYER_WIN_S stZmeLayerWin;
    memcpy(&stZmeLayerWin, &stZmeWin, sizeof(HI_PQ_ZME_WIN_S));

    return PQ_MNG_ZME_2L_Check(u32InWitdh, u32InHeigh, stZmeLayerWin, pu32OutWitdh, pu32OutHeigh);
}
...
HI_S32 DRV_PQ_SetAlgCalcCfg(HI_PQ_WBC_INFO_S* pstVpssWbcInfo, HI_PQ_CFG_INFO_S* pstVpssCfgInfo)
{
...
#if defined(CHIP_TYPE_hi3798cv200_a)||defined(CHIP_TYPE_hi3798mv100)||defined(CHIP_TYPE_hi3796mv100)
    /* HSharpen or Sharpen */
    ...
    {
        (HI_VOID)PQ_MNG_LoadPhyList(REG_TYPE_VPSS, sg_u32SourceMode, sg_u32OutputMode, PQ_BIN_MODULE_SHARPEN);
...
HI_S32 DRV_PQ_ProcRead(struct seq_file* s, HI_VOID* data)
{
    ...
    PROC_PRINT(s, "%-20s: %-20s\n", "demo", stPqParam.bDemoOnOff[HI_PQ_DEMO_DEI] ? "on" : "off");
...
static HI_VOID DRV_PQ_ProcPrintHelp(HI_VOID)
{
    ...
    HI_DRV_PROC_EchoHelper("echo sharpen    < 0~100  >                               > /proc/msp/pq\n");
    HI_DRV_PROC_EchoHelper("echo dci        < 0~63   >                               > /proc/msp/pq\n");
    HI_DRV_PROC_EchoHelper("echo acm        < 0~1023 >                               > /proc/msp/pq\n");
    ...
    HI_DRV_PROC_EchoHelper("echo printbin   <hex>/all                                > /proc/msp/pq\n");
    HI_DRV_PROC_EchoHelper("echo printtype  <hex>                                    > /proc/msp/pq\n");
    HI_DRV_PROC_EchoHelper("echo sharp/dci/color/sr/tnr/db/dm/deshoot/all enable/disable   > /proc/msp/pq\n");
    HI_DRV_PROC_EchoHelper("echo sharp/dci/color/sr/tnr/dei/all           demoon/demooff   > /proc/msp/pq\n");
    HI_DRV_PROC_EchoHelper("\n");
...
static HI_S32 DRV_PQ_ProcCmdProcee(HI_CHAR* pArg1, HI_CHAR* pArg2)
{
    ...
    if (0 == HI_OSAL_Strncmp(pArg1, "bright", strlen("bright")))
    {
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "zme", strlen("zme")))
    {
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "deshoot", strlen("deshoot")))
    {
    }
    else if (0 == HI_OSAL_Strncmp(pArg1, "fleshtone", strlen("fleshtone")))
    {
        ...
        DRV_PQ_SetFleshToneLevel(enFleshToneLevel);
    }
    else if (0 == HI_OSAL_Strncmp(pArg1, "colormode", strlen("colormode")))
    {
        ...
        DRV_PQ_SetColorEnhanceMode(enColorSpecMode);
    }
    else if (0 == HI_OSAL_Strncmp(pArg1, "sharpen", strlen("sharpen")))
    {
        ...
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_SHARPNESS, HI_TRUE);
        }
        else if (0 == HI_OSAL_Strncmp(pArg2, "demooff", strlen("demooff")))
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_SHARPNESS, HI_FALSE);
        }
        else
        {
            u32Data = (HI_U32)simple_strtol(pArg2, NULL, 10);
            DRV_PQ_SetSharpness(u32Data);
        }
    }
    else if (0 == HI_OSAL_Strncmp(pArg1, "dci", strlen("dci")))
    {
        ...
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_DCI, HI_TRUE);
        }
        else if (0 == HI_OSAL_Strncmp(pArg2, "demooff", strlen("demooff")))
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_DCI, HI_FALSE);
        }
        else
        {
            u32Data = (HI_U32)simple_strtol(pArg2, NULL, 10);
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "color", strlen("color")))
    {
        if (0 == HI_OSAL_Strncmp(pArg2, "enable", strlen("enable")))
        {
        ...
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_COLOR, HI_TRUE);
        }
        else if (0 == HI_OSAL_Strncmp(pArg2, "demooff", strlen("demooff")))
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_COLOR, HI_FALSE);
        }
        else
        {
            u32Data = (HI_U32)simple_strtol(pArg2, NULL, 10);
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "sr", strlen("sr")))
    {
        if (0 == HI_OSAL_Strncmp(pArg2, "enable", strlen("enable")))
        {
        ...
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_SR, HI_TRUE);
        }
        else if (0 == HI_OSAL_Strncmp(pArg2, "demooff", strlen("demooff")))
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_SR, HI_FALSE);
        }
        else
        {
            return HI_FAILURE;
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "all", strlen("all")))
    {
        ...
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_ALL, HI_TRUE);
        }
        else if (0 == HI_OSAL_Strncmp(pArg2, "demooff", strlen("demooff")))
        {
            DRV_PQ_SetDemoMode(HI_PQ_DEMO_ALL, HI_FALSE);
        }
        else
        {
            return HI_FAILURE;
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "deimode", strlen("deimode")))
    {
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "fodmode", strlen("fodmode")))
    {
        if (0 == HI_OSAL_Strncmp(pArg2, "auto", strlen("auto")))
        {
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "printbin", strlen("printbin")))
    {
        if (0 == HI_OSAL_Strncmp(pArg2, "all", strlen("all")))
        {
            PQ_MNG_PrintTable(HI_TRUE, u32Data);
        }
        else
        {
            u32Data = (HI_U32)simple_strtol(pArg2, NULL, 16);
            PQ_MNG_PrintTable(HI_FALSE, u32Data);
        }
    }
    else if (0 == HI_OSAL_Strncmp(pArg1, "printtype", strlen("printtype")))
    {
