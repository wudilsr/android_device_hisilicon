...
HI_S32 TVP_VDEC_GetChanImage( HI_S32 ChanID, IMAGE *pImage )
{
    ...
    result = TEEK_InvokeCommand(&g_TeeSession, VFMW_CMD_ID_VFMW_GETCHANIMAGE, &operation, HI_NULL);
...
HI_S32 TVP_VDEC_ReleaseChanImage(HI_S32 ChanID, IMAGE *pImage )
{
    ...
    result = TEEK_InvokeCommand(&g_TeeSession, VFMW_CMD_ID_VFMW_RELEASECHANIMAGE, &operation, HI_NULL);
    if (result != TEEC_SUCCESS)
    {
...
static HI_S32 TVP_VDEC_ReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    if (g_bSecEnvSetUp && g_SecureInstNum > 0)
    {
        ...
        result = TEEK_InvokeCommand(&g_TeeSession, VFMW_CMD_ID_VFMW_READPROC, &operation, HI_NULL);
        if (result != TEEC_SUCCESS)
        {
...
HI_S32 TVP_VDEC_TrustDecoderInit(VDEC_OPERATION_S *pArgs)
{
    ...
    result = TEEK_InvokeCommand(&g_TeeSession, VFMW_CMD_ID_VDEC_INITWITHOPERATION, &operation, HI_NULL);
...
InitWithfree_4:
    ...
    operation.params[0].value.a = 0;
    operation.params[0].value.b = TEEC_VALUE_UNDEF;
    TEEK_InvokeCommand(&g_TeeSession, VFMW_CMD_ID_VDEC_EXIT, &operation, HI_NULL);
