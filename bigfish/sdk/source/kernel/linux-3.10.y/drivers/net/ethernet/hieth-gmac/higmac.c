...
next_mii:
		...
		if (!gmac) {/* first time */
			...
			higmac_board_info[2].phy_intf = phy_intf;
#endif
			gmac++;
			s = strchr(s, ',');
			if (s)
...
{
	char str[MAX_PARAM_LEN], buf[MAX_TAGS_LEN];
	char *tag = &buf[0];
	unsigned char mac[6];
	int i, len;
...
#if (CONFIG_GMAC_NUMS > 2)
	memset(str, 0, sizeof(str));
	len = get_param_data("phyio2", str, sizeof(str));
	if (len > 0) {
		memcpy(&higmac_board_info[2].gpio_base, str, sizeof(u32) * 2);
		pr_info("phy2_gpiobase=0x%x, gpio_bit=%x\n",
				higmac_board_info[2].gpio_base,
...
#endif
#if (CONFIG_GMAC_NUMS > 2)
	ret = higmac_mdiobus_driver_init(ld[2], mdio_bus[2]);
	if (ret) {
		pr_err("register mdio bus%d failed!\n", ld[2]->index);
		higmac_mdiobus_driver_exit(ld[1]);
		return ret;
	}
#endif
...
static struct resource higmac_resources[] = {
	...
	[2] = {/* eth2 io space */
		.start	= CONFIG_HIGMAC_THIRD_IOBASE,
		.end	= CONFIG_HIGMAC_THIRD_IOBASE + HIGMAC_IOSIZE - 1,
		.flags	= IORESOURCE_MEM,
	},
	[3] = {/* gmac0 irq */
	...
	[5] = {/* gmac2 irq */
		.start	= CONFIG_HIGMAC_IRQNUM + 3,
		.end	= CONFIG_HIGMAC_IRQNUM + 3,
		.flags	= IORESOURCE_IRQ,
	}
};
