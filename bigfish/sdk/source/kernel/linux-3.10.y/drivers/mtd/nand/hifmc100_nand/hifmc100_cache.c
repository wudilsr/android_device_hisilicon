...
#define pr_fmt(fmt) "hifmc_cache: " fmt
...
#define HIFMC_CACHE_FLAGS_STOP      0x01
#define HIFMC_CACHE_FLAGS_CRC32     0x02
#define HIFMC_CACHE_FLAGS_PEND      0x04
	unsigned int flags;
	...
	unsigned int nr_remove[HIFMC_CACHE_REMOVE_CAUSE_MAX];
	unsigned int sz_cache;
};

#define CF_CRC_ON(_cache, _flags)          ((_cache)->_flags & HIFMC_CACHE_FLAGS_CRC32)
#define CF_CRC_ON_SET(_cache, _flags)      ((_cache)->_flags |= HIFMC_CACHE_FLAGS_CRC32)
#define CF_CRC_ON_CLEAN(_cache, _flags)    ((_cache)->_flags &= ~HIFMC_CACHE_FLAGS_CRC32)

#define CF_CACHE_OFF(_cache, _flags)       ((_cache)->_flags & HIFMC_CACHE_FLAGS_STOP)
#define CF_CACHE_OFF_SET(_cache, _flags)   ((_cache)->_flags |= HIFMC_CACHE_FLAGS_STOP)
#define CF_CACHE_OFF_CLEAN(_cache, _flags) ((_cache)->_flags &= ~HIFMC_CACHE_FLAGS_STOP)

#define CF_CACHE_PEND(_cache, _flags)       ((_cache)->_flags & HIFMC_CACHE_FLAGS_PEND)
#define CF_CACHE_PEND_SET(_cache, _flags)   ((_cache)->_flags |= HIFMC_CACHE_FLAGS_PEND)
#define CF_CACHE_PEND_CLEAN(_cache, _flags) ((_cache)->_flags &= ~HIFMC_CACHE_FLAGS_PEND)

#define TO_CACHE(_cache, _handle)  \
	struct hifmc_cache *_cache = (struct hifmc_cache *)_handle

struct hifmc_cache_node {
...
#define RB_CACHE_ENTRY(_rbnode) \
	rb_entry(_rbnode, struct hifmc_cache_node, rbnode);
...
static int hifmc_dbgfs_init(struct hifmc_cache *cache, struct dentry *parent);
static void hifmc_dbgfs_destory(struct hifmc_cache *cache);

/******************************************************************************/

static int page_compare(struct hifmc_cache_node *node, unsigned int count,
			unsigned int index)
{
	if (node->count > count)
		return -1;
...
static struct hifmc_cache_node *
find_cache_node(struct hifmc_cache *cache, const unsigned int count,
		const unsigned int index)
{
	...
	while (node) {
		struct hifmc_cache_node *leaf = RB_CACHE_ENTRY(node);
...
static void update_cache_min_count(struct hifmc_cache *cache)
{
	struct rb_node *rbnode;
	struct hifmc_cache_node *node;
...
static struct hifmc_cache_node *
insert_cache_node(struct hifmc_cache *cache, struct hifmc_cache_node *newnode,
...
static void remove_cache_node(struct hifmc_cache *cache,
			      struct hifmc_cache_node *node, int free_cache,
			      int update_min_count, unsigned int cause)
{
	rb_erase(&node->rbnode, &cache->rbroot);
	cache->nr_cache--;

	if (cause < HIFMC_CACHE_REMOVE_CAUSE_MAX)
	...
	if (free_cache) {
		...
		cache->sz_cache -= sizeof(struct hifmc_cache_node);
		cache->nr_alloc--;
	}
...
static struct hifmc_cache_node *
new_cache_node(struct hifmc_cache *cache, unsigned int count,
...
static void free_all_cache_node(struct hifmc_cache *cache)
{
	struct rb_node *rbnode = rb_first(&cache->rbroot);

	while (rbnode) {
		struct hifmc_cache_node *node = RB_CACHE_ENTRY(rbnode);
		rbnode = rb_next(rbnode);

		remove_cache_node(cache, node, 1, 1,
			HIFMC_CACHE_REMOVE_CAUSE_MAX);
	}
}
/******************************************************************************/

static void hifmc_reclaim_cache(struct hifmc_cache *cache, int nr_page,
				int cause)
{
	struct rb_node *rbnode;
	struct hifmc_cache_node *node;
...
static int hifmc_shrinker(struct shrinker *shrink, struct shrink_control *sc)
{
	unsigned int nr_caches;
	unsigned int shrink_percent;
	struct hifmc_cache *cache =
		container_of(shrink, struct hifmc_cache, shrink);
	...
	hifmc_reclaim_cache(cache, nr_caches, HIFMC_CACHE_REMOVE_CAUSE_RECLAIM);
		     ...
		     char **poob, int *status)
{
	...
	*status = 0;
	...
	if (pages[index] & EMPTY_MARK)
		*status |= HIFMC_CACHE_STATUS_EMPTY_PAGE;
	...
	remove_cache_node(cache, node, 0, 0, HIFMC_CACHE_REMOVE_CAUSE_MOVE);
		     ...
		     char **poob)
{
	...
	if (cache->nr_cache == cache->max_caches) {
		struct hifmc_cache_node *first_node;
...
static int hifmc_cache_crc(void *handle, unsigned int index, int checkit)
{
	TO_CACHE(cache, handle);
...
{
	return hifmc_cache_crc(handle, index, 0);
}
/******************************************************************************/

int hifmc_cache_check_crc(void *handle, unsigned int index)
{
	return hifmc_cache_crc(handle, index, 1);
}
/******************************************************************************/

static int __hifmc_cache_enable(void *handle)
{
	TO_CACHE(cache, handle);
...
static int __hifmc_cache_disable(void *handle)
{
	TO_CACHE(cache, handle);
	...
	hifmc_reclaim_cache(cache, cache->nr_cache,
		HIFMC_CACHE_REMOVE_CAUSE_MANUAL);
...
void hifmc_cache_enable(void *handle, int enable)
{
	...
	if (enable)
		__hifmc_cache_enable(handle);
	else
		__hifmc_cache_disable(handle);

	mutex_unlock(&cache->mutex);
}
/******************************************************************************/

static int hifmc_crc_enable(void *handle)
{
	TO_CACHE(cache, handle);
...
static int hifmc_crc_disable(void *handle)
{
	TO_CACHE(cache, handle);
...
int hifmc_cache_crc_enable(void *handle, int enable)
{
	...
	if (!!CF_CRC_ON(cache, flags) + !!enable == 1) {
		if (enable)
			ret = hifmc_crc_enable(handle);
		else
			ret = hifmc_crc_disable(handle);
	}
...
void hifmc_read_cache_finish(void *handle)
{
	...
	if (cache->active.index != -1) {
		...
		if (remove)
			hifmc_remove_cache(handle, index, 1, reset_count,
				cause);
...
static void hifmc_set_weight(void *handle, struct hifmc_weight *weight,
			     int nr_weight)
{
	TO_CACHE(cache, handle);
		     ...
		     unsigned int oobsize, struct dentry *parent)
{
	...
	*handle = NULL;
	...
	cache = (struct hifmc_cache *)vmalloc(sizeof(struct hifmc_cache));
	if (!cache) {
		pr_err("out of memory.\n");
		return -ENOMEM;
	}
	memset(cache, 0, sizeof(struct hifmc_cache));
	...
	cache->rbcache = kmem_cache_create("nand_rbcache",
		sizeof(struct hifmc_cache_node), 0, 0, NULL);
	if (!cache->rbcache) {
		pr_err("out of memory.\n");
		goto rbcache_fail;
	}
	...
	if (hifmc_dbgfs_init(cache, parent))
		goto dbgfs_fail;
	...
	cache->shrink.shrink = hifmc_shrinker;
				...
				size_t count, loff_t *ppos)
{
	...
	if (*ptr == '?') {
		...
		hifmc_remove_cache(cache, index, count, 0,
			HIFMC_CACHE_REMOVE_CAUSE_MANUAL);
		...
		hifmc_remove_cache(cache, index, count, 1,
			HIFMC_CACHE_REMOVE_CAUSE_MANUAL);
		...
		struct hifmc_weight weight[max_weight];
...
next:
		...
		hifmc_set_weight(cache, weight, ix);
...
#define CP_BUF(_fmt, _arg...) do { \
	...
	CP_BUF("%d\n", "Remove(erase)", cache->nr_remove[HIFMC_CACHE_REMOVE_CAUSE_ERASE]);
	CP_BUF("%d\n", "Remove(write)", cache->nr_remove[HIFMC_CACHE_REMOVE_CAUSE_WRITE]);
	CP_BUF("%d\n", "Remove(empty page)", cache->nr_remove[HIFMC_CACHE_REMOVE_CAUSE_EMPTY_PAGE]);
	CP_BUF("%d\n", "Remove(manual)", cache->nr_remove[HIFMC_CACHE_REMOVE_CAUSE_MANUAL]);
	CP_BUF("%d\n", "Remove(reclaim)", cache->nr_remove[HIFMC_CACHE_REMOVE_CAUSE_RECLAIM]);
	CP_BUF("%d\n", "Remove(move)", cache->nr_remove[HIFMC_CACHE_REMOVE_CAUSE_MOVE]);
	CP_BUF("%d\n", "Remove(full)", cache->nr_remove[HIFMC_CACHE_REMOVE_CAUSE_FULL]);
...
static void set_max_cache(struct hifmc_cache *cache, unsigned int max_caches)
{
	...
	if (max_caches < cache->nr_cache)
		hifmc_reclaim_cache(cache, cache->nr_cache - max_caches,
			HIFMC_CACHE_REMOVE_CAUSE_MANUAL);
			       ...
			       size_t count, loff_t *ppos)
{
	...
	if (*ptr == '?') {
		...
		if (!memcmp(ptr, "on", 3)) {
			hifmc_cache_enable(cache, 1);
		} else if (!memcmp(ptr, "off", 4)) {
			hifmc_cache_enable(cache, 0);
		}
		...
		if (!memcmp(ptr, "on", 3)) {
			hifmc_cache_crc_enable(cache, 1);
		} else if (!memcmp(ptr, "off", 4)) {
			hifmc_cache_crc_enable(cache, 0);
		}
