...
#ifndef CONFIG_HIFMC100_DBG_NAND_NUM_OF_LOGS
#  define CONFIG_HIFMC100_DBG_NAND_NUM_OF_LOGS      (80)
#endif /* CONFIG_HIFMC100_DBG_NAND_NUM_OF_LOGS */

#ifndef CONFIG_HIFMC100_DBG_NAND_LOG_LENGTH
#  define CONFIG_HIFMC100_DBG_NAND_LOG_LENGTH       (40)
#endif /* CONFIG_HIFMC100_DBG_NAND_LOG_LENGTH */

struct hifmc100_dbg_dump_item_t {
	unsigned short hour;
	unsigned short min;
	unsigned short sec;
	unsigned short msec;
	...
	unsigned char data[CONFIG_HIFMC100_DBG_NAND_LOG_LENGTH];
};

struct hifmc100_dbg_dump_t {
	...
	struct hifmc100_dbg_dump_item_t logs[CONFIG_HIFMC100_DBG_NAND_NUM_OF_LOGS];

	unsigned int read_index;
};

static DEFINE_MUTEX(dbg_dump_mutex);
static struct hifmc100_dbg_dump_t *dbg_dump = NULL;
			       ...
			       size_t count, loff_t *ppos)
{
	...
	struct hifmc100_dbg_dump_item_t *logs;

	if (*ppos == 0) {
		if (dbg_dump->count
		    < CONFIG_HIFMC100_DBG_NAND_NUM_OF_LOGS)
	...
	for (read_index = dbg_dump->read_index;
		...
		if (read_index >= CONFIG_HIFMC100_DBG_NAND_NUM_OF_LOGS)
			read_index = 0;
...
static int dbgfs_dump_init(struct dentry *root, struct hifmc_host *host)
{
	struct hifmc100_dbg_dump_t *dump;

	if (dbg_dump)
		return 0;

	dump = vmalloc(sizeof(struct hifmc100_dbg_dump_t));
	if (!dump) {
		pr_err("Can't allocate memory.\n");
		return -ENOMEM;
	}
	memset(dump, 0, sizeof(struct hifmc100_dbg_dump_t));
...
static int dbgfs_dump_uninit(struct hifmc_host *host)
{
	if (!dbg_dump)
		return 0;
...
static void dbg_dump_rw(struct hifmc_host *host, char op)
{
	int offset;
	...
	if (logs->length > CONFIG_HIFMC100_DBG_NAND_LOG_LENGTH)
		logs->length = CONFIG_HIFMC100_DBG_NAND_LOG_LENGTH;
	...
	if (++dbg_dump->index >= CONFIG_HIFMC100_DBG_NAND_NUM_OF_LOGS)
		dbg_dump->index = 0;
...
static void dbg_dump_read(struct hifmc_host *host)
{
	dbg_dump_rw(host, 'R');
}
/*****************************************************************************/

static void dbg_dump_write(struct hifmc_host *host)
{
	dbg_dump_rw(host, 'W');
}
/*****************************************************************************/

static void dbg_dump_erase(struct hifmc_host *host)
{
	struct hifmc100_dbg_dump_item_t *logs;
...
struct hifmc100_dbg_inf_t hifmc100_dbg_inf_dump = {
	"dump", 0,
	dbgfs_dump_init,
	dbgfs_dump_uninit,
	dbg_dump_read,
