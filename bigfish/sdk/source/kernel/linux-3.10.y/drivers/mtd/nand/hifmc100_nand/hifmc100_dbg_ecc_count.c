...
#ifndef CONFIG_HIFMC100_DBG_ECC_COUNT_NUM
#  define CONFIG_HIFMC100_DBG_ECC_COUNT_NUM          (100)
#endif /* CONFIG_HIFMC100_DBG_ECC_COUNT_NUM */

struct hifmc100_dbg_ecc_count_item_t{
	unsigned int page;
...
struct hifmc100_dbg_ecc_count_t {
...
#define GET_ITEM(_ecc_count, _index)     \
	((struct hifmc100_dbg_ecc_count_item_t *)((_ecc_count)->item + \
		((sizeof(struct hifmc100_dbg_ecc_count_item_t) + \
			(_ecc_count)->ecc.section) * (_index))))

static DEFINE_MUTEX(dbg_ecc_count_mutex);
static struct hifmc100_dbg_ecc_count_t *dbg_ecc_count = NULL;
/*****************************************************************************/
...
static int dbgfs_dump_ecc(struct hifmc100_dbg_ecc_count_t *ecc_count,
			  char __user *usrbuf, unsigned int count, int withhdr)
{
	...
	unsigned int index;
	char buf[128] = {0};
	struct hifmc100_dbg_ecc_count_item_t *item;
	...
	for (index = ecc_count->read_index;
		...
		if (index >= CONFIG_HIFMC100_DBG_ECC_COUNT_NUM)
			index = 0;
...
static int dbgfs_dump_ecc_dist(struct hifmc100_dbg_ecc_count_t *ecc_count,
				    ...
				    size_t count, loff_t *ppos)
{
	...
	if (*ppos == 0) {
		if (dbg_ecc_count->count
		    < CONFIG_HIFMC100_DBG_ECC_COUNT_NUM)
...
static int dbgfs_ecc_count_init(struct dentry *root, struct hifmc_host *host)
{
	unsigned int size;
	unsigned int pagesize;
	unsigned int chipsize;
	struct hifmc100_dbg_ecc_count_t *ecc_count;

	if (dbg_ecc_count)
		return 0;

	size = sizeof(struct hifmc100_dbg_ecc_count_t);
	size += (CONFIG_HIFMC100_DBG_ECC_COUNT_NUM + 1) *
		(sizeof(struct hifmc100_dbg_ecc_count_item_t) +
		 host->ecc.section);
	...
	ecc_count->item = (char *)ecc_count +
		sizeof(struct hifmc100_dbg_ecc_count_t);
...
static int dbgfs_ecc_count_uninit(struct hifmc_host *host)
{
	if (!dbg_ecc_count)
		return 0;
...
static void dbg_ecc_log(struct hifmc_host *host,
			struct hifmc100_dbg_ecc_count_item_t *item,
			struct hifmc100_dbg_ecc_count_t *ecc_count)
{
	int ix;
...
static void dbg_ecc_count_read(struct hifmc_host *host)
{
	unsigned int page;
	struct hifmc100_dbg_ecc_count_item_t *item;
	...
	if (++dbg_ecc_count->index >= CONFIG_HIFMC100_DBG_ECC_COUNT_NUM)
...
struct hifmc100_dbg_inf_t hifmc100_dbg_inf_ecc_count = {
	"ecc_count", 0,
