...
static int hifmc100_hynix_bg_cdie_get_rr_param(void *host)
{
	...
	/* according to hynix doc, no need to config OP_WAIT_READY_EN */
	...
	regval = HIFMC100_OP_CMD2_EN(1)
		| HIFMC100_OP_CMD1_EN(1);
	hifmc_write(hifmc_host, regval, HIFMC100_OP);
	FMC_CMD_WAIT_CPU_FINISH(hifmc_host);
	...
	/* according to hynix doc, need to config OP_WAIT_READY_EN */
	regval = HIFMC100_OP_CFG_FM_CS(hifmc_host->chipselect)
...
static int hifmc100_hynix_bg_cdie_set_rr_param(void *host,
					       int param)
{
	unsigned char *rr;
	struct hifmc_host *hifmc_host = (struct hifmc_host *)host;
	...
	return hifmc100_hynix_bg_cdie_set_rr_reg((struct hifmc_host *)hifmc_host,
						(char *)rr);
}
/*****************************************************************************/
...
{
	return hifmc100_hynix_bg_cdie_set_rr_param(host, 0);
}
/*****************************************************************************/
