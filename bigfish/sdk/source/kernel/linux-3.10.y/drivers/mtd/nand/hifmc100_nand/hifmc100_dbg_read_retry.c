...
#ifndef CONFIG_HIFMC100_DBG_READ_RETRY_NUM
#  define CONFIG_HIFMC100_DBG_READ_RETRY_NUM          (100)
#endif /* CONFIG_HIFMC100_DBG_READ_RETRY_NUM */

struct hifmc100_dbg_read_retry_item_t {
	unsigned int page;
...
struct hifmc100_dbg_read_retry_t {
	...
	struct hifmc100_dbg_read_retry_item_t item[CONFIG_HIFMC100_DBG_READ_RETRY_NUM];
};

static DEFINE_MUTEX(dbg_read_retry_mutex);
static struct hifmc100_dbg_read_retry_t *dbg_read_retry = NULL;
/*****************************************************************************/
				    ...
				    size_t count, loff_t *ppos)
{
	...
	struct hifmc100_dbg_read_retry_item_t *item;

	if (*ppos == 0) {
		if (dbg_read_retry->count
		    < CONFIG_HIFMC100_DBG_READ_RETRY_NUM)
	...
	for (read_index = dbg_read_retry->read_index;
		...
		if (read_index >= CONFIG_HIFMC100_DBG_READ_RETRY_NUM)
			read_index = 0;
...
static int dbgfs_read_retry_init(struct dentry *root, struct hifmc_host *host)
{
	unsigned int pagesize;
	unsigned int chipsize;
	struct hifmc100_dbg_read_retry_t *read_retry;
	...
	read_retry = vmalloc(sizeof(struct hifmc100_dbg_read_retry_t));
	if (!read_retry) {
		pr_err("Can't allocate memory.\n");
		return -ENOMEM;
	}
	memset(read_retry, 0, sizeof(struct hifmc100_dbg_read_retry_t));
...
static int dbgfs_read_retry_uninit(struct hifmc_host *host)
{
	if (!dbg_read_retry)
		return 0;
...
static void hifmc100_dbg_read_retry_rr(struct hifmc_host *host, int index)
{
	unsigned int page;
	struct hifmc100_dbg_read_retry_item_t *item;
	...
	if (++dbg_read_retry->index >= CONFIG_HIFMC100_DBG_READ_RETRY_NUM)
...
struct hifmc100_dbg_inf_t hifmc100_dbg_inf_read_retry = {
	"read_retry", 0,
