...
struct hifmc100_dbg_erase_count_t {
...
static struct hifmc100_dbg_erase_count_t *dbg_erase_count = NULL;
	...
	while (pos < count) {
		...
		case 'c':
			...
			memset(dbg_erase_count->pe, 0,
			       dbg_erase_count->blocknum *
			       sizeof(struct hifmc100_dbg_erase_count_t));
...
static int dbgfs_erase_count_init(struct dentry *root, struct hifmc_host *host)
{
	unsigned int size;
	unsigned int blocknum;
	...
	struct hifmc100_dbg_erase_count_t *erase_count;
...
static int dbgfs_erase_count_uninit(struct hifmc_host *host)
{
	if (!dbg_erase_count)
		return 0;
...
static void dbg_erase_count_erase(struct hifmc_host *host)
{
...
struct hifmc100_dbg_inf_t hifmc100_dbg_inf_erase_count = {
	"erase_count", 0,
