...
{
	int regval = (host->NFC_CFG & HISFC400_CFG_PAGEISZE_MASK) >> HISFC400_CFG_PAGEISZE_SHIFT;
...
void hisfc400_set_pagesize(struct hisfc_host *host, int pagesize)
{
	...
	int regval = match_type_to_reg(hisfc400_pagesize,
...
{
	int regval = (host->NFC_CFG & HISFC400_CFG_ECCTYPE_MASK) >> HISFC400_CFG_ECCTYPE_SHIFT;
...
{
	int regval = match_type_to_reg(hisfc400_ecc, ARRAY_SIZE(hisfc400_ecc),
