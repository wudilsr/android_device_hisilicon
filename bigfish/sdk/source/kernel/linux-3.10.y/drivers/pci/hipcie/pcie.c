...
static void * __iomem pcie0_sys_base_virt;
static void * __iomem pcie1_sys_base_virt;
...
#include "pcie_s40v3.c"
#elif(defined(CONFIG_ARCH_HI3716MV410))
#include "pcie_mv410.c"
#elif(defined(CONFIG_ARCH_HIFONE))
#include "pcie_hifone.c"
#else
#error You must have defined either CONFIG_ARCH_HI3798MX or ...
#endif
...
static int __init pcie0_mem_parser(char *str)
{
	...
	if (size >= 0x10000000)
		size=CONFIG_PCIE0_DEVICES_MEM_SIZE;
		//size = 0x7800000;
...
static irqreturn_t hipcie_linkdown_isr(int irq, void *dev_id)
{
...
static int init_pcie_controller_info(int controller)
{
	...
	pcie_info[pcie_controllers_nr].conf_base_addr =
		(void __iomem *)ioremap_nocache(config_base_addr, 0x1000);
	...
	pcie_info[pcie_controllers_nr].base_addr =
		(void __iomem *)ioremap_nocache(dbi_base_addr,
				dbi_base_size);
...
static inline void __iomem * to_pcie_address(struct pci_bus *bus,
		unsigned int devfn, int where)
{
	...
	address = (unsigned int)(info->base_addr) | PCIE_CFG_BUS(bus->number)
		...
		int where, int size, u32 *value)
{
	...
	tmp = readl(pcie0_sys_base_virt + PCIE_SYS_CTRL0);
	tmp |= (1 << pcie_bit_reg_dev_ctrl);
	writel(tmp, pcie0_sys_base_virt + PCIE_SYS_CTRL0);
	
	tmp = readl(pcie0_sys_base_virt + PCIE_SYS_CTRL1);
	tmp |= (1 << pcie_bit_reg_dev_ctrl);
	writel(tmp, pcie0_sys_base_virt + PCIE_SYS_CTRL1);
#endif
	...
	tmp = readl(pcie1_sys_base_virt + PCIE_SYS_CTRL0);
	tmp |= (1 << pcie_bit_reg_dev_ctrl);
	writel(tmp, pcie1_sys_base_virt + PCIE_SYS_CTRL0);
	
	tmp = readl(pcie1_sys_base_virt + PCIE_SYS_CTRL1);
	tmp |= (1 << pcie_bit_reg_dev_ctrl);
	writel(tmp, pcie1_sys_base_virt + PCIE_SYS_CTRL1);
#endif
	val = (u32)readl((void *)(info->conf_base_addr + (where & (~0x3))));
#if 1
...
static int __init pcie_init(void)
{
	...
	pcie0_sys_base_virt = ioremap_nocache(PCIE0_SYS_BASE_PHYS, 0x1000);
	if (!pcie0_sys_base_virt) {
		pcie_error("Cannot map system controller register base!");
		return -EIO;
	}
#ifdef CONFIG_PCIE1_SEL
	pcie1_sys_base_virt = ioremap_nocache(PCIE1_SYS_BASE_PHYS, 0x1000);
	if (!pcie1_sys_base_virt) {
		pcie_error("Cannot map system controller register base!");
		return -EIO;
	}
#endif	
...
Err_controller:
	...
	if (pcie0_sys_base_virt)
		iounmap(pcie0_sys_base_virt);
#ifdef CONFIG_PCIE1_SEL	
	if (pcie1_sys_base_virt)
		iounmap(pcie1_sys_base_virt);
#endif	
	return -EIO;
...
static int hisi_pcie_plat_driver_probe(struct platform_device *pdev)
{
	return 0;
}
static int hisi_pcie_plat_driver_remove(struct platform_device *pdev)
{
	return 0;
}
