...
static HI_VOID reset_evt_statistics_nolock(HI_VOID)
{
  ...
  */
 HI_S32 low_delay_start_statistics(HI_LD_Scenes_E scenes_id, HI_VOID *  filter)
 {
    HI_U32 index;
    ...
    g_active_evt_head.filter_handle  = (HI_U32)filter;
...
static HI_S32 low_delay_proc_read(struct seq_file *s, HI_VOID *pArg)
{
    ...
    if (!dup)
    {
        printk("create dup evt head failed.\n");
        goto out;
    }
...
HI_VOID low_delay_notify_event(HI_LD_Event_S * evt)
{
    ...
    {
        HI_DBG_STAT("filter invalid evt[name:'%s', handle:%u, frame:%u, time:%u].\n", g_event_name[evt->evt_id], evt->handle, evt->frame, evt->time);
        goto out;
    }
    ...
    if ( !evt_queue->active) 
    {
        HI_DBG_STAT("filter inactive evt[name:'%s', handle:%u, frame:%u, time:%u].\n", g_event_name[evt->evt_id], evt->handle, evt->frame, evt->time);
        goto out;
    }
