...
static struct ion_handle *tui_handle;
static ion_phys_addr_t ion_addr;
...
int get_frame_addr(void)
{
	...
	if (!tui_handle) {
		...
		tui_handle = ion_alloc(tui_client, 1<<fls(screen_r),
			0, HEAP_ID_MASK, ION_FLAG_SECURE_BUFFER);
	...
	tui_dbg("width=%d height=%d ion_addr=0x%x len =0x%llx  memory=%d M\n",
			dss_fd->panel_info.xres, dss_fd->panel_info.yres,
				(unsigned int)ion_addr, len, 1<<fls(screen_r));
	}
