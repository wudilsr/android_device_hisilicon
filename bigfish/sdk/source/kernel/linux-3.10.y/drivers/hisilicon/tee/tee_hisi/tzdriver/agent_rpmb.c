...
extern int mmc_blk_ioctl_rpmb_cmd(struct block_device *bdev,
				  ...
				  int stage)
{
	...
	if (IS_ERR(bdev)) {
		if (0 == stage) /* statge 0 failed can be ignored */
			TCDEBUG("open %s failed with %ld in stage-%d\n",
...
static void send_ioccmd(struct tee_agent_kernel_ops *agent_instance)
{
    ...
    ret = mmc_blk_ioctl_rpmb_cmd(bdev, rpmb_data);
    if (ret)
