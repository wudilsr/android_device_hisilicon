...
static int __init xhci_hcd_init(void)
{
	...
	retval = platform_device_register(&hiusb_xhci1_platdev);
	if (retval < 0) {
		printk(KERN_ERR "%s->%d, platform_device_register xhci1 fail.\n",
				__func__, __LINE__);
		return -ENODEV;
	}
#endif
...
#ifdef CONFIG_ARCH_HIFONE
	platform_device_unregister(&hiusb_xhci1_platdev);
#endif
#endif
	return retval;
}
