...
#  undef CONFIG_HAVE_ARM_LOCAL_TIMER_HIFONE
#  undef CONFIG_HAVE_SP804_LOCAL_TIMER_HIFONE
#endif
...
#ifdef CONFIG_HAVE_SP804_LOCAL_TIMER_HIFONE
...
static int __cpuinit hifone_local_timer_setup(struct clock_event_device *evt)
{
...
static void hifone_local_timer_stop(struct clock_event_device *evt)
{
...
static struct local_timer_ops hifone_timer_tick_ops __cpuinitdata = {
	.setup	= hifone_local_timer_setup,
	.stop	= hifone_local_timer_stop,
};

static void __init hifone_local_timer_init(void)
{
	unsigned int cpu = 0;
	...
	local_timer_register(&hifone_timer_tick_ops);
}
#endif /* CONFIG_HAVE_SP804_LOCAL_TIMER_HIFONE */

/*****************************************************************************/
#ifdef CONFIG_HAVE_ARM_LOCAL_TIMER_HIFONE
...
struct hifone_clocksource {
	void __iomem *base;
	struct clocksource clksrc;
};

static struct hifone_clocksource hifone_clocksource = {0};

static inline struct hifone_clocksource *to_hifone_clksrc(struct clocksource *cs)
{
	return container_of(cs, struct hifone_clocksource, clksrc);
}

static void hifone_clocksource_start(void __iomem *base)
{
...
static cycle_t hifone_clocksource_read(struct clocksource *cs)
{
	return ~readl_relaxed(to_hifone_clksrc(cs)->base + TIMER_VALUE);
}

static notrace u32 hifone_sched_clock_read(void)
{
	return ~readl_relaxed(hifone_clocksource.base + TIMER_VALUE);
}

static void hifone_clocksource_resume(struct clocksource *cs)
{
	hifone_clocksource_start(to_hifone_clksrc(cs)->base);
}

static void __init hifone_clocksource_init(void __iomem *base, const char *name)
{
	long rate = sp804_get_clock_rate(name);
	struct clocksource *clksrc = &hifone_clocksource.clksrc;
	...
	clksrc->name   = name;
	clksrc->rating = 200;
	clksrc->read   = hifone_clocksource_read;
	clksrc->mask   = CLOCKSOURCE_MASK(32),
	clksrc->flags  = CLOCK_SOURCE_IS_CONTINUOUS,
	clksrc->resume = hifone_clocksource_resume,

	hifone_clocksource.base = base;

	hifone_clocksource_start(base);

	clocksource_register_hz(clksrc, rate);

	setup_sched_clock(hifone_sched_clock_read, 32, rate);
}

void __init hifone_timer_init(void)
{
#ifdef CONFIG_HAVE_ARM_LOCAL_TIMER_HIFONE
...
#ifdef CONFIG_HAVE_SP804_LOCAL_TIMER_HIFONE
	hifone_local_timer_init();
#endif

	hifone_clocksource_init((void *)TIMER(3)->addr,
		TIMER(3)->name);
