...
HI_S32 HI_TEE_HDMI_CreateHandle(HI_HANDLE* phHDMI, HI_TEE_HDMI_ID_E enHdmi)
{
    ...
    *phHDMI = pstHdmiCtx;
    ...
    HDMI_INFO("HI_List_Add %d, gHdmiListCount:%d\n", pstHdmiCtx, gHdmiListCount);
...
HI_S32 HI_TEE_HDMI_DestoryHandle(HI_HANDLE hHDMI)
{
    ...
    s32Ret = HI_TEE_HDMI_GetHDCPStrategy(pstHdmiCtx->enHdmi,&enCurHdcpLevel);
    ...
    {
        pHdmiContext = HI_LIST_ENTRY(pList, HDMI_CONTENT_S, list);
        HDMI_INFO("HI_List_For_Each:ctxaddress:%d hHdcp:%d", pHdmiContext, pHdmiContext->enHdcp);
    ...
    if(enCurHdcpLevel != enHdcpLevel)
    {
        HDMI_INFO("HI_TEE_HDMI_SetHDCPStrategy hdcplevel:%d", enHdcpLevel);
        s32Ret = HI_TEE_HDMI_SetHDCPStrategy(enHdmi, enHdcpLevel);
        if(HI_SUCCESS != s32Ret)
        {
            HDMI_ERR("HI_TEE_HDMI_SetHDCPStrategy fail: ret:%d", s32Ret);
            return s32Ret;
        }
    }
...
HI_S32 HI_TEE_HDMI_GetHDCPStatus(HI_HANDLE hHDMI, HI_TEE_HDMI_HDCP_STATUS_E *penHdcpStatus)
{
    ...
    if(HI_SUCCESS != s32Ret)
    {
        HDMI_ERR("__HI_DRV_HDMI_GetHDCPStatus fail: ret:%d", s32Ret);
        return HI_FAILURE;
    }
...
HI_S32 HI_TEE_HDMI_SetHDCPStrategy(HI_HANDLE hHDMI, HI_TEE_HDMI_HDCP_LEVLEL_E enHdcp)
{
    ...
    if(enCurHdcpLevel < enHdcp)
    {
        HDMI_INFO("set HDCP enHdcp:%d", enHdcp);
        s32Ret = __HI_DRV_HDMI_SetHDCPStrategy(pstHdmiCtx->enHdmi, pstHdmiCtx->enHdcp);
        if(HI_SUCCESS != s32Ret)
        {
            HDMI_ERR("HI_TEE_MON_SetHDCPStrategy fail: s32Ret:%d", s32Ret);
            return s32Ret;
        }
    }
