...
#define     TEST_TZ_ADVCA_DCAS_SET_CW          51
#define     TEST_TZ_ADVCA_DCAS_SET_CLEAN_CW    52
#define     TEST_TZ_ADVCA_DCAS_CLEAR_CW        53
#define     TEST_TZ_ADVCA_DCAS_GET_RESPONSE_TO_CHALLENGE  54
...
static int dcas_setCW(TEEC_Session session, uint32_t hKey)
{
    TEEC_Operation operation = {0};
    TEEC_Result result;
    uint32_t origin;
    ...
    operation.started = 1;
    operation.params[0].value.a = hKey;
    operation.paramTypes = TEEC_PARAM_TYPES(
        TEEC_VALUE_INPUT,
        TEEC_NONE,
        TEEC_NONE,
...
static int dcas_setCleanCW(TEEC_Session session, uint32_t hKey)
{
    TEEC_Operation operation = {0};
    TEEC_Result result;
    uint32_t origin;
    ...
    result = TEEC_InvokeCommand(
                 &session,
                 TEST_TZ_ADVCA_DCAS_SET_CLEAN_CW,
                 &operation,
                 &origin);
    if (result != TEEC_SUCCESS)
    {
...
static int dcas_clearCW(TEEC_Session session, uint32_t hKey)
{
    TEEC_Operation operation = {0};
    TEEC_Result result;
    uint32_t origin;
...
static int dcas_getChipID(TEEC_Session session)
{
    TEEC_Operation operation = {0};
    TEEC_Result result;
    uint32_t origin;
...
static int dcas_getResponseToChallenge(TEEC_Session session)
{
    TEEC_Operation operation = {0};
    TEEC_Result result;
    uint32_t origin;
    ...
    result = TEEC_InvokeCommand(
                 &session,
                 TEST_TZ_ADVCA_DCAS_GET_RESPONSE_TO_CHALLENGE,
                 &operation,
                 &origin);
    if (result != TEEC_SUCCESS)
    {
...
int main(int argc,char *argv[])
{
    ...
    if (argc <= 1)
    {
        printf("Usage: ./test_advca mode [value]\n"
               ...
               "       ./test_advca 7 0x201(pid) to test set encrypted CW\n"
               "       ./test_advca 8 0x201(pid) to test set clean CW\n"
               "       ./test_advca 9 0x201(pid) to test clear CW\n"
               "       ./test_advca 10  to test get chip ID\n"
               "       ./test_advca 11  to test get response to challenge\n");
        return -1;
    }
    ...
    switch(mode)
    {
        ...
        case 6:
            ...
            break;
        case 7:
            dcas_setCW(session, value);
            break;
        case 8:
            dcas_setCleanCW(session, value);
            break;
        case 9:
            dcas_clearCW(session, value);
            break;
        case 10:
            dcas_getChipID(session);
            break;
        case 11:
            dcas_getResponseToChallenge(session);
            break;
        default:
