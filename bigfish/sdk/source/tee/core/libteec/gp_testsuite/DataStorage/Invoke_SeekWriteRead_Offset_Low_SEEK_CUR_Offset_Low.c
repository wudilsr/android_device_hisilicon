...
TEEC_Result Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_Low(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_Low: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_Low: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_Low(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_Low is failed\n");
        goto error;
    }
    printf("Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_Low is successful!\n");
