...
TEEC_Result Invoke_InitObjectAndAttributes_src_is_persistent(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_src_is_persistent: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_src_is_persistent: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_InitObjectAndAttributes_src_is_persistent(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_InitObjectAndAttributes_src_is_persistent is failed!\n");
        goto error;
    }
    printf("Invoke_InitObjectAndAttributes_src_is_persistent is successful!\n");
