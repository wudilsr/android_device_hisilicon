...
TEEC_Result Invoke_InitObjectAndAttributes_TEE_TYPE_AES_success_attribute_TEE_ATTR_SECRET_VALUE_correct_size(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_TEE_TYPE_AES_success_attribute_TEE_ATTR_SECRET_VALUE_correct_size: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_TEE_TYPE_AES_success_attribute_TEE_ATTR_SECRET_VALUE_correct_size: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    ...
    retrieveBufferFromUint32(sizeof(TEE_ATTR_AES_256_VALUE01 ), shrdMem01.buffer+8);

    void * p2=shrdMem02.buffer;
    strncpy(p2,TEE_ATTR_AES_256_VALUE01 ,sizeof(TEE_ATTR_AES_256_VALUE01 ));
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_InitObjectAndAttributes_TEE_TYPE_AES_success_attribute_TEE_ATTR_SECRET_VALUE_correct_size(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_InitObjectAndAttributes_TEE_TYPE_AES_success_attribute_TEE_ATTR_SECRET_VALUE_correct_size is failed!\n");
        goto error;
    }
    printf("Invoke_InitObjectAndAttributes_TEE_TYPE_AES_success_attribute_TEE_ATTR_SECRET_VALUE_correct_size is successful!\n");
