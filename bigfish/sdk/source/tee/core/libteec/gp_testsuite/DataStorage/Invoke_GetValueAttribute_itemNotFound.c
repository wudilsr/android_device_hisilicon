...
TEEC_Result Invoke_GetValueAttribute_itemNotFound(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetValueAttribute_itemNotFound: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetValueAttribute_itemNotFound: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetValueAttribute_itemNotFound: TEEC_AllocateSharedMemory shrdMem03failed\n");
        goto error0;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetValueAttribute_itemNotFound: TEEC_AllocateSharedMemory shrdMem04failed\n");
        goto error_1;
    }
    ...
    retrieveBufferFromUint32(VALUE_ATTRIBUTE , shrdMem04.buffer);
    retrieveBufferFromUint32(TEE_ATTR_DH_X_BITS , shrdMem04.buffer+4);
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_GetValueAttribute_itemNotFound(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_GetValueAttribute_itemNotFound is failed\n");
        goto error;
    }
    printf("Invoke_GetValueAttribute_itemNotFound is successful!\n");
