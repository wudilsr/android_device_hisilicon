...
TEEC_Result Invoke_GenerateKey_TEE_TYPE_AES_correct_size(TEEC_Context *context)
{
    TEEC_Result result;
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GenerateKey_TEE_TYPE_AES_correct_size: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_GenerateKey_TEE_TYPE_AES_correct_size(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_GenerateKey_TEE_TYPE_AES_correct_size is failed!\n");
        goto error;
    }
    printf("Invoke_GenerateKey_TEE_TYPE_AES_correct_size is successful!\n");
