...
TEEC_Result Invoke_InitObjectAndAttributes_handle_different_from_TEE_HANDLE_NULL(TEEC_Context* context)
{
    TEEC_Result result; 
    TEEC_Operation operation;
    TEEC_SharedMemory shrdMem01, shrdMem02;
    uint32_t OUT_ExpectedReturn = TEE_ERROR_BAD_PARAMETERS  ;
    printf("-----------come into invoke------------\n");
    //allocate shared memory 
    shrdMem01.size = BIG_SIZE;
    shrdMem01.flags = TEEC_MEM_INPUT;
    result = TEEC_AllocateSharedMemory(context, &shrdMem01);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_handle_different_from_TEE_HANDLE_NULL: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_handle_different_from_TEE_HANDLE_NULL: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    ...
    printf("Invoke_InitObjectAndAttributes_handle_different_from_TEE_HANDLE_NULL success!\n");
    result = TEEC_SUCCESS;
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_InitObjectAndAttributes_handle_different_from_TEE_HANDLE_NULL(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_InitObjectAndAttributes_handle_different_from_TEE_HANDLE_NULL failed\n");
        goto error;
    }
    printf("Invoke_InitObjectAndAttributes_handle_different_from_TEE_HANDLE_NULL test success!\n");
