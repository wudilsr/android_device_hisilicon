...
TEEC_Result Invoke_InitObjectAndAttributes_TEE_TYPE_DH_KEYPAIR_success_attribute_TEE_ATTR_DH_SUBPRIME(TEEC_Context* context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_TEE_TYPE_DH_KEYPAIR_success_attribute_TEE_ATTR_DH_SUBPRIME : TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_TEE_TYPE_DH_KEYPAIR_success_attribute_TEE_ATTR_DH_SUBPRIME : TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    ...
    retrieveBufferFromUint32(TEE_ATTR_DH_SUBPRIME   , shrdMem01.buffer+48);
    retrieveBufferFromUint32(offset, shrdMem01.buffer+52);
    retrieveBufferFromUint32(sizeof(TEE_ATTR_DH_SUBPRIME_VALUE01  ), shrdMem01.buffer+56);
    offset +=sizeof(TEE_ATTR_DH_SUBPRIME_VALUE01  );
    ...
    p2+=sizeof(TEE_ATTR_DH_PRIVATE_VALUE_VALUE01);
    strncpy(p2,TEE_ATTR_DH_SUBPRIME_VALUE01,sizeof(TEE_ATTR_DH_SUBPRIME_VALUE01));
    ...
    operation.params[1].memref.size = 6*12;
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_InitObjectAndAttributes_TEE_TYPE_DH_KEYPAIR_success_attribute_TEE_ATTR_DH_SUBPRIME(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_InitObjectAndAttributes_TEE_TYPE_DH_KEYPAIR_success_attribute_TEE_ATTR_DH_SUBPRIME is failed!\n");
        goto error;
    }
    printf("Invoke_InitObjectAndAttributes_TEE_TYPE_DH_KEYPAIR_success_attribute_TEE_ATTR_DH_SUBPRIME is successful!\n");
