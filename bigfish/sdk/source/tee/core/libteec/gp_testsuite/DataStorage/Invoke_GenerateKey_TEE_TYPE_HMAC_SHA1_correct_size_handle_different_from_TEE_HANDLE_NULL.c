...
TEEC_Result Invoke_GenerateKey_TEE_TYPE_HMAC_SHA1_correct_size_handle_different_from_TEE_HANDLE_NULL(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GenerateKey_TEE_TYPE_HMAC_SHA1_correct_size_handle_different_from_TEE_HANDLE_NULL: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GenerateKey_TEE_TYPE_HMAC_SHA1_correct_size_handle_different_from_TEE_HANDLE_NULL: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    ...
    operation.params[3].value.b = SIZE_HMAC_SHA1_256 ;
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_GenerateKey_TEE_TYPE_HMAC_SHA1_correct_size_handle_different_from_TEE_HANDLE_NULL(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_GenerateKey_TEE_TYPE_HMAC_SHA1_correct_size_handle_different_from_TEE_HANDLE_NULL is failed!\n");
        goto error;
    }
    printf("Invoke_GenerateKey_TEE_TYPE_HMAC_SHA1_correct_size_handle_different_from_TEE_HANDLE_NULL is successful!\n");
