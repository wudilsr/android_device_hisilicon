...
TEEC_Result Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_High(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_High: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_High: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    ...
    operation.params[0].value.a = TEE_DATA_SEEK_CUR ;
    operation.params[0].value.b = OFFSET_HIGH ;

    operation.params[1].value.a = OFFSET_LOW ;
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_High(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_High is failed\n");
        goto error;
    }
    printf("Invoke_SeekWriteRead_Offset_Low_SEEK_CUR_Offset_High is successful!\n");
