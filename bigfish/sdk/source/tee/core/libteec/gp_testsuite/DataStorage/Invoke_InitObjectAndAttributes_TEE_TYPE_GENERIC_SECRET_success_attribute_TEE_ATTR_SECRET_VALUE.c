...
TEEC_Result Invoke_InitObjectAndAttributes_TEE_TYPE_GENERIC_SECRET_success_attribute_TEE_ATTR_SECRET_VALUE(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_TEE_TYPE_GENERIC_SECRET_success_attribute_TEE_ATTR_SECRET_VALUE: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_InitObjectAndAttributes_TEE_TYPE_GENERIC_SECRET_success_attribute_TEE_ATTR_SECRET_VALUE: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    ...
    retrieveBufferFromUint32(sizeof(TEE_ATTR_GENERIC_SECRET_2048_VALUE01), shrdMem01.buffer+8);

    void * p2=shrdMem02.buffer;
    strncpy(p2,TEE_ATTR_GENERIC_SECRET_2048_VALUE01,sizeof(TEE_ATTR_GENERIC_SECRET_2048_VALUE01));

    operation.params[0].value.a = TEE_TYPE_GENERIC_SECRET ;
    operation.params[0].value.b = SIZE_GENERIC_SECRET_2048 ;
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_InitObjectAndAttributes_TEE_TYPE_GENERIC_SECRET_success_attribute_TEE_ATTR_SECRET_VALUE(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_InitObjectAndAttributes_TEE_TYPE_GENERIC_SECRET_success_attribute_TEE_ATTR_SECRET_VALUE is failed!\n");
        goto error;
    }
    printf("Invoke_InitObjectAndAttributes_TEE_TYPE_GENERIC_SECRET_success_attribute_TEE_ATTR_SECRET_VALUE is successful!\n");
