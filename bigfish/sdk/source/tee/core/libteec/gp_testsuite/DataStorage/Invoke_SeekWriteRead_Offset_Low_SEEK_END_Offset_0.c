...
TEEC_Result Invoke_SeekWriteRead_Offset_Low_SEEK_END_Offset_0(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_SeekWriteRead_Offset_Low_SEEK_END_Offset_0: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_SeekWriteRead_Offset_Low_SEEK_END_Offset_0: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    ...
    operation.params[0].value.a = TEE_DATA_SEEK_END ;
    operation.params[0].value.b = OFFSET_0 ;
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_SeekWriteRead_Offset_Low_SEEK_END_Offset_0(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_SeekWriteRead_Offset_Low_SEEK_END_Offset_0 is failed\n");
        goto error;
    }
    printf("Invoke_SeekWriteRead_Offset_Low_SEEK_END_Offset_0 is successful!\n");
