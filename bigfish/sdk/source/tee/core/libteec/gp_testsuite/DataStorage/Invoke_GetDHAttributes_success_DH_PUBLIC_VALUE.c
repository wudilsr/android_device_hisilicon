...
TEEC_Result Invoke_GetDHAttributes_success_DH_PUBLIC_VALUE(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetDHAttributes_success_DH_PUBLIC_VALUE: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetDHAttributes_success_DH_PUBLIC_VALUE: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetDHAttributes_success_DH_PUBLIC_VALUE: TEEC_AllocateSharedMemory shrdMem03failed\n");
        goto error0;
    }
    ...
    operation.params[0].value.b = TEE_ATTR_DH_PUBLIC_VALUE ;
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_GetDHAttributes_success_DH_PUBLIC_VALUE(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_GetDHAttributes_success_DH_PUBLIC_VALUE is failed\n");
        goto error;
    }
    printf("Invoke_GetDHAttributes_success_DH_PUBLIC_VALUE is successful!\n");
