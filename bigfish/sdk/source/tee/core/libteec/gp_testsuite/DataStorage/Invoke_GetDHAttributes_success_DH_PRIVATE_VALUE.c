...
TEEC_Result Invoke_GetDHAttributes_success_DH_PRIVATE_VALUE(TEEC_Context *context)
{
    TEEC_Result result; 
    TEEC_Operation operation;
    TEEC_SharedMemory shrdMem01, shrdMem02, shrdMem03;
    uint32_t offset = 0;
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetDHAttributes_success_DH_PRIVATE_VALUE: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    memset(shrdMem01.buffer, 0, shrdMem01.size);
    strncpy(shrdMem01.buffer,TEE_ATTR_DH_PRIME_VALUE01 ,sizeof(TEE_ATTR_DH_PRIME_VALUE01));
    
    shrdMem02.size = DS_BIG_SIZE;
    shrdMem02.flags = TEEC_MEM_INPUT;
    result = TEEC_AllocateSharedMemory(context, &shrdMem02);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetDHAttributes_success_DH_PRIVATE_VALUE: TEEC_AllocateSharedMemory shrdMem02 failed\n");
        goto error2;
    }
    memset(shrdMem02.buffer, 0, shrdMem02.size);
    strncpy(shrdMem02.buffer,TEE_ATTR_DH_BASE_VALUE01  ,sizeof(TEE_ATTR_DH_BASE_VALUE01 ));

    shrdMem03.size = DS_BIG_SIZE;
    shrdMem03.flags = TEEC_MEM_OUTPUT;
    result = TEEC_AllocateSharedMemory(context, &shrdMem03);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_GetDHAttributes_success_DH_PRIVATE_VALUE: TEEC_AllocateSharedMemory shrdMem03failed\n");
        goto error0;
    }
    ...
    operation.params[0].value.a = BUFFER_ATTRIBUTE ;
    operation.params[0].value.b = TEE_ATTR_DH_PRIVATE_VALUE ;
    
    operation.params[1].memref.parent = &shrdMem01;
    operation.params[1].memref.size = sizeof(TEE_ATTR_DH_PRIME_VALUE01);
    operation.params[1].memref.offset = OFFSET0 ;
    
    operation.params[2].memref.parent = &shrdMem02;
    operation.params[2].memref.size = sizeof(TEE_ATTR_DH_BASE_VALUE01 );
    ...
    result = TEEC_InvokeCommand(&session, CMD_DS_GetDHAttributes, &operation, &origin);
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_GetDHAttributes_success_DH_PRIVATE_VALUE(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_GetDHAttributes_success_DH_PRIVATE_VALUE is failed\n");
        goto error;
    }
    printf("Invoke_GetDHAttributes_success_DH_PRIVATE_VALUE is successful!\n");
