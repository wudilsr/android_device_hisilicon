...
TEEC_Result Invoke_ReadObjectDataAttribute_above_count(TEEC_Context *context)
{
    TEEC_Result result; 
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_ReadObjectDataAttribute_above_count: TEEC_AllocateSharedMemory shrdMem01 failed\n");
        goto error;
    }
    ...
    operation.params[0].value.a = LARGER_THAN_INITIAL ;
    ...
    cmp_count = LARGER_THAN_INITIAL>INITIAL_DATA_SIZE ? INITIAL_DATA_SIZE:LARGER_THAN_INITIAL;
...
int main(void)
{
    ...
    //start test
    if((result= Invoke_ReadObjectDataAttribute_above_count(&context)) != 0)   //CASE_GET_OBJECT_BUFFER_ATTRIB_BIT28
    {
        TEEC_Error("Invoke_ReadObjectDataAttribute_above_count is failed!\n");
        goto error;
    }
    printf("Invoke_ReadObjectDataAttribute_above_count is successful!\n");
