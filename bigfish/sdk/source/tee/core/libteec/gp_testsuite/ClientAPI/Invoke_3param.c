...
TEEC_Result Invoke_3param_and_check_expectedParameterType(TEEC_Context *context, TEEC_Session *session)
{
    TEEC_Operation operation;
    TEEC_Result result;
    ...
    if (result != TEEC_SUCCESS) {
        printf("Invoke_3param_and_check_expectedParameterType : TEEC_InvokeCommand failed, result = 0x%x\n", result);
        goto error;
    }

    if ((operation.params[0].value.a != VALUE01)  || (operation.params[0].value.b != TEEC_VALUE_UNDEF)){
        printf("Invoke_3param_and_check_expectedParameterType : "
...
int main(void)
{
    ...
    result = TEEC_InitializeContext(
               ...
               &context);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_3param: TEEC_InitializeContext failed\n");
        goto error;
    }
    ...
    result = TEEC_OpenSession(
        ...
        NULL);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_3param: TEEC_OpenSession failed, result=0x%x\n", result);
        goto clean;
    }

    result = Invoke_Remember_Expected_ParamTypes(&session);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_3param: Invoke_Remember_Expected_ParamTypes failed, result=0x%x\n", result);
        goto clean1;
    }

    result = Invoke_3param_and_check_expectedParameterType(&context, &session);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_3param: Invoke_3param_and_check_expectedParameterType   failed, result=0x%x\n", result);
        goto clean1;
    }
