...
int main(void)
{
    ...
    result = TEEC_InitializeContext(
               ...
               &context);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_memrefPartialIn_memrefPartialOut_allocated: TEEC_InitializeContext failed\n");
        goto error;
    }
    ...
    result = TEEC_OpenSession(
        ...
        NULL);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_memrefPartialIn_memrefPartialOut_allocated: TEEC_OpenSession failed, result=0x%x\n", result);
        goto clean;
    }

    result = Invoke_Remember_Expected_ParamTypes(&session);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_memrefPartialIn_memrefPartialOut_allocated: Invoke_Remember_Expected_ParamTypes failed, result=0x%x\n", result);
        goto clean1;
    }

    result = Invoke_2param_Copy_ParamIn_to_ParamOut (&context, &session);
    if (result != TEEC_SUCCESS) {
        printf("Invoke_2param_memrefPartialIn_memrefPartialOut_allocated: Invoke_2param_Copy_ParamIn_to_ParamOut  failed, result=0x%x\n", result);
        goto clean1;
    }
    ...
    printf("Invoke_2param_memrefPartialIn_memrefPartialOut_allocated success\n");
    return 0;
clean1:
    TEEC_CloseSession(&session);
clean:
