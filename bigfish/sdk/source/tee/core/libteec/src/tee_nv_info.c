...
int handle_nv(SecureBoot_LockInfo *info){
    ...
    if(NV_NAME_LENGTH < (strlen(WIDEVINE_NV_WVLOCK_NAME) + 1))
    {
	TEEC_Error("nv name len is short than str(WIDEVINE_NV_WVLOCK_NAME) %d\n", strlen(WIDEVINE_NV_WVLOCK_NAME) + 1);
	return -1;
    }

    strncpy(nvinfo.nv_name, WIDEVINE_NV_WVLOCK_NAME, strlen(WIDEVINE_NV_WVLOCK_NAME)+1);
    ...
    memcpy(info, nvinfo.nv_data, sizeof(SecureBoot_LockInfo));
