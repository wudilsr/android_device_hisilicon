...
#define SSA_SERVICE_NAME "SS_AGENT"
...
typedef struct __attribute__ ((__packed__)) nwd_cmd_t {
...
#define SESSION_MAX         16  //concurrent opened session count
...
TEE_Result process_open_session(smc_cmd_t* cmd, uint32_t cmd_type, TEE_Result ret);

TEE_Result process_unlock_agent(struct TA_to_global_msg *ret_msg, uint32_t task_id);
TEE_Result process_open_session(smc_cmd_t* cmd, uint32_t cmd_type, TEE_Result ret);
TEE_Result process_open_session_error(smc_cmd_t* cmd, TEE_Result ret);
