...
#endif
//fpc
#ifdef TEE_FINGERPRINT
#include "fpc1020_common.h"
#include "tee_fpc1020.h"
#endif
...
extern int __mmap(UINT32 phy_addr , UINT32 size, UINT32 *virt_addr,secure_mode_type secure_mode, cache_mode_type cache_mode);
extern int __unmap(UINT32 va_addr, UINT32 size);
...
/*memmove*/
extern void *__memmove(void *dest, const void *src, UINT32 n);
extern void *__memcpy(void *dest, const void *src, UINT32 n);
...
extern INT32 __get_taskmem_buff(UINT32 *addr, UINT32 *size);
extern void __reset_taskmem_buff(UINT32 ta_exec_addr, UINT32 ta_exec_size, UINT8 ta_load_state);
...
/*sec boot*/
extern int __hisi_secboot_copy_soc_data(paddr_t dst_addr, const paddr_t src_addr, unsigned len);
extern UINT32 __hisi_secboot_soc_verification(int socType, UINT32 vrlAddress, paddr_t imageAddress, unsigned int lock_state);
extern int __hisi_secboot_soc_reset(int soc_type);
extern int __hisi_secboot_soc_set(int soc_type, int reset_type);
...
//fpc
#ifdef TEE_FINGERPRINT
extern INT32 __FPC_InitCapture(fpc1020_setup_t* setup);
extern INT32 __FPC_ReadImage(fpc_image_t* image);
#endif
//backtraces
...
extern CRYSError_t  __CC_CRYS_PKCS5_PBKDF2_HMAC1(
    CIPHER_PBKDF2_S                *stInfo,
    DxUint8_t                *DataOut_ptr);
...
extern void __get_provision_key(DX_UTIL_OemKey_t pOemKey);
...
extern unsigned int __rpmb_partition_info_write(unsigned int start_blk, unsigned int total_blk);
...
char* __tui_map_fb(unsigned int size);
int __tui_unmap_fb(char* addr);
int __init_tui_resource(int init);
int __init_tui_sdriver(int secure);
