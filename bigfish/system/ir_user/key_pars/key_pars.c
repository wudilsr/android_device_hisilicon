...
#define PARS_LOG(fmt ...) printf(fmt)

#define KEY_XML_PATH  "/etc/key.xml"
...
xmlNodePtr readXml(char *filename)
{
    ...
    if (mDoc == NULL){
        PARS_LOG("can't open the key.xml\n");
    }
    ...
    if (xmlStrcmp(curNode->name, BAD_CAST "key_xml")){
...
void Pars_Key(xmlNodePtr nodeptr, struct _key_arry *keyarry)
{
    ...
    while (xmlChildNodePtr != NULL) {
        ...
        if (pchar != NULL) {
            ir_keycode = strtoull(pchar,NULL,16);
        }
        ...
        if (pchar != NULL) {
            ...
            if (ret != 0 ) {
                ...
                keynum = keynum +1;
                PARS_LOG("   linux_keycode=0x%04x, ir_keycode=0x%.08llx, %s\n",linux_keycode,ir_keycode,pchar);
                if (keynum >= KEYNUMMAX) {
