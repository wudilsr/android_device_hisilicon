/***********************************************************************************
 * just reference sdk/source/msp/api/ir/, 2013,04,20
 ***********************************************************************************/
		...
		char *pszProtocolName, signed int  s32NameSize, unsigned int u32TimeoutMs)
{
	...
	if (pszProtocolName && (s32NameSize - 1 <= 0 || s32NameSize < PROTOCOL_NAME_SZ))
	{
	...
	if (Ret == (signed int)sizeof(struct key_attr))
	{
		if (pszProtocolName)
		{
			memcpy(pszProtocolName, Irkey.protocol_name, (unsigned int)(s32NameSize - 1));
		...
		*pu64KeyId = Irkey.lower;
		if (Irkey.upper) {
			IR_LOG("This infrared code constains more than"\
					" 64bits data!\n");
		}
	}
	else
