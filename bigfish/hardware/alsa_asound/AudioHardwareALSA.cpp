...
size_t AudioHardwareALSA::getBufferSize(uint32_t sampleRate, int channelCount)
{
    ...
    if (sampleRate == 11025) {
        bufSize = sampleRate/15;
    } else if (sampleRate == 22050) {
        bufSize = sampleRate/15;
    } else if (sampleRate == 32000) {
        bufSize = sampleRate/15;
    } else if (sampleRate == 44100) {
        bufSize = sampleRate/20;
    } else if (sampleRate == 48000) {
        bufSize = sampleRate/10;
    } else {
        bufSize = sampleRate/10;
    }
    //bufSize = sampleRate/30;
...
{
/*
  1.SetMode method is useless for out product
  2.Some phone app will use this method to process device,
  for us, it's no defference.So we return success directly.
*/
/*
    ...
    if (mode != mMode) {
        ...
        if (status == NO_ERROR) {
            ...
            for(ALSAHandleList::iterator it = mDeviceList.begin();
                ...
                if(it->curDev == 0) {
                    ALOGI("Warning:device 0 is omitted,the device is not active");
                    continue;
                }
                                   ...
                                   AudioSystem::audio_in_acoustics acoustics)
{
    ...
    for(ALSAHandleList::iterator it = mDeviceList.begin();it != mDeviceList.end(); ++it)
        if (it->devices & devices) {
            ...
            ALOGV("the return samplerate is %u ",it->sampleRate);
