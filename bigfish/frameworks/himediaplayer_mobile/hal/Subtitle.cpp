...
int SubtitleFontManager::RenderGfx()
{
    ...
    while ( p != NULL)
    {
        ...
        resolutions=atoi(resolution);
        if(resolutions == 1080)
        {
            //if UI is 1080, dont adjust
            LOGV("resolutions=%d, (wont reduce)",resolutions);
        }
        else
        {
            //in 3716C , UI is 720p,but if window size is 1920x1080,adjust
            if (mWinwidth == 1920)
            {
                float ratioAdjust =1280.0/1920.0;
                ratio *= ratioAdjust;
                y *= ratioAdjust;
                x *= ratioAdjust;
