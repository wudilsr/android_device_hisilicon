...
TEST_F(HiVSinkTest, SeamLessPlay) {
    HI_S32 ret;
    HI_U32 i, j;
    HI_U32 cnt = 6;
    ...
    ret = HI_SVR_VSINK_CheckFence(pstVsink, aPics + i);
    EXPECT_EQ(HI_SUCCESS, ret);
    ...
    sink->setNativeWindow(0);
    (void)native_window_api_disconnect(mSurface.get(), NATIVE_WINDOW_API_MEDIA);
...
TEST_F(HiVSinkTest, CancelBuffer) {
    HI_S32 ret;
    HI_U32 i;
    HI_U32 cnt = 6;
    int pic_size = mWidth * mHeight * 3 / 2;
    ...
    //cancel remain buffer
    ret = HI_SVR_VSINK_Cancel(pstVsink, aPics + i, cnt - cnt / 2);
    EXPECT_EQ(HI_SUCCESS, ret);
