#define LOG_NDEBUG 0
#define LOG_TAG "HisiMediaSystem"
#include <utils/Log.h>
...
static int HiMediaSystem_VO_Init(HI_UNF_VO_DEV_MODE_E enDevMode)
{
    ...
    {
        LOGE("call HI_UNF_DISP_Init failed.");
    }
...
namespace android
{
    ...
    int HiMediaSystem::initPlayer()
    {
        ...
        {
            LOGE("ERR: HiMediaSystem::mInstPlayerCount >= MAX_INSTANCE_NUM");
            //return UNKNOWN_ERROR;
        }
        ...
        if (0 == HiMediaSystem::mPlayerInstCount)
        {
            ...
            if (property_get("drm.client.enabled", value, NULL) > 0)
            {
                if (!strncmp(value, "true", 4))
                {
                    drm_enable = 1;
                    LOGV("Drm.client.enable");
                }
            }

            if (1 == drm_enable)
            {
                /* if tvp path, we should play asf file by using extractor_demux */
                s32Ret = HI_SVR_PLAYER_RegisterDynamic(HI_SVR_PLAYER_DLL_PARSER, "libextrator_demux.so");
                if (HI_SUCCESS != s32Ret)
                {
                    LOGE("ERR: HI_SVR_PLAYER_RegisterDynamic libextrator_demux.so fail");
                }
            }
