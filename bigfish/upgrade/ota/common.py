...
def LoadRecoveryFSTab(zip, fstab_version):
  ...
  try:
      data = zip.read("META/recovery.fstab")
  except KeyError:
    print "Warning: could not find META/recovery.fstab in %s." % zip
    try:
        recovery_path = os.path.join(OPTIONS.search_path,"recovery.fstab")
        data = zip.read(recovery_path)
    except KeyError:
      data = ""
...
def GetRawImage(name, prebuilt_name, unpack_dir,
                     info_dict=None):
  """Return a File object (with name 'name') with the desired bootable
  image.  Look for it in 'unpack_dir' under the name
  'prebuilt_name'"""
  ...
  if os.path.exists(prebuilt_path):
    ...
    print "waring there is no %s in %s "% (prebuilt_name,unpack_dir)
    return None

def GetRawImageDifferent(name, prebuilt_name, unpack_source_dir,unpack_target_dir,
                     info_dict=None):
  """Return a File object (with name 'name') with the desired bootable
  image.  Look for it in 'unpack_dir' under the name
  'prebuilt_name'"""
  source_image = GetRawImage(name,prebuilt_name,unpack_source_dir)
  target_image = GetRawImage(name,prebuilt_name,unpack_target_dir)
  if source_image is not None and target_image is not None:
    name_different = (source_image.data != target_image.data)
  elif source_image is None and target_image is not None:
    name_different  = True
  else:
    name_different  = None

  return name_different,target_image
...
def GetBootargs(bootargs_file,emmc_begin,nand_begin):
    #bootargs_file_read = open(bootargs_file,"r")
    #for line in bootargs_file_read.readlines():
    bootargs_file_read = bootargs_file
    for line in bootargs_file_read.split("\n"):
        line =  line.strip().replace(' ','')
        if line.startswith('recoverybootargs'):
            if line.find(nand_begin) != -1:
                n = line.find(nand_begin)
                m = line.find("ubi.mtd")
                flash_type = "Nand"
                return flash_type ,line[n+len(nand_begin):m]
            if line.find(emmc_begin) != -1:
                n = line.find(emmc_begin)
                flash_type = "eMMC"
                return flash_type ,line[n+len(emmc_begin):]
...
def LoadBootargs(zip):
  try:
    data = zip.read("META/bootargs.txt")
  except KeyError:
    print "Warning: could not find META/bootargs.txt in %s" % zip
    data = None
  if data is None:
      return None, None

  flash_type,bootargs = GetBootargs(data,"mmcblk0:","hinand:");
  return flash_type,bootargs


def CreateUpdaterPcg(bootargs,flash_type):
    if flash_type == "eMMC":
        pcg_prebuilt = 'partchange("EMMC","/dev/block/mmcblk0:'
        pcg_misc = 'setmisc("EMMC","/dev/block/platform/hi_mci.1/by-name/misc");\n'
        pcg_cache = 'ui_print("format cache......");\nformat("ext4", "EMMC", "/dev/block/platform/hi_mci.1/by-name/cache", "0", "/cache");\n'
        pcg_sdcard = 'ui_print("format sdcard......");\nformat("ext4", "EMMC", "/dev/block/platform/hi_mci.1/by-name/sdcard", "0", "/sdcard");\n'

    if flash_type == "Nand":
        pcg_prebuilt = 'partchange("MTD","hinand:'
        pcg_misc = 'setmisc("MTD","misc");\n'
        pcg_cache = 'ui_print("format cache......");\nformat("ubifs", "MTD", "cache", "0", "/cache");\n'
        pcg_sdcard = 'ui_print("format sdcard......");\nformat("ubifs", "MTD", "sdcard", "0", "/sdcard");\n'

    pcg_end = '");\n'
    pcg_umount = 'ifelse(is_mounted("/system"),unmount("/system"));\nifelse(is_mounted("/cache"),unmount("/cache"));\nifelse(is_mounted("/data"),unmount("/data"));\n'
    updater_pcg = pcg_umount + pcg_prebuilt + bootargs + pcg_end + pcg_misc + pcg_cache + pcg_sdcard
    return updater_pcg
