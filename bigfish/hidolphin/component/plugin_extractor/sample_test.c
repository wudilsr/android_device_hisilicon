...
HI_S32 main(HI_S32 argc, HI_CHAR **argv)
{
    ...
    s32Ret = SVR_EXTRACTOR_SetDataSource(extractor, argv[1], NULL);
    ...
    while (1)
    {
        ...
        if (HI_SUCCESS == s32Ret)
        {
            if (NULL != stFrame.pu8FrameHeader)
                fwrite(stFrame.pu8FrameHeader, 1, stFrame.u32FrameHeaderLen, pfile);

            fwrite(stFrame.pu8Addr, 1, stFrame.u32Len, pfile);
        }
        else
        {
            break;
