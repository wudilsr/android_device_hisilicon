...
double entropyTime = 0.0;
/*double totalDecompressTime = 0.0;*/
double totalDecompressTime = 0.0;
double invTQTime = 0.0;
double intraPredTime = 0.0;
double mcTime = 0.0;
double deblockTime = 0.0;
double getTime()
{
...
#ifdef PROFILING
double profiling_start = getTime();
#endif
...
#ifdef PROFILING
    mcTime += (getTime()-profiling_start);
#endif
}
...
#ifdef PROFILING
double profiling_start,profiling_start1,profiling_total;
profiling_total = getTime();
#endif
...
#ifdef PROFILING
intraPredTime += (getTime()-profiling_total);
#endif
...
#ifdef PROFILING
invTQTime += (getTime()-profiling_start);
#endif
...
static void field_end(H264Context *h, int in_setup){
     ...
     */
    if( (!FIELD_PICTURE)&&(s->isFMOType2 !=1))
        ff_er_frame_end(s);
...
static int decode_slice(struct AVCodecContext *avctx, void *arg){
    ...
    MpegEncContext * const s = &h->s;
    int slicemark,curMB;//add dpy FMO
    const int part_mask= s->partitioned_frame ? (AC_END|AC_ERROR) : 0x7F;
 #ifdef PROFILING
double profiling_start=0;
 #endif
    s->mb_skip_run= -1;
...
#ifdef PROFILING
             entropyTime += (getTime()-profiling_start) ;
#endif
            int eos;
//STOP_TIMER("decode_mb_cabac")
...
#ifdef PROFILING
             totalDecompressTime += (getTime()-profiling_start) ;
#endif
            ...
            profiling_start = getTime();
                ...
                loop_filter(h);
#ifdef PROFILING
   deblockTime += (getTime()-profiling_start);
#endif
                decode_finish_row(h);
                ++s->mb_y;
                if(FIELD_OR_MBAFF_PICTURE) {
   ...
   totalDecompressTime += (getTime()-profiling_start);
        ...
        if(s->isFMOType2 ==1)
            {
            slicemark=s->MapUnitToSliceGroupMap[s->mb_y*s->mb_width+s->mb_x];
            curMB= s->mb_y*s->mb_width+s->mb_x;
            while((++curMB)<(s->mb_width* s->mb_height-1)&&(s->MapUnitToSliceGroupMap[curMB]!=slicemark))
                ;
            s->mb_x = curMB%s->mb_width;
            s->mb_y = curMB/s->mb_width;
            }
        else
            {
            if(++s->mb_x >= s->mb_width){
...
#ifdef PROFILING
   totalDecTime += (getTime()-profiling_start);
#endif
