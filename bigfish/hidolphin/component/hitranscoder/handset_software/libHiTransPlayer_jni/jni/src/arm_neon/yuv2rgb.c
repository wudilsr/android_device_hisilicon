...
#ifdef HAVE_NEON
    msg_Dbg(VLC_OBJECT(p_filter), "HAVE_NEON width %d height %d YStride %d UVStride %d dstStride %d",
    width, height, p_pic->Y_PITCH,         // y stride
...
#else
    msg_Dbg(VLC_OBJECT(p_filter), "NO_NEON width %d height %d YStride %d UVStride %d dstStride %d",
    width, height, p_pic->Y_PITCH,         // y stride
        p_pic->U_PITCH,         // uv stride
        p_dst->p[0].i_pitch);
    yuv420_2_rgb565(
