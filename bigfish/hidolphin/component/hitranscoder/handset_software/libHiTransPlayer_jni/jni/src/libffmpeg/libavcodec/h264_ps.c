...
int ff_h264_decode_picture_parameter_set(H264Context *h, int bit_length){
    ...
    PPS *pps;
     int i ,yTopLeft,xTopLeft,yBottomRight,xBottomRight,x,y,mb_width,mb_height;
...
//        av_log(h->s.avctx, AV_LOG_ERROR, "FMO supported\n");
...
#if 1//add FMO dpy
s->isFMOType2 = 1;
mb_width = h->sps_buffers[pps->sps_id]->mb_width;
mb_height = h->sps_buffers[pps->sps_id]->mb_height;

for (i=0; i<pps->slice_group_count-1; i++)
      {
        //! JVT-F078: avoid reference of SPS by using ue(v) instead of u(v)
        pps->top_left [i]  = get_ue_golomb (&s->gb);
        pps->bottom_right [i] = get_ue_golomb (&s->gb);
    //av_log(h->s.avctx, AV_LOG_ERROR, "top_left [%d] %d,pps->bottom_right [%d] %d\n",\
    //    i,pps->top_left [i],i,pps->bottom_right [i]);
      }
    s->NumberOfSliceGroups = pps->slice_group_count-1;
    if(s->MapUnitToSliceGroupMap == NULL)
        {
        s->MapUnitToSliceGroupMap = (int*)av_malloc(mb_width*mb_height*sizeof(int));
        if(s->MapUnitToSliceGroupMap != NULL)
            {
                memset(s->MapUnitToSliceGroupMap,s->NumberOfSliceGroups,mb_width*mb_height*sizeof(int));
            }
        }
    if(s->MapUnitToSliceGroupMap == NULL)
        {
        av_log(h->s.avctx, AV_LOG_ERROR, "FMO:s->MapUnitToSliceGroupMap == NULL\n");
         return -1;
        }
    for(i=0;i<mb_width*mb_height;i++ )
        {
        s->MapUnitToSliceGroupMap[i ] = s->NumberOfSliceGroups;
        }
    for( i = s->NumberOfSliceGroups - 1 ; i >= 0; i-- )
        {
        yTopLeft = pps->top_left[ i ] / mb_width;
         xTopLeft = pps->top_left[ i ] %mb_width;
        yBottomRight = pps->bottom_right[ i ] / mb_width;
        xBottomRight = pps->bottom_right[ i ] % mb_width;
        for( y = yTopLeft; y <= yBottomRight; y++ )
            for( x = xTopLeft; x <= xBottomRight; x++ )
                    s->MapUnitToSliceGroupMap[ y *mb_width + x ] = i;
        }
    //av_log(h->s.avctx, AV_LOG_ERROR, "FMO:init MapUnitToSliceGroupMap OK\n");
/*
|   for( i = 0; i < num_slice_groups_minus1; i++ )  |   |        |
|{                                                  |   |        |
