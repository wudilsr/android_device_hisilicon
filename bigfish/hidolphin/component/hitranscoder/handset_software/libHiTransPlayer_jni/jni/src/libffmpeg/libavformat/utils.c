...
#include <assert.h>
extern void* g_pAHS;
#define OMITINVALIDDTS 1
#define FASTFINDSTREAMINFO 0
/**
 * @file
 * various utility functions for use within Libav
 */
                       ...
                       AVFormatParameters *ap)
{
    ...
    pd->buf = NULL;
    pd->buf_size = 0;
    av_log(NULL, AV_LOG_DEBUG, "av_open_input_file 1\n");
    if (!fmt) {
        /* guess format if no file can be opened */
        fmt = av_probe_input_format(pd, 0);
    }
    av_log(NULL, AV_LOG_DEBUG, "av_open_input_file 2\n");
    ...
    if (!fmt || !(fmt->flags & AVFMT_NOFILE)) {
     ...
     av_log(NULL, AV_LOG_DEBUG, "av_open_input_file 3\n");
/*
     if(g_pAHS)
     {
         char pType[128];
         if(AHS_GetMediaType(g_pAHS, pType) == 0)
         {
             if(strstr(pType,"mpeg2TS")||strstr(pType,"mpeg2ts"))
            {
                fmt = av_find_input_format("mpegts");
            }
            else if(strstr(pType,"3gpp"))
            {
                fmt = av_find_input_format("3gp");
            }
         }
    ...
    av_log(NULL, AV_LOG_DEBUG, "av_open_input_file 4\n");
...
/*            if(!st->time_base.num)
        ...
        av_log(ic, AV_LOG_DEBUG, "i: %d codecid : %d \n", i, st->codec->codec_id);
        codec = avcodec_find_decoder(st->codec->codec_id);
...
#if FASTFINDSTREAMINFO
        /*else if(g_pAHS)
        {//add by x54178
            AHSStat strStat;
        memset(&strStat,0,sizeof(AHSStat));
        AHS_GetStatus(g_pAHS, &strStat);
            if(strstr(strStat.pMediaType,"mpeg2TS")||strstr(strStat.pMediaType,"mpeg2ts"))
           {
               break;
            }
        }*/
...
/*    if(g_pAHS)
    {//get duration from ahsmodule
        int64_t dur = AHS_GetDuration(g_pAHS);
        if(dur)
        {
            ic->duration = AV_TIME_BASE/1000*dur;
        }
        av_log(ic, AV_LOG_WARNING, "AHS_GetDuration:%lld,%lld\n",ic->duration,dur);
    }
    */
