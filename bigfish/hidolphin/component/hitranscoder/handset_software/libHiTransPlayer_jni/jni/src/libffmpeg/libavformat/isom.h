...
typedef struct MOVStreamContext {
    ...
    int has_palette;
    //x54178 用于变阵率，scale变化表明镇吕变化
    //本次scale的duration
    int64_t msc_duration;
    //本次scale变化时的起始st->duration
    int64_t st_duration_start;
} MOVStreamContext;
//add by x54178 "styp"
typedef struct MOVSegment
{
    uint64_t segment_offset;//offset of the first byte of the segment
} MOVSegment;
...
typedef struct MOVContext {
    ...
    MOVFragment fragment; ///< current fragment in moof atom
        //add by x54178
    MOVSegment segment;
    unsigned int uCurStreamSN;
    int nTrackNum;
    int found_moof;
    int bNeedUpdateExtraData;
