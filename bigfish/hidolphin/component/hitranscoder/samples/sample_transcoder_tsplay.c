...
HI_VOID main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    {
        printf("Usage: sample_transcoder_tsplay file=test.ts [w=1280] [h=720] [f=25] [b=4*1024*1024] [pro=http]\n"
        ...
        printf("Example:./sample_transcoder_tsplay  file=test.ts  f=25 w=1280 h=720 pro=rtsp b=4*1024*1024\n");
        return ;
    }
    ...
    for(;;)
    {
        ...
        switch(InputCmd[0])
        {
            ...
            case 'r':
                ...
                {
                    s32Ret = HI_Transcoder_GetAttr((HI_HANDLE)g_hTranHandle,TRANSCODER_ALL_ATTR, &stTranHandleAttr);
                    printf("hejinghai...gop..%d\n,",stTranHandleAttr.s32Gop);
                    if(HI_SUCCESS != s32Ret)
                    {
                        printf("HI_Transcoder_GetAttr failed\n");
                        continue;
    ...
    if(HI_SUCCESS != s32Ret)
    {
        printf("HI_Protocol_UnRegistHandle failed\n");
    }
...
PROTOCOL_DESTROY:
    ...
    if(HI_SUCCESS != s32Ret)
    {
        printf("HI_Protocol_destory failed\n");
    }
...
PROTOCOL_DEINIT:
    ...
    if(HI_SUCCESS != s32Ret)
    {
        printf("HI_Protocol_DeInit failed\n");
    }
...
TRANS_DESTROY:
    ...
    if(HI_SUCCESS != s32Ret)
    {
        printf("HI_Transcoder_Destroy failed\n");
    }
...
TRANS_DEINIT:
    ...
    if(HI_SUCCESS != s32Ret)
    {
        printf("HI_Transcoder_DeInit failed\n");
    }
