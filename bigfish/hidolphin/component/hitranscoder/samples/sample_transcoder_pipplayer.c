...
HI_S32 closePipPlayer(HI_HANDLE hpipSrcHandle, HI_HANDLE hPipWinHandle)
{
    HI_S32 Ret = HI_SUCCESS;
    HI_UNF_AVPLAY_STOP_OPT_S Stop;
    HI_HANDLE hAvplay = hpipSrcHandle;
    ...
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_AVPLAY_Stop failed.\n");
    }

    Ret = HI_UNF_VO_SetWindowEnable(hPipWinHandle,HI_FALSE);
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_VO_SetWindowEnable failed.\n");
    }
    Ret = HI_UNF_VO_DetachWindow(hPipWinHandle, hAvplay);
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_VO_DetachWindow failed.\n");
    }

    Ret = HI_UNF_SND_Detach(pipTrackHandle, hAvplay);
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_SND_Detach failed.\n");
    }
    if(pipTrackHandle)
    {
        Ret = HI_UNF_SND_DestroyTrack(pipTrackHandle);
        if (Ret != HI_SUCCESS)
        {
            printf("PIP call HI_UNF_SND_DestroyTrack failed.\n");
        }
    }
    Ret = HI_UNF_AVPLAY_ChnClose(hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_VID | HI_UNF_AVPLAY_MEDIA_CHAN_AUD);
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_AVPLAY_ChnClose failed.\n");
    }

    Ret = HI_UNF_VO_DestroyWindow(hPipWinHandle);
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_VO_DetachWindow failed.\n");
    }
    ...
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_AVPLAY_Destroy failed.\n");
    }
    Ret = HI_UNF_DMX_DetachTSPort(HI_DMX_ID_4_PLAY);
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_DMX_DetachTSPort failed.\n");
    }
    return Ret;
}

HI_S32 openPipPlayer(HI_HANDLE* phpipSrcHandle, HI_HANDLE* phPipWinHandle)
{
    HI_S32                  Ret = HI_SUCCESS;
    HI_HANDLE hWin = 0;
    HI_HANDLE hAvplay = 0;
    ...
    HI_RECT_S stOutputRect;
    stOutputRect.s32X = 0;
    stOutputRect.s32Y = 0;
    stOutputRect.s32Width = 400;
    stOutputRect.s32Height = 300;
    Ret |= HIADP_VO_CreatWin(&stOutputRect,&hWin);
    if (HI_SUCCESS != Ret)
    {
        printf("PIP call HIADP_VO_CreatWin failed.Ret: %d\n", Ret);
        goto failed;
    }
#if 0
    memset(&WinAttr, 0x00, sizeof(HI_UNF_WINDOW_ATTR_S));
    ...
    memset(&WinAttr.stInputRect,0,sizeof(HI_RECT_S));
    memset(&WinAttr.stOutputRect,0,sizeof(HI_RECT_S));
    WinAttr.stOutputRect.s32Width = 400;
    WinAttr.stOutputRect.s32Height = 300;
    Ret = HI_UNF_VO_CreateWindow(&WinAttr,&hWin);
    if (Ret != HI_SUCCESS)
    {
        printf("call HI_UNF_VO_CreateWindow failed. ret : %x\n",Ret);
        goto failed;
    }
#endif
    Ret = HI_UNF_DMX_AttachTSPort(HI_DMX_ID_4_PLAY,  HI_UNF_DMX_PORT_IF_0);
    if (HI_SUCCESS != Ret)
    {
    ...
    AvplayAttr.u32DemuxId = HI_DMX_ID_4_PLAY;
    Ret |= HI_UNF_AVPLAY_Create(&AvplayAttr, &hAvplay);
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_AVPLAY_Create failed.Ret: %d\n", Ret);
        goto DMX_DETACH;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        printf("PIP call HI_UNF_AVPLAY_ChnOpen failed.Ret: %d\n", Ret);
        goto CHN_CLOSE;
    }
    Ret = HI_UNF_VO_AttachWindow(hWin, hAvplay);
    if (HI_SUCCESS != Ret)
    {
        printf("PIP call HI_UNF_VO_AttachWindow failed.Ret: %d\n", Ret);
        goto CHN_CLOSE;
    }
    Ret = HI_UNF_VO_SetWindowEnable(hWin, HI_TRUE);
    if (HI_SUCCESS != Ret)
    {
        printf("PIP call HI_UNF_VO_SetWindowEnable failed.Ret: %d\n", Ret);
        goto WIN_DETACH;
    }
    ...
    stGain.bLinearMode = 0;
    stGain.s32Gain = 60;
    Ret = HI_UNF_SND_SetTrackWeight(hTrack, &stGain);
    ...
    if (Ret != HI_SUCCESS)
    {
        printf("PIP call HI_UNF_AVPLAY_SetAttr failed.Ret: %d\n", Ret);
        goto SND_DETACH;
    }
    *phpipSrcHandle = hAvplay;
    *phPipWinHandle = hWin;
    pipTrackHandle = hTrack;
    return HI_SUCCESS;
...
DMX_DETACH:
    HI_UNF_DMX_DetachTSPort(HI_DMX_ID_4_PLAY);
...
HI_S32 main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    {
        printf("Usage: sample_transcoder_PipPlayer freq=307 [srate=6875] [qam=64] [w=1280] [h=720] [f=25] [b=4*1024*1024] [pro=http]\n"
        ...
        printf("Example:./sample_transcoder_PipPlayer  freq=610 srate=6875 qam=64 f=25 w=1280 h=720 pro=rtsp b=4*1024*1024\n");
        return HI_FAILURE;
    }
    ...
    Ret = openPipPlayer(&hPipHandle, &hPipWinHandle);
    if (Ret != HI_SUCCESS)
    {
        printf("call HI_UNF_AVPLAY_SetAttr failed.\n");
        goto AVPLAY_STOP;
    ...
    stTranHandleAttr.hSrc = hPipHandle;
    ...
    if (Ret != HI_SUCCESS)
    {
        printf("call SwitchProg hAvplay  failed.\n");
        goto PIP_CLOSE;
    }
    ProgNum = 0;
    Ret = HIADP_AVPlay_PlayProg(hPipHandle,g_pProgTbl,ProgNum,HI_TRUE);
    if (Ret != HI_SUCCESS)
    {
        printf("call SwitchProg hPipHandle failed.\n");
        goto PIP_CLOSE;
    }
    ...
    while(1)
    {
        ...
        Ret = HIADP_AVPlay_PlayProg(hPipHandle,g_pProgTbl,ProgNum-1,HI_TRUE);
        if (Ret != HI_SUCCESS)
        {
...
PIP_CLOSE:
    closePipPlayer(hPipHandle, hPipWinHandle);
