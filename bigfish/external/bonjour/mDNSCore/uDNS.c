...
#define ZoneDataSRV(X) (\
	(X)->mZoneService == ZoneServiceUpdate ? ((X)->ZonePrivate ? PRIVATE_UPDATE_SERVICE_TYPE : PUBLIC_UPDATE_SERVICE_TYPE) : \
	(X)->mZoneService == ZoneServiceQuery  ? ((X)->ZonePrivate ? PRIVATE_QUERY_SERVICE_TYPE  : (const domainname*)""     ) : \
	(X)->mZoneService == ZoneServiceLLQ    ? ((X)->ZonePrivate ? PRIVATE_LLQ_SERVICE_TYPE    : PUBLIC_LLQ_SERVICE_TYPE   ) : (const domainname*)"")
	...
	if (answer->rrtype == kDNSType_SOA)
		...
		if (answer->rdlength)
			...
			if (AuthInfo && AuthInfo->AutoTunnel)
				...
				zd->mZoneDataCallback(m, mStatus_NoSuchNameErr, zd);
				}
			else 
				{
...
#if 0
		if (!answer->rdlength && zd->ZonePrivate && zd->mZoneService != ZoneServiceQuery)
			{
			...
			if (answer->rdlength)
				...
				zd->Port = zeroIPPort;
				zd->Addr = zeroAddr;
				zd->mZoneDataCallback(m, mStatus_NoError, zd);
				}
			}
		}
