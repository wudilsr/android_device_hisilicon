...
extern IpFilterFun gIpFilterFun;
...
mDNSlocal void SendResponses(mDNS *const m)
	...
	while (intf)
		...
		for (rr = m->ResourceRecords; rr; rr=rr->next)
			{
                if((rr->resrec.rrtype==kDNSType_A) && (gIpFilterFun.blackIpFun!= NULL)) {
                    const RDataBody2 *const rdb = (RDataBody2 *)rr->resrec.rdata->u.data;
                    if(gIpFilterFun.blackIpFun(&rdb->ipv4) == mDNStrue)
                    {
                        continue;
                    }
                }
...
mDNSlocal void mDNSCoreReceiveQuery(mDNS *const m, const DNSMessage *const msg, const mDNSu8 *const end,
    ...
    if (gSnifferFun.queryFun!= NULL)
    {
        gSnifferFun.queryFun(msg, end, srcaddr, dstaddr, InterfaceID);
    }
    if (msg->h.numAnswers > 0 && gSnifferFun.responseFun != NULL)
    {
        gSnifferFun.responseFun(m, msg, end, srcaddr, dstaddr, InterfaceID);
    }
...
mDNSlocal void mDNSCoreReceiveResponse(mDNS *const m,
    ...
    if (gSnifferFun.responseFun != NULL)
    {
        gSnifferFun.responseFun(m, response, end, srcaddr, dstaddr, InterfaceID);
    }
