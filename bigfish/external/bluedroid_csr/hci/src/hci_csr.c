 ...
 * hci_h4.c (Copyright (C) 2009-2012) by Broadcom Corporation
 ***************************************************************************/
...
/* HCI message type definitions */
#define USB_TYPE_COMMAND         1
#define USB_TYPE_ACL_DATA        2
#define USB_TYPE_SCO_DATA        3
#define USB_TYPE_EVENT               4

static const uint16_t msg_evt_table[] =
{
    MSG_HC_TO_STACK_HCI_ERR,       /* TYPE_COMMAND  */
    MSG_HC_TO_STACK_HCI_ACL,       /* TYPE_ACL_DATA  */
    MSG_HC_TO_STACK_HCI_SCO,      /* TYPE_SCO_DATA  */
    MSG_HC_TO_STACK_HCI_EVT        /* TYPE_EVENT         */
};
...
typedef struct
{
    ...
    tHCI_USB_RCV_STATE rcv_state;    /* Receive state of current rx message */
...
uint8_t hci_usb_send_int_cmd(uint16_t opcode, HC_BT_HDR *p_buf, \
...
void get_acl_data_length_cback(void *p_mem)
{
    ...
    if (opcode == HCI_READ_BUFFER_SIZE)
    {
        ...
        *p = 0;
        ...
        if ((status = hci_usb_send_int_cmd(HCI_LE_READ_BUFFER_SIZE, p_buf, \
    ...
    {
        if (status == 0)
            usb_cb.hc_ble_acl_data_size = (len) ? len : usb_cb.hc_acl_data_size;
...
**                    L2CAP message.
**
** Returns          the address of the receive buffer USB RX should use
...
static HC_BT_HDR *acl_rx_frame_buffer_alloc (void)
{
    ...
    if (pkt_type == ACL_RX_PKT_START)       /*** START PACKET ***/
    {
        ...
        if (p_return_buf)
        {
            ALOGW("USB - dropping incomplete ACL frame");
...
static uint8_t acl_rx_frame_end_chk (void)
{
    ...
    if (hci_len > 0)
    {
        ...
        {
            /* If the L2CAP length has not been reached, tell USB not to send
...
/*******************************************************************************
**
** Function        hci_usb_init
**
** Description     Initialize usb module
**
** Returns         None
**
*******************************************************************************/
void hci_usb_init(void)
{
    ...
    memset(&usb_cb, 0, sizeof(tHCI_USB_CB));
    utils_queue_init(&(usb_cb.acl_rx_q));
     ...
     */
    usb_cb.hc_acl_data_size = 1021;
    usb_cb.hc_ble_acl_data_size = 27;
...
/*******************************************************************************
**
** Function        hci_usb_cleanup
**
** Description     Clean usb module
**
** Returns         None
**
*******************************************************************************/
...
/*******************************************************************************
**
** Function        hci_usb_send_msg
**
** Description    Determine message type, set HCI packet indicator, and
**                     send message through USB driver
**
** Returns         None
**
*******************************************************************************/
void hci_usb_send_msg(HC_BT_HDR *p_msg)
{
    uint8_t type = 0;
    uint16_t handle;
    ...
    if (event == MSG_STACK_TO_HC_HCI_ACL)
        type = USB_TYPE_ACL_DATA;
    else if (event == MSG_STACK_TO_HC_HCI_SCO)
        type = USB_TYPE_SCO_DATA;
    else if (event == MSG_STACK_TO_HC_HCI_CMD)
        type = USB_TYPE_COMMAND;

    if (sub_event == LOCAL_BR_EDR_CONTROLLER_ID)
    {
        acl_data_size = usb_cb.hc_acl_data_size;
        acl_pkt_size = usb_cb.hc_acl_data_size + HCI_ACL_PREAMBLE_SIZE;
    }
    else
    {
        acl_data_size = usb_cb.hc_ble_acl_data_size;
        acl_pkt_size =  usb_cb.hc_ble_acl_data_size + HCI_ACL_PREAMBLE_SIZE;
    }
    ...
    if ((event == MSG_STACK_TO_HC_HCI_ACL) && (p_msg->len > acl_pkt_size))
    {
        ...
        while (p_msg->len > acl_pkt_size)
        {
            ...
            //sachin - no need to append type for USB
            *p = type;
...
**
*******************************************************************************/
uint16_t hci_usb_receive_msg(void)
{
    ...
    while (TRUE)
    {
        ...
        {
        case USB_RX_MSGTYPE_ST:
            /* Start of new message */
            if ((byte < USB_TYPE_ACL_DATA) || (byte > USB_TYPE_EVENT))
            {
                /* Unknown HCI message type */
                /* Drop this byte */
                ALOGE("[usb] Unknown HCI message type drop this byte 0x%x", byte);
                break;
            }
            ...
            // p_cb->p_rcv_msg = NULL;
            p_cb->rcv_state = USB_RX_LEN_ST; /* Next, wait for length to come */
            break;
        ...
        case USB_RX_LEN_ST:
            ...
            if (p_cb->rcv_len == 0)
            {
                if (p_cb->rcv_msg_type == USB_TYPE_ACL_DATA)
                {
                ...
                if (p_cb->p_rcv_msg == NULL)
                {
                    ...
                    ALOGE( \
                     "USB: Unable to acquire buffer for incoming HCI message." \
                    );

                    if (msg_len == 0)
                    {
                        /* Wait for next message */
                        p_cb->rcv_state = USB_RX_MSGTYPE_ST;
                    }
                    else
                    {
                        /* Ignore rest of the packet */
                        p_cb->rcv_state = USB_RX_IGNORE_ST;
                    }
...
uint8_t hci_usb_send_int_cmd(uint16_t opcode, HC_BT_HDR *p_buf, \
                                                       tINT_CMD_CBACK p_cback)
{
     if (usb_cb.int_cmd_rsp_pending > INT_CMD_PKT_MAX_COUNT)
    {
    ...
    usb_cb.int_cmd_rsp_pending++;
    usb_cb.int_cmd[usb_cb.int_cmd_wrt_idx].opcode = opcode;
    usb_cb.int_cmd[usb_cb.int_cmd_wrt_idx].cback = p_cback;
    usb_cb.int_cmd_wrt_idx = ((usb_cb.int_cmd_wrt_idx+1) & INT_CMD_PKT_IDX_MASK);
...
/*******************************************************************************
**
** Function        hci_usb_get_acl_data_length
**
...
**
*******************************************************************************/
void hci_usb_get_acl_data_length(void)
{
    ...
    if (p_buf)
    {
        ...
        *p = 0;
        ...
        if ((ret = hci_usb_send_int_cmd(HCI_READ_BUFFER_SIZE,
		                                              p_buf,
                                                            get_acl_data_length_cback)) == FALSE)
        {
            bt_hc_cbacks->dealloc((TRANSAC) p_buf, (char *) (p_buf + 1));
...
/******************************************************************************
**  HCI USB Services interface table
******************************************************************************/
const tHCI_IF hci_usb_func_table =
{
    hci_usb_init,
    hci_usb_cleanup,
    hci_usb_send_msg,
    hci_usb_send_int_cmd,
    hci_usb_get_acl_data_length,
    hci_usb_receive_msg
};
