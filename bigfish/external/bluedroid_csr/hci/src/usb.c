...
static libusb_device_handle *libusb_open_bt_device()
{
    ...
    USBDBG("get_device_list\n");
    if (libusb_get_device_list(NULL, &devs) < 0)
    {
        return NULL;
    }

    USBDBG("get_device_list after\n");
    for (i = 0; (dev = devs[i]) != NULL; i++)
    {
   
        USBDBG("CHECK IS BTUSBDEV i= %d\n", i ); 
...
#if 0
            usleep(1 * 1000);  //delay 1ms to prevent USB Ctrl Tx error. ysy@DUSUN
#endif 
