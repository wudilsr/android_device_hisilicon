...
// CSR porting
...
#else
    BTM_TRACE_DEBUG1 ("BTM Event: Vendor Specific event from controller: len%d",evt_len);

    /* There would be no command complete from bluecore.
     * So clear the VS command complete timer as well and call the command complete callback.
     */
    BT_HDR      *p_cmd;
    ...
    while (p_cmd) {
        ...
        if (opcode_dequeued != HCI_GRP_VENDOR_SPECIFIC) {
            p_cmd = (BT_HDR *) GKI_getnext(p_cmd);
            continue;
        }
        BTM_TRACE_DEBUG0("VS: found our command");
