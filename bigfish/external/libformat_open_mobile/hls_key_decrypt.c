        ...
        const uint8_t *key, int keylen, const uint8_t *iv, int ivlen)
{
    ...
    if (!strncmp(h->file_name, "hicache", 7) && url) {
        av_log(NULL, AV_LOG_INFO, "use hicache to open crypted:%s", url);
        AVDictionary *options = NULL;
        char tmp[32];
        snprintf(tmp, sizeof(tmp), "%d", h->cur_segment);
        av_dict_set(&options, "segment", tmp, 0);
        av_dict_set(&options, "url", strstr(url, "http"), 0);
        ret = hls_open_h(&h->hd, h->file_name, URL_RDONLY, &(h->interrupt_callback), &options);
