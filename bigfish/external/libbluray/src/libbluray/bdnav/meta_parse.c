...
const META_DL *meta_get(const META_ROOT *meta_root, const char *language_code)
{
    ...
    if (language_code) {
        for (i = 0; i < meta_root->dl_count; i++) {
            if (strncmp(language_code, meta_root->dl_entries[i].language_code,
                    strlen(language_code) > strlen(meta_root->dl_entries[i].language_code) ?
                    strlen(language_code) : strlen(meta_root->dl_entries[i].language_code)) == 0) {
    ...
    for (i = 0; i < meta_root->dl_count; i++) {
        if (strncmp(DEFAULT_LANGUAGE, meta_root->dl_entries[i].language_code,
                strlen(DEFAULT_LANGUAGE) > strlen(meta_root->dl_entries[i].language_code) ?
                strlen(DEFAULT_LANGUAGE) : strlen(meta_root->dl_entries[i].language_code)) == 0) {
