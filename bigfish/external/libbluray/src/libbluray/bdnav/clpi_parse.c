...
clpi_parse(const char *path)
{
    ...
    if (!cl) {
        ...
        backup = (char *)malloc(len + 8);
        if (NULL == backup) {
            return NULL;
        }

        memset(backup, 0, len + 8);
        strncpy(backup, path, len - 18);
        strncpy(backup + len - 18, "BACKUP/", 7);
        strncpy(backup + len - 18 + 7, path + len - 18, 18);
...
clpi_copy(const CLPI_CL* src_cl)
{
    ...
    if (src_cl) {
        ...
        dest_cl->cpi_ss.num_stream_pid = src_cl->cpi_ss.num_stream_pid;
        dest_cl->cpi_ss.entry = malloc(src_cl->cpi_ss.num_stream_pid * sizeof(CLPI_EP_MAP_ENTRY));
        for (ii = 0; ii < dest_cl->cpi_ss.num_stream_pid; ii++) {
            dest_cl->cpi_ss.entry[ii].pid = src_cl->cpi_ss.entry[ii].pid;
            dest_cl->cpi_ss.entry[ii].ep_stream_type = src_cl->cpi_ss.entry[ii].ep_stream_type;
            dest_cl->cpi_ss.entry[ii].num_ep_coarse = src_cl->cpi_ss.entry[ii].num_ep_coarse;
            dest_cl->cpi_ss.entry[ii].num_ep_fine = src_cl->cpi_ss.entry[ii].num_ep_fine;
            dest_cl->cpi_ss.entry[ii].ep_map_stream_start_addr = src_cl->cpi_ss.entry[ii].ep_map_stream_start_addr;
            dest_cl->cpi_ss.entry[ii].coarse = malloc(src_cl->cpi_ss.entry[ii].num_ep_coarse * sizeof(CLPI_EP_COARSE));
            for (jj = 0; jj < src_cl->cpi_ss.entry[ii].num_ep_coarse; jj++) {
                dest_cl->cpi_ss.entry[ii].coarse[jj].ref_ep_fine_id = src_cl->cpi_ss.entry[ii].coarse[jj].ref_ep_fine_id;
                dest_cl->cpi_ss.entry[ii].coarse[jj].pts_ep = src_cl->cpi_ss.entry[ii].coarse[jj].pts_ep;
                dest_cl->cpi_ss.entry[ii].coarse[jj].spn_ep = src_cl->cpi_ss.entry[ii].coarse[jj].spn_ep;
            }
            dest_cl->cpi_ss.entry[ii].fine = malloc(src_cl->cpi_ss.entry[ii].num_ep_fine * sizeof(CLPI_EP_FINE));
            for (jj = 0; jj < src_cl->cpi_ss.entry[ii].num_ep_fine; jj++) {
                dest_cl->cpi_ss.entry[ii].fine[jj].is_angle_change_point = src_cl->cpi_ss.entry[ii].fine[jj].is_angle_change_point;
                dest_cl->cpi_ss.entry[ii].fine[jj].i_end_position_offset = src_cl->cpi_ss.entry[ii].fine[jj].i_end_position_offset;
                dest_cl->cpi_ss.entry[ii].fine[jj].pts_ep = src_cl->cpi_ss.entry[ii].fine[jj].pts_ep;
                dest_cl->cpi_ss.entry[ii].fine[jj].spn_ep = src_cl->cpi_ss.entry[ii].fine[jj].spn_ep;
            }
        }
#endif
    }
