...
_parse_playitem(BITSTREAM *bits, MPLS_PI *pi)
{
    ...
    strncpy(pi->clip[0].clip_id, clip_id, sizeof(pi->clip[0].clip_id) - 1);
    strncpy(pi->clip[0].codec_id, codec_id, sizeof(pi->clip[0].codec_id) - 1);
...
_parse_subplayitem(BITSTREAM *bits, MPLS_SUB_PI *spi)
{
    ...
    strncpy(spi->clip[0].clip_id, clip_id, sizeof(spi->clip[0].clip_id) - 1);
    strncpy(spi->clip[0].codec_id, codec_id, sizeof(spi->clip[0].codec_id) - 1);
...
mpls_parse(const char *path)
{
    ...
    if (!pl) {
        ...
        strncpy(backup + len - 19, "BACKUP/", 7);
        strncpy(backup + len - 19 + 7, path + len - 19, 19);
