...
static int _filter_dup(MPLS_PL *pl_list[], unsigned count, MPLS_PL *pl)
{
    ...
    for (ii = 0; ii < count; ii++) {
        ...
        if (pl->ext_sub_count != pl_list[ii]->ext_sub_count) {
            continue;
        }
...
_find_repeats(MPLS_PL *pl, const char *m2ts, uint32_t in_time, uint32_t out_time)
{
    ...
    for (ii = 0; ii < pl->list_count; ii++) {
        ...
        if (strncmp(pi->clip[0].clip_id, m2ts,
                   strlen(pi->clip[0].clip_id) > strlen(m2ts) ?
                   strlen(pi->clip[0].clip_id) : strlen(m2ts)) == 0 &&
            pi->in_time  == in_time &&
...
#else
#define _fill_mark(title, mark, entry) BD_DEBUG(DBG_NAV, "Call at[%s,%d]\n", __FILE__, __LINE__), _fill_mark_(title, mark, entry)
static void
_fill_mark_(NAV_TITLE *title, NAV_MARK *mark, int entry)
#endif
{
    MPLS_PL *pl = title->pl;
    MPLS_PLM *plm;
...
_extrapolate_title(NAV_TITLE *title)
{
    ...
    for (ii = 0, jj = 0; ii < pl->mark_count; ii++) {
        plm = &pl->play_mark[ii];
        if (plm->mark_type == BD_MARK_ENTRY && jj < title->chap_list.count) {
...
NAV_TITLE* nav_title_open(const char *root, const char *playlist, unsigned angle)
{
    ...
    if (title->pl->ext_sub_count > 0) {
        ...
        BD_DEBUG(DBG_NAV, "ext_sub_path_count:%d\n", title->ext_sub_path_count);
        for (ss = 0; ss < title->ext_sub_path_count; ss++) {
            NAV_SUB_PATH *ext_sub_path = &title->ext_sub_path[ss];

            ext_sub_path->type            = title->pl->ext_sub_path[ss].type;
            ext_sub_path->clip_list.count = title->pl->ext_sub_path[ss].sub_playitem_count;
            ext_sub_path->clip_list.clip  = calloc(ext_sub_path->clip_list.count, sizeof(NAV_CLIP));

            BD_DEBUG(DBG_NAV, "ext_sub_path->clip_list.count:%d\n", ext_sub_path->clip_list.count);

            pos = time = 0;
            for (ii = 0; ii < ext_sub_path->clip_list.count; ii++) {
                MPLS_SUB_PI *pi   = &title->pl->ext_sub_path[ss].sub_play_item[ii];
                NAV_CLIP    *clip = &ext_sub_path->clip_list.clip[ii];
...
void nav_title_close(NAV_TITLE *title)
{
    ...
    for (ss = 0; ss < title->ext_sub_path_count; ss++) {
        for (ii = 0; ii < title->ext_sub_path[ss].clip_list.count; ii++) {
            clpi_free(title->ext_sub_path[ss].clip_list.clip[ii].cl);
        }
...
NAV_CLIP* nav_time_search(NAV_TITLE *title, uint32_t tick, uint32_t *clip_pkt, uint32_t *out_pkt)
{
    ...
    if (ii == title->pl->list_count) {
        ...
        *clip_pkt = clip->end_pkt;
        ...
        clip = &title->clip_list.clip[ii];
        if (clip->cl != NULL) {
            *clip_pkt = clpi_lookup_spn(clip->cl, tick - pos + pi->in_time, title->seekflag,
