...
static void _hdmv_trace_cmd(int pc, MOBJ_CMD *cmd)
{
    ...
    if (bd_get_debug_mask() & DBG_HDMV) {
        ...
        int len, sum = 0,size = sizeof(buf);

        len = snprintf(dst, (size - sum), "%04d:  ", pc);
        sum += len;
        if (sum  < size) {
            dst += len;
            len = mobj_sprint_cmd(dst, (size - sum), cmd);
            sum += len;
        }
	    else {
                BD_DEBUG(DBG_HDMV, "error, string len > buffer size\n");
        }
...
static void _hdmv_trace_res(uint32_t new_src, uint32_t new_dst, uint32_t orig_src, uint32_t orig_dst)
{
    ...
    if (bd_get_debug_mask() & DBG_HDMV) {
        ...
        if (new_dst != orig_dst || new_src != orig_src) {
            ...
            tmplen = snprintf(dst, sizeof(buf), "    :  [");
            dst += tmplen;
            sumlen += tmplen;

            if (new_dst != orig_dst) {
                tmplen = snprintf(dst, sizeof(buf) - sumlen, " dst 0x%x <== 0x%x ", orig_dst, new_dst);
                dst += tmplen;
                sumlen += tmplen;
            }
            if (new_src != orig_src) {
                tmplen = snprintf(dst, sizeof(buf) - sumlen, " src 0x%x <== 0x%x ", orig_src, new_src);
                dst += tmplen;
                sumlen += tmplen;
            }
            dst += snprintf(dst, sizeof(buf) - sumlen, "]");
