...
int ff_rtmp_calc_digest_pos(const uint8_t *buf, int off, int mod_val,
                            int add_val);
/**
 * rtmp send command to inform server whether send audio to client or not
 *
 * @param s URLContext which holds RTMPContext
 * @param receive true or false to indicate whether to receive audio or not.
 */
int ff_rtmp_receiveAudio(URLContext * s, int receive);

/**
 * rtmp send command to inform server whether send video to client or not
 *
 * @param s URLContext which holds RTMPContext
 * @param receive true or false to indicate whether to receive audio or not.
 */
int ff_rtmp_receiveVideo(URLContext * s, int receive);
