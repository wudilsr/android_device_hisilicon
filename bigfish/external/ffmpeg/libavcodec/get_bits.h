                                 ...
                                 int bit_size)
{
    ...
    /* changed: fix AVI file with VC1 can not play */
    if(NULL == buffer)
        return;
...
static inline int init_get_bits2(GetBitContext *s, const uint8_t *buffer,
                                int bit_size)
{
    int buffer_size;
    int ret = 0;
                                 ...
                                 int byte_size)
{
    if (byte_size > INT_MAX / 8 || byte_size < 0)
        byte_size = -1;
    return init_get_bits2(s, buffer, byte_size * 8);
}
...
{
    /* changed: fix NULL pointer in vc1 sometime */
    if (0 == table)
    {
        return -1;
    }
