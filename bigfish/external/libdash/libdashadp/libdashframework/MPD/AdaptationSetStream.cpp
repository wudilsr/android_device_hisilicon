...
RepresentationStreamType    AdaptationSetStream::DetermineRepresentationStreamType  (IRepresentation *representation)
{
    ...
    if (representation->GetSegmentTemplate() &&
        representation->GetSegmentTemplate()->Getmedia().length() > 0)
        return libdash::framework::mpd::SegmentTemplate;

    if (representation->GetSegmentBase() ||
        (representation->GetBaseURLs().size() > 0 &&
         representation->GetBaseURLs().at(0)->GetUrl().find(".") != std::string::npos))
...
void                        AdaptationSetStream::Initialize()
{
    for (size_t i = 0; i < adaptationSet->GetRepresentation().size(); i++)
    {
        ...
        if (type == UNDEFINED)
        {
            dash_log(DASH_LOG_ERROR, "[%s,%d] error representation[%d] type UNDEFINED\n", __FUNCTION__, __LINE__, i, type);
        }
    }
}
