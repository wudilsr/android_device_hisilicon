...
AVFormatContext*    LibavDecoder::OpenInput               ()
{
  ...
  //  int             err                 = 0;
    ...
    if (this->pb == NULL)
    {
        this->pb                            = avio_alloc_context(NULL, 0, 0, receiver, IORead, NULL, NULL);
        if (this->pb != NULL)
        {
            this->pb->seekable                        = 0;
        }
        ...
        {
            dash_log(DASH_LOG_ERROR, "[%s,%d] fatal error, alloc pb error!\n", __FUNCTION__, __LINE__);
        }
    }
...
AVIOContext* LibavDecoder::GetAVIOContextPtr()
{
    return this->pb;
}
