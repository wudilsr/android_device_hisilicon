...
dash::mpd::Descriptor*                      Node::ToDescriptor          ()  const
{
    ...
    for(size_t i = 0; i < subNodes.size(); i++)
    {
         if (subNodes.at(i)->GetName().find("mspr") != std::string::npos)
        {
            dash_log(DASH_LOG_INFO, "find mspr!\n");
            descriptor->AddMSPR(subNodes.at(i)->ToMSPR());
            continue;
        }
...
dash::mpd::MSPR*                            Node::ToMSPR                ()  const
{
    dash::mpd::MSPR *mspr = new dash::mpd::MSPR();

    if (this->GetName().find("pro") != std::string::npos)
    {
        dash_log(DASH_LOG_INFO, "set pro:%s\n", this->GetText().c_str());
        mspr->SetPRO(this->GetText());
    }
...
dash::mpd::MPD*                             Node::ToMPD                 ()  const
{
    ...
    std::vector<Node *> subNodes = this->GetSubNodes();
    dash::mpd::Period *lastPeriod, *thisPeriod;
    ...
    for(size_t i = 0; i < subNodes.size(); i++)
    {
        ...
        if (subNodes.at(i)->GetName() == "Period")
        {
            ...
            {
                lastPeriod = (dash::mpd::Period *)mpd->GetPeriods().at(mpd->GetPeriods().size() - 2);
                thisPeriod = (dash::mpd::Period *)mpd->GetPeriods().at(mpd->GetPeriods().size() - 1);
                if (lastPeriod->GetDuration().length() == 0 &&
                    lastPeriod->GetStart().length() > 0 &&
                    thisPeriod->GetStart().length() > 0)
                {
                    //set last period's duration
                    std::string lastDuration;
                    double lastStart = dash::helpers::String::timeToSeconds(lastPeriod->GetStart());
                    double thisStart = dash::helpers::String::timeToSeconds(thisPeriod->GetStart());
                    dash::helpers::String::secondsToTime(lastDuration, (thisStart - lastStart));
                    lastPeriod->SetDuration(lastDuration);
                    dash_log(DASH_LOG_INFO, "[%s,%d] set period[%d] duration=%s(%s)\n", __FUNCTION__, __LINE__, (mpd->GetPeriods().size() - 2), lastDuration.c_str(), lastPeriod->GetDuration().c_str());
                }
                else if (thisPeriod->GetStart().length() == 0 &&
                     lastPeriod->GetDuration().length() > 0 &&
                     lastPeriod->GetStart().length() > 0)
                {
                    //set this period's start
                    std::string thisStart;
                    double lastStart = dash::helpers::String::timeToSeconds(lastPeriod->GetStart());
                    double lastDur   = dash::helpers::String::timeToSeconds(lastPeriod->GetDuration());
                    dash::helpers::String::secondsToTime(thisStart, (lastStart + lastDur));
                    thisPeriod->SetStart(thisStart);
                    dash_log(DASH_LOG_INFO, "[%s,%d] set period[%d] start=%s(%s)\n", __FUNCTION__, __LINE__, (mpd->GetPeriods().size() - 1), thisStart.c_str(), thisPeriod->GetStart().c_str());
                }
