...
static int xmlTextReaderParseHiSiUrl(const char *HiSiURL, char *URL, int URLBufSize)
{
    char *urlEnd;
    int len;
    ...
    urlEnd = strstr(HiSiURL, HISIURL_END);
    if (urlEnd == NULL)
    {
        len = strlen(HiSiURL);
        if (len > 0)
...
xmlNewTextReaderFilename(const char *URI) {
    ...
    if (xmlTextReaderParseHiSiUrl(URI, realURI, sizeof(realURI)) < 0)
        return (NULL);

    input = xmlParserInputBufferCreateFilename(URI, XML_CHAR_ENCODING_NONE);
    if (input == NULL)
	return(NULL);
    ret = xmlNewTextReader(input, realURI);
    if (ret == NULL) {
    ...
    if (ret->ctxt->directory == NULL)
        directory = xmlParserGetDirectory(realURI);
