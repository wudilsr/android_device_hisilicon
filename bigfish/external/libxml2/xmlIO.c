...
xmlIOHTTPRead(void * context, char * buffer, int len) {
    ...
    ret = xmlNanoHTTPRead(context, &buffer[0], len);

    log_env = getenv(HIPLAYER_LOG_ENV);
    if (log_env != NULL)
    {
        if (0 == strncmp(log_env, HIPLAYER_ENV_LOG_LEVEL_INFO, strlen(HIPLAYER_ENV_LOG_LEVEL_INFO)) )
        {
            if (ret > 0 && buffer != NULL && len > 0)
                printf("%s", buffer);
            else if (ret == 0)
                printf("\n");
        }
