...
static unsigned int timeout = 15;//60;/* the select() timeout in seconds */
...
static int xmlNanoHTTPParseHiSiUrl(const char *HiSiURL, char *URL, int URLBufSize, char *headers, int headersBufSize)
{
    char *end, *hdrs;
    int len;

    if (HiSiURL == NULL || URL == NULL || URLBufSize <= 4)
        return -1;

    URL[0] = 0;
    if (headers != NULL && headersBufSize > 0)
        headers[0] = 0;

    end = strstr(HiSiURL, HISIURL_END);
    if (end == NULL)
    {
        len = strlen(HiSiURL);
        if (len > 0)
            memcpy(URL, HiSiURL, len);
        URL[len] = 0;
        return 0;
    }
    len = (end - HiSiURL);
    if (len >= URLBufSize)
    {
        printf("[%s,%d] error, url buffer size %d is smaller than %d\n", __FUNCTION__, __LINE__, URLBufSize, (len + 1));
        return -1;
    }
    memcpy(URL, HiSiURL, len);
    URL[len] = 0;
    ...
    if (headers == NULL || headersBufSize <= 2)
        return 0;
    headers[0] = 0;
    hdrs = strstr(HiSiURL, HISIURL_HTTPHEADERS);
    if (hdrs == NULL)
        return 0;
    hdrs += strlen(HISIURL_HTTPHEADERS);
    len = strlen(hdrs);
    if (len <= 0)
        return 0;

    if ((len) >= headersBufSize)
    {
        printf("[%s,%d] error, url buffer size %d is smaller than %d\n", __FUNCTION__, __LINE__, URLBufSize, (len + 1));
        return 0;
    }

    memcpy(headers, hdrs, len);
    headers[len] = 0;
...
void*
xmlNanoHTTPMethodRedir(const char *HiSiURL, const char *method, const char *input,
...
#endif
    char URL[2048] = {0};
    char headersInURL[2048] = {0};

    if (HiSiURL == NULL) return(NULL);

    printf("[%s,%d] HiSiURL:%s\n", __FUNCTION__, __LINE__, HiSiURL);
    if (xmlNanoHTTPParseHiSiUrl(HiSiURL, URL, sizeof(URL), headersInURL, sizeof(headersInURL)) < 0) {
        return (NULL);
    }

    if (headers != NULL) {
        snprintf((headersInURL + strlen(headersInURL)), (sizeof(headersInURL) - strlen(headersInURL)), "%s", headers);
    }

    if (strlen(headersInURL) > 0) {
        headers = headersInURL;
    }
...
retry:
	...
	ctxt->location = xmlMemStrdup(redirURL);
       printf("[%s,%d] start redirect %s, redirURL:%s\n", __FUNCTION__, __LINE__, (ctxt->location?ctxt->location:"NULL"), (redirURL?redirURL:"NULL"));
    }
    ...
    else if ((ctxt->returnValue < 200) || (ctxt->returnValue >= 400)) {
        printf("[%s,%d] error, xml http return code %d \n", __FUNCTION__, __LINE__, ctxt->returnValue);
	if (redirURL != NULL)
	    xmlFree(redirURL);
	  xmlNanoHTTPFreeCtxt(ctxt);
        return (NULL);
