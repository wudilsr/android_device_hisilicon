...
// add by c00190074
static int64_t be64decHiImpl(const char *data)
{
    int i;
    int64_t ret = 0;
    for (i=0; i < 8; i++)
...
static plist_t parse_unicode_node(char *bnode, uint64_t size)
{
    ...
    if (tmpstr == NULL)
    {
        perror("plist_utf16_to_utf8 return NULL");
        free(unicodestr);
        free(data);
        return NULL;
    }
...
void plist_from_bin(const char *plist_bin, uint32_t length, plist_t * plist)
{
    ...
    // FIXME c00190074 2012/09/15
    num_objects = be64dec(trailer + BPLIST_TRL_NUMOBJ_IDX);
//    num_objects = be64decHiImpl(trailer + BPLIST_TRL_NUMOBJ_IDX);
    root_object = be64dec(trailer + BPLIST_TRL_ROOTOBJ_IDX);
//    root_object = be64decHiImpl(trailer + BPLIST_TRL_ROOTOBJ_IDX);
    offset_table_index = be64dec(trailer + BPLIST_TRL_OFFTAB_IDX);
//    offset_table_index = be64decHiImpl(trailer + BPLIST_TRL_OFFTAB_IDX);
    ...
    for (i = 0; i < num_objects; i++)
    {
        ...
        case PLIST_DICT:
            for (j = 0; j < data->length; j++)
            {
                ...
                plist_data_t first_data = plist_get_data(nodeslist[index1]);
                if (first_data == NULL)
                {
                    perror("plist_get_data return NULL");
                    continue;
                }
                first_data->type = PLIST_KEY;
...
static void write_array(bytearray_t * bplist, node_t* node, hashtable_t* ref_table, uint8_t dict_param_size)
{
    ...
    {
        void *val = hash_table_lookup(ref_table, cur);
        if (val != NULL)
        {
            idx = *(uint64_t *) (val);
#if PLIST_BYTE_ORDER == PLIST_BIG_ENDIAN
...
static void write_dict(bytearray_t * bplist, node_t* node, hashtable_t* ref_table, uint8_t dict_param_size)
{
    ...
    idx1 = idx1 << ((sizeof(uint64_t) - dict_param_size) * 8);
            ...
            void *val2 = hash_table_lookup(ref_table, cur->next);
            if (val2 != NULL)
            {
                idx2 = *(uint64_t *) (val2);
#if PLIST_BYTE_ORDER == PLIST_BIG_ENDIAN
...
void plist_to_bin(plist_t plist, char **plist_bin, uint32_t * length)
{
    ...
    if (objects == NULL)
    {
        perror("ptr_array_new return NULL");
        return;
    }
    ...
    if (bplist_buff == NULL)
    {
        perror("byte_array_new return NULL");
    ...
    memset(offsets, 0, num_objects*sizeof(uint64_t));
    for (i = 0; i < num_objects; i++)
    {
