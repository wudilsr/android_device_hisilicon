...
static char *strndup(const char *__string, size_t __n)
{
    ...
    char *newstr = (char *)UPNP_MALLOC(strsize + 1);
...
UpnpString *UpnpString_dup(const UpnpString *p)
{
    ...
    q->m_string = UPNP_STRDUP(((struct SUpnpString *)p)->m_string);
    if (q->m_string == NULL) {
        goto error_handler2;
    }
...
{
    if (p != q) {
        (void)UpnpString_set_String(p, UpnpString_get_String(q));
    }
}
...
int UpnpString_set_String(UpnpString *p, const char *s)
{
    ...
    char *q = UPNP_STRDUP(s);
    if (!q) goto error_handler1;
    UPNP_FREE(((struct SUpnpString *)p)->m_string);
