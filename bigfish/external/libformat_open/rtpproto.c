...
#define RTP_READ_TIMEOUT (30000000) //30s

#define __FILE_NAME__  av_filename(__FILE__)
#define RTP_TRACE()    av_log(NULL, AV_LOG_INFO, "[%s:%d]\n", __FILE_NAME__, __LINE__)
...
static int rtp_open(URLContext *h, const char *uri, int flags)
{
    ...
    av_log(NULL, AV_LOG_INFO, "[%s:%d]: rtp_open(%s, %d)\n", __FILE_NAME__, __LINE__, uri, flags);
    ...
    av_log(NULL, AV_LOG_INFO, "[%s:%d]: build rtp url(%s, %d)\n", __FILE_NAME__, __LINE__, buf, flags);
    if (ffurl_open(&s->rtp_hd, buf, flags, &h->interrupt_callback, NULL) < 0) {
        av_log(0, AV_LOG_ERROR, "[%s,%d] ffurl_open return error!\n", __FUNCTION__, __LINE__);
        goto fail;
    }
    ...
    av_log(NULL, AV_LOG_INFO, "[%s:%d]: build rtcp url(%s, %d)\n", __FILE_NAME__, __LINE__, buf, flags);
...
static int rtp_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    for(;;) {
        if (ff_check_interrupt(&h->interrupt_callback)) {
            av_log(NULL, AV_LOG_ERROR, "[%s:%d]: Force exit\n", __FILE_NAME__, __LINE__);
            return AVERROR_EXIT;
        }

        /* timeout */
        if (RTP_READ_TIMEOUT < llabs(av_gettime() - time_start)) {
            av_log(NULL, AV_LOG_ERROR, "[%s,%d] RTP READ TIME OUT!\n",__FILE__, __LINE__);
        ...
        if (n > 0) {
            ...
            if (p[1].revents & POLLIN) {
                ...
                if (len < 0) {
                    ...
                    av_log(NULL, AV_LOG_ERROR, "[%s:%d]: rtcp read:0x%x \n", __FUNCTION__, __LINE__, len);
                    return AVERROR(EIO);
                }
                break;
            }
            ...
            if (p[0].revents & POLLIN) {
                ...
                if (len < 0) {
                    ...
                    av_log(NULL, AV_LOG_ERROR, "[%s:%d]: rtp read:0x%x \n", __FUNCTION__, __LINE__, len);
                    return AVERROR(EIO);
                }
                break;
            }
            ...
            av_log(NULL, AV_LOG_ERROR, "[%s:%d]: poll:0x%x \n", __FUNCTION__, __LINE__, ff_neterrno());
            return AVERROR(EIO);
        }
    }
    return len;
