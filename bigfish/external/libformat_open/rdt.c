...
//l00192899 add rtsp
enum IfParsedMltidata {
    MLTIDATA_ASMRUlEBOOk_NULL = 0,    /*ASMRUlEBOOk and OpaqueData:buffer not arrive */
    MLTIDATA_NULL = 1,                /*OpaqueData:buffer not arrive */
    MLTIDATA_PARSED  = 2,             /*both arrived*/
};
...
struct PayloadContext {
    ...
    enum IfParsedMltidata if_parsed_mltidata;  //l00192899 add rtsp
...
#define XOR_TABLE_SIZE 37
    ...
    /* add tail */
    av_strlcpy (response + 32, "01d0a8e3", strlen("01d0a8e3") + 1);
...
rdt_load_mdpr (PayloadContext *rdt, AVStream *st, int rule_nr)
{
    ...
    if (!rdt->mlti_data)
    {
        rdt->if_parsed_mltidata = MLTIDATA_NULL;    //l00192899 add rtsp
        return -1;
    }
    rdt->if_parsed_mltidata = MLTIDATA_PARSED;    //l00192899 add rtsp
...
#if 1
    /* l00192899 add start: if OpaqueData:buffer is later than ASMRuleBook:string*/
        if ((NULL!= rdt->mlti_data) && (MLTIDATA_NULL == rdt->if_parsed_mltidata))
        {
            int n, first = -1;
