                            ...
                            const int response_codes[], char **response)
{
    ...
    av_log(0, AV_LOG_ERROR, "[%s,%d] conn_control = %p, start to write cmd='%s'\n", __FUNCTION__, __LINE__, s->conn_control, command);
    if ((err = ffurl_write(s->conn_control, command, strlen(command))) < 0) {
        av_log(0, AV_LOG_ERROR, "[%s,%d] start to write cmd..error, err = %d, AVERROR_EOF=%d\n", __FUNCTION__, __LINE__,err, AVERROR_EOF);
        return err;
    }
    av_log(0, AV_LOG_ERROR, "[%s,%d] start to write cmd..OK, err = %d\n", __FUNCTION__, __LINE__,err);
    ...
    /* return status */
    if (response_codes) {
        err = ftp_status(s, response, response_codes);
        av_log(0, AV_LOG_ERROR, "[%s,%d] ftp_status err = %d\n", __FUNCTION__, __LINE__,err);
        return err;
    }
    return 0;
}
...
{
    av_log(0, AV_LOG_ERROR, "[%s,%d] close conn_control\n", __FUNCTION__, __LINE__);
    ffurl_close(s->conn_control);
    s->conn_control = NULL;
...
static int ftp_retrieve(FTPContext *s)
{
    ...
    ret = ftp_send_command(s, command, retr_codes, NULL);
    if (ret != 150 && ret != 125) {
        av_log(0, AV_LOG_ERROR, "[%s,%d] ftp_retrieve return %d\n", __FUNCTION__, __LINE__, ret);
        return AVERROR(EIO);
    }
...
static int ftp_abort(URLContext *h)
{
    ...
    */
    ...
    if (ftp_send_command(s, command, NULL, NULL) < 0) {
        ...
        ftp_close_data_connection(s);
        err = ftp_status(s, NULL, abor_codes);
        if (err  < 225) {
             av_log(0, AV_LOG_ERROR, "[%s,%d] close conn_control, err=%d\n", __FUNCTION__, __LINE__, err);
...
static int64_t ftp_seek(URLContext *h, int64_t pos, int whence)
{
    ...
    av_log(0, AV_LOG_INFO, "[%s,%d] ftp_seek to %lld, whence=%d\n",
    ...
    av_log(0, AV_LOG_INFO, "[%s,%d] ftp_seek to %lld, whence=%d OK\n",
        __FUNCTION__, __LINE__, pos, whence);
  ...
  retry:
    if (s->state == DISCONNECTED) {
        ...
        if (s->position >= s->filesize) {
            av_log(0, AV_LOG_WARNING, "[%s,%d] position %lld is > file size=%d\n", s->position, s->filesize);
            return 0;
        }
        if ((err = ftp_connect_data_connection(h)) < 0) {
            av_log(0, AV_LOG_WARNING, "[%s,%d] ftp_connect_data_connection return err=%d\n", err);
            return err;
        }
    }
    ...
    if (s->conn_data && s->state == DOWNLOADING) {
        ...
        if (read <= 0 && read != AVERROR_EXIT && read != AVERROR_EOF
            && s->position < s->filesize && !h->is_streamed) {
            /* Server closed connection. Probably due to inactivity */
            int64_t pos = s->position;
            av_log(h, AV_LOG_INFO, "Reconnect to FTP server, read=%d\n", read);
            if ((err = ftp_abort(h)) < 0)
                return err;
