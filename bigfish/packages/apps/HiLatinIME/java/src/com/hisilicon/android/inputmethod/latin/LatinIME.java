...
import com.hisilicon.android.inputmethod.accessibility.AccessibilityUtils;
import com.hisilicon.android.inputmethod.compat.CompatUtils;
import com.hisilicon.android.inputmethod.compat.EditorInfoCompatUtils;
import com.hisilicon.android.inputmethod.compat.InputConnectionCompatUtils;
import com.hisilicon.android.inputmethod.compat.InputMethodManagerCompatWrapper;
import com.hisilicon.android.inputmethod.compat.InputMethodServiceCompatWrapper;
import com.hisilicon.android.inputmethod.compat.InputTypeCompatUtils;
import com.hisilicon.android.inputmethod.compat.SuggestionSpanUtils;
import com.hisilicon.android.inputmethod.compat.VibratorCompatWrapper;
import com.hisilicon.android.inputmethod.deprecated.LanguageSwitcherProxy;
import com.hisilicon.android.inputmethod.deprecated.VoiceProxy;
import com.hisilicon.android.inputmethod.keyboard.Key;
import com.hisilicon.android.inputmethod.keyboard.Keyboard;
import com.hisilicon.android.inputmethod.keyboard.KeyboardActionListener;
import com.hisilicon.android.inputmethod.keyboard.KeyboardSwitcher;
import com.hisilicon.android.inputmethod.keyboard.KeyboardView;
import com.hisilicon.android.inputmethod.keyboard.LatinKeyboard;
import com.hisilicon.android.inputmethod.keyboard.LatinKeyboardView;
import com.hisilicon.android.inputmethod.latin.R;
 ...
 */
public class LatinIME extends InputMethodServiceCompatWrapper implements
		KeyboardActionListener,	SuggestionsView.Listener {
	 ...
	 *
	 * @deprecated Use {@link LatinIME#IME_OPTION_NO_MICROPHONE} with package
	 *	       name prefixed.
	 */
	...
	/**
	 * The subtype extra value used	to indicate that the subtype keyboard layout
	 * is capable for typing ASCII characters.
	 */
	public static final String SUBTYPE_EXTRA_VALUE_ASCII_CAPABLE = "AsciiCapable";

	/**
	 * The subtype extra value used	to indicate that the subtype keyboard layout
	 * supports touch position correction.
	 */
	public static final String SUBTYPE_EXTRA_VALUE_SUPPORT_TOUCH_POSITION_CORRECTION = "SupportTouchPositionCorrection";
	/**
	 * The subtype extra value used	to indicate that the subtype keyboard layout
	 * should be loaded from the specified locale.
	 */
	...
	private	static final int SUGGESTION_VISIBILILTY_SHOW_VALUE = R.string.prefs_suggestion_visibility_show_value;
	private	static final int SUGGESTION_VISIBILILTY_SHOW_ONLY_PORTRAIT_VALUE = R.string.prefs_suggestion_visibility_show_only_portrait_value;
	private	static final int SUGGESTION_VISIBILILTY_HIDE_VALUE = R.string.prefs_suggestion_visibility_hide_value;
	...
	// TODO: Create	an inner class to group	options	and pseudo-options to
	// improve readability.
	// These variables are initialized according to	the {@link
	...
	// Magic space:	a space	that should disappear on space/apostrophe insertion,
	// move	after the
	// punctuation on punctuation insertion, and become a real space on alpha
	// char	insertion.
	private	boolean	mJustAddedMagicSpace; // This indicates	whether	the last
											// char	is a magic space.
	// This	indicates whether the last keypress resulted in	processing of double
	// space replacement
	// with	period-space.
	...
	private	final ComposingStateManager mComposingStateManager = ComposingStateManager
			.getInstance();
	...
	public static class UIHandler extends StaticInnerHandlerWrapper<LatinIME> {
		...
		public void onCreate() {
			...
			mDelayBeforeFadeoutLanguageOnSpacebar =	res
					.getInteger(R.integer.config_delay_before_fadeout_language_on_spacebar);
			...
			mDurationOfFadeoutLanguageOnSpacebar = res
					.getInteger(R.integer.config_duration_of_fadeout_language_on_spacebar);
			mFinalFadeoutFactorOfLanguageOnSpacebar	= res
					.getInteger(R.integer.config_final_fadeout_percentage_of_language_on_spacebar) / 100.0f;
			mDoubleSpacesTurnIntoPeriodTimeout = res
					.getInteger(R.integer.config_double_spaces_turn_into_period_timeout);
			mIgnoreSpecialKeyTimeout = res
					.getInteger(R.integer.config_ignore_special_key_timeout);
		}
		...
		public void handleMessage(Message msg) {
			...
			case MSG_VOICE_RESULTS:
				latinIme.mVoiceProxy.handleVoiceResults(latinIme
			...
			case MSG_FADEOUT_LANGUAGE_ON_SPACEBAR:
				...
				sendMessageDelayed(
						obtainMessage(MSG_DISMISS_LANGUAGE_ON_SPACEBAR,	msg.obj),
		...
		public void startDisplayLanguageOnSpacebar(boolean localeChanged) {
			...
			final LatinKeyboardView	inputView = latinIme.mKeyboardSwitcher
					.getKeyboardView();
			if (inputView != null) {
				final LatinKeyboard keyboard = latinIme.mKeyboardSwitcher
						.getLatinKeyboard();
				...
				if (localeChanged && mDelayBeforeFadeoutLanguageOnSpacebar > 0)	{
					sendMessageDelayed(
							obtainMessage(MSG_FADEOUT_LANGUAGE_ON_SPACEBAR,
									keyboard),
		...
		public void startKeyTypedTimer() {
			...
			sendMessageDelayed(obtainMessage(MSG_KEY_TYPED),
					mIgnoreSpecialKeyTimeout);
		}
		...
		private	void executePendingImsCallback(LatinIME	latinIme,
	...
	public void onCreate() {
		...
		for (int i = 0;	i < Utils.GCUtils.GC_TRY_LOOP_MAX && tryGC; ++i) {
			try {
				...
				tryGC =	Utils.GCUtils.getInstance().tryGCOrWait("InitSuggest",
						e);
			}
		}
		...
		newDictFilter
				.addAction(DictionaryPackInstallBroadcastReceiver.NEW_DICTIONARY_INTENT_ACTION);
	...
	/* package */void loadSettings() {
		...
		if (null == mSubtypeSwitcher)
			mSubtypeSwitcher = SubtypeSwitcher.getInstance();
		mSettingsValues	= new Settings.Values(mPrefs, this,
	...
	private	void initSuggest() {
		...
		final Locale keyboardLocale = LocaleUtils
				.constructLocaleFromString(localeStr);

		final Resources	res = mResources;
		final Locale savedLocale = LocaleUtils.setSystemLocale(res,
				keyboardLocale);
		...
		mUserUnigramDictionary = new UserUnigramDictionary(this, this,
				localeStr, Suggest.DIC_USER_UNIGRAM);
		mSuggest.setUserUnigramDictionary(mUserUnigramDictionary);

		mUserBigramDictionary =	new UserBigramDictionary(this, this, localeStr,
				Suggest.DIC_USER_BIGRAM);
	 ...
	 * settings.
	 *
	 * This	method takes an	optional contacts dictionary to	use. Since the
	 * contacts dictionary does not	depend on the locale, it can be	reused
	 * across different instances of Suggest. The dictionary will also be opened
	 * or closed as	necessary depending on the settings.
	 *
	...
	private	void resetContactsDictionary(
		...
		if (!shouldSetDictionary) {
			...
			// Make	sure the old contacts dictionary is opened. If it is already
			// open, this is a
	...
	public void setInputView(View view) {
		...
		mSuggestionsView = (SuggestionsView) view
				.findViewById(R.id.suggestions_view);
	...
	private	void onStartInputViewInternal(EditorInfo attribute,
			boolean	restarting) {
		super.onStartInputView(attribute, restarting);
		final KeyboardSwitcher switcher	= mKeyboardSwitcher;
		inputView = switcher.getKeyboardView();

		if (DEBUG) {
			Log.d(TAG,
					"onStartInputView: attribute:"
							+ ((attribute == null) ? "none"	: String.format(
		...
		// Most	such things we decide below in
		// initializeInputAttributesAndGetMode,	but we need to
		// know	now whether this is a password text field, because we need to
		// know	now whether we
		// want	to enable the voice button.
		final VoiceProxy voiceIme = mVoiceProxy;
		final int inputType = (attribute != null) ? attribute.inputType	: 0;
		voiceIme.resetVoiceStates(InputTypeCompatUtils
		...
		setSuggestionStripShownInternal(isSuggestionsStripVisible(), /*	needsInputViewShown */
				false);
		// Delay updating suggestions because keyboard input view may not be
		// shown at this point.
		...
		if (TRACE)
			Debug.startMethodTracing("/data/trace/hilatinime");
		...
		if (inputClass == InputType.TYPE_CLASS_TEXT) {
			...
			if (InputTypeCompatUtils.isPasswordInputType(inputType)
					|| InputTypeCompatUtils
							.isVisiblePasswordInputType(inputType))	{
				mIsSettingsSuggestionStripOn = false;
			}
			if (InputTypeCompatUtils.isEmailVariation(variation)
					|| variation ==	InputType.TYPE_TEXT_VARIATION_PERSON_NAME) {
				// The point in	turning	this off is that we don't want to insert
				// a space after
				// a name when filling a form: we can't	delete trailing	spaces
	...
	private	void onFinishInputInternal() {
		...
		if (inputView != null)
			inputView.closing();
		if (mUserUnigramDictionary != null)
			mUserUnigramDictionary.flushPendingWrites();
	...
	public void onUpdateSelection(int oldSelStart, int oldSelEnd,
		...
		if (DEBUG) {
			Log.i(TAG, "onUpdateSelection: oss=" + oldSelStart + ",	ose="
					...
					+ newSelEnd + ", cs=" +	candidatesStart	+ ", ce="
					+ candidatesEnd);
		}
		...
		final boolean selectionChanged = (newSelStart != candidatesEnd || newSelEnd != candidatesEnd)
				&& mLastSelectionStart != newSelStart;
		final boolean candidatesCleared	= candidatesStart == -1
				&& candidatesEnd == -1;
		if (!mExpectingUpdateSelection)	{
			if (((mComposingStringBuilder.length() > 0 && mHasUncommittedTypedChars) || mVoiceProxy
	...
	public void onDisplayCompletions(
			CompletionInfo[] applicationSpecifiedCompletions) {
		if (DEBUG) {
		...
		if (mApplicationSpecifiedCompletionOn) {
			...
			SuggestedWords.Builder builder = new SuggestedWords.Builder()
					.setApplicationSpecifiedCompletions(
							applicationSpecifiedCompletions)
					.setTypedWordValid(false).setHasMinimalSuggestion(false);
	...
	private	void setSuggestionStripShownInternal(boolean shown,
			boolean	needsInputViewShown) {
		// TODO: Modify	this if	we support suggestions with hard keyboard
		if (onEvaluateInputViewShown() && mSuggestionsContainer	!= null) {
			final boolean shouldShowSuggestions = shown
					&& (needsInputViewShown	? mKeyboardSwitcher
	...
	public void onComputeInsets(InputMethodService.Insets outInsets) {
		...
		if (mKeyboardSwitcher.isInputViewShown()) {
			...
			if (DEBUG) {
				Log.d(TAG, "Touchable region: y=" + touchY + " width="
						+ touchWidth + " height=" + touchHeight);
			}
			setTouchableRegionCompat(outInsets, 0, touchY, touchWidth,
					touchHeight);
		}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		case KeyEvent.KEYCODE_BACK:
			if (event.getRepeatCount() == 0) {
				...
				final LatinKeyboardView	keyboardView = mKeyboardSwitcher
						.getKeyboardView();
				...
				if (nLastKeyIndex >= nCurKeyboardKeyNums - 1) {
					...
					for (int i = 0;	i < nearestKeyIndices.length; i++) {
						...
						if (nLastKeyIndex < index) {
							...
							// ensure not bound out	of softkeyboard
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		...
		case KeyEvent.KEYCODE_DPAD_RIGHT:
			...
			if (mKeyboardSwitcher.isInputViewShown()
				...
				KeyEvent newEvent = new	KeyEvent(event.getDownTime(),
		...
		if (inputView != null && hideFlag) {
			hideFlag = false;
			return true;
		} else if (inputView !=	null &&	inputView.isShown()
	...
	private	void onSettingsKeyPressed() {
		...
		} else if (Utils.hasMultipleEnabledIMEsOrSubtypes(mImm,	false /*
																	 * exclude
																	 * aux
																	 * subtypes
																	 */)) {
	...
	// Virtual codes representing custom requests. These are used in
	// onCustomRequest() below.
	...
	public boolean onCustomRequest(int requestCode)	{
		...
		case CODE_SHOW_INPUT_METHOD_PICKER:
			if (Utils.hasMultipleEnabledIMEsOrSubtypes(mImm, true /*
																 * include aux
																 * subtypes
																 */)) {
	...
	public void onCodeInput(int primaryCode, int[] keyCodes, int x,	int y) {
		...
		if (primaryCode	!= Keyboard.CODE_DELETE
		...
		case Keyboard.CODE_DELETE:
			...
			break;
		case Keyboard.CODE_DEL:
			handleBackspace_del(lastStateOfJustReplacedDoubleSpace);
			mDeleteCount++;
			mExpectingUpdateSelection = true;
			LatinImeLogger.logOnDelete();
			break;
		...
		case Keyboard.CODE_TAB:
			handleTab();
			// There are two cases for tab.	Either we send a "next"	event, that
			// may change the
			// focus but will never	move the cursor. Or, we	send a real tab
			// keycode, which some
			// applications	may accept or ignore, and we don't know	whether	this
			// will	move the
			// cursor or not. So actually, we don't	really know.
			// So to go with the safer option, we'd	rather behave as if the	user
			// moved the
			// cursor when they didn't than	the opposite. We also expect that
			// most	applications
	...
	private	void handleBackspace(boolean justReplacedDoubleSpace) {
		...
		if (mHasUncommittedTypedChars) {
			...
			if (length > 0)	{
				...
				final CharSequence textWithUnderline = mComposingStateManager
						.isAutoCorrectionIndicatorOn() ? SuggestionSpanUtils
						.getTextWithAutoCorrectionIndicatorUnderline(this,
				...
				if (1 == length) {
					// 1 ==	length means we	are about to erase the last
					// character of	the word,
					// so we can show bigrams.
					mHandler.postUpdateBigramPredictions();
				} else {
					// length > 1, so we still have	letters	to deduce a
					// suggestion from.
					mHandler.postUpdateSuggestions();
				}
			} else {
		...
		if (mEnteredText != null && sameAsTextBeforeCursor(ic, mEnteredText)) {
			...
			if (mSuggestionsView !=	null
					&& mSuggestionsView.dismissAddToDictionaryHint()) {
	...
	// BEGIN: Delete button when synchronous delete tip
	private void handleBackspace_del(boolean justReplacedDoubleSpace) {
		if (mVoiceProxy.logAndRevertVoiceInput())
			return;
		mVoiceProxy.handleBackspace();
		...
		final boolean deleteChar = !mHasUncommittedTypedChars;
		if (mHasUncommittedTypedChars) {
			Log.d(TAG, "mHasUncommittedTypedChars=" + mHasUncommittedTypedChars);
			final int length = mComposingStringBuilder.length();
	...
	private	void handleTab() {
		...
		// True	if keyboard is in either chording shift	or manual temporary
		// upper case mode.
		final boolean isManualTemporaryUpperCase = mKeyboardSwitcher
	...
	private	void handleCharacter(int primaryCode, int[] keyCodes, int x, int y) {
		...
		if ((isAlphabet(code) || mSettingsValues
				.isSymbolExcludedFromWordSeparators(code))
		...
		if (switcher.isShiftedOrShiftLocked()) {
			...
			if (switcher.isAlphabetMode() && Character.isLowerCase(code)) {
				// In some locales, such as Turkish, Character.toUpperCase() may
				// return a wrong
				// character because it	doesn't	take care of locale.
				final String upperCaseString = new String(new int[] { code },
						0, 1).toUpperCase(mSubtypeSwitcher.getInputLocale());
		...
		if (LatinIME.PERF_DEBUG)
			measureCps();
		TextEntryState.typedCharacter((char) code,
				mSettingsValues.isWordSeparator(code), x, y);
	}
	...
	public boolean isShowingPunctuationList() {
		if (mSuggestionsView ==	null)
			return false;
		return mSettingsValues.mSuggestPuncList	== mSuggestionsView
				.getSuggestions();
	}

	public boolean isShowingSuggestionsStrip() {
		return (mSuggestionVisibility == SUGGESTION_VISIBILILTY_SHOW_VALUE)
				|| (mSuggestionVisibility == SUGGESTION_VISIBILILTY_SHOW_ONLY_PORTRAIT_VALUE &&	mDisplayOrientation == Configuration.ORIENTATION_PORTRAIT);
	}
	...
	public void setSuggestions(SuggestedWords words) {
		if (mSuggestionsView !=	null) {
			...
			mKeyboardSwitcher.onAutoCorrectionStateChanged(words
					.hasWordAboveAutoCorrectionScoreThreshold());
		}
		...
		final InputConnection ic = getCurrentInputConnection();
		if (ic != null)	{
			final boolean oldAutoCorrectionIndicator = mComposingStateManager
					.isAutoCorrectionIndicatorOn();
			final boolean newAutoCorrectionIndicator = Utils
					.willAutoCorrect(words);
			if (oldAutoCorrectionIndicator != newAutoCorrectionIndicator) {
				if (LatinImeLogger.sDBG) {
					Log.d(TAG, "Flip the indicator.	"
							+ oldAutoCorrectionIndicator + " -> "
							+ newAutoCorrectionIndicator);
				}
				final CharSequence textWithUnderline = newAutoCorrectionIndicator ? SuggestionSpanUtils
						.getTextWithAutoCorrectionIndicatorUnderline(this,
								mComposingStringBuilder)
						: mComposingStringBuilder;
	...
	public void updateSuggestions()	{
		...
		if (null == ic)	{
			...
			prevWord = EditingUtils.getPreviousWord(ic,
					mSettingsValues.mWordSeparators);
		}
		// getSuggestedWordBuilder handles gracefully a	null value of prevWord
		final SuggestedWords.Builder builder = mSuggest
				.getSuggestedWordBuilder(wordComposer, prevWord,
						mKeyboardSwitcher.getLatinKeyboard().getProximityInfo());

		boolean	autoCorrectionAvailable	= !mInputTypeNoAutoCorrect
				&& mSuggest.hasAutoCorrection();
		final CharSequence typedWord = wordComposer.getTypedWord();
		// Here, we want to promote a whitelisted word if exists.
		// TODO: Change	this scheme - a	boolean	is not enough. A whitelisted
		// word	may be "valid"
		// but still autocorrected from	- in the case the whitelist only
		// capitalizes the word.
		// The whitelist should	be case-insensitive, so	it's not possible to be
		// consistent with
		// a boolean flag. Right now this is handled with a slight hack	in
		// WhitelistDictionary#shouldForciblyAutoCorrectFrom.
		final boolean allowsToBeAutoCorrected =	AutoCorrection
				.allowsToBeAutoCorrected(mSuggest.getUnigramDictionaries(),
						typedWord, preferCapitalization());
		...
		// Basically, we update	the suggestion strip only when suggestion count
		// > 1.	However,
		// there is an exception: We update the	suggestion strip whenever typed
		// word's length
		// is 1	or typed word is found in dictionary, regardless of suggestion
		// count. Actually,
		// in most cases, suggestion count is 1	when typed word's length is 1,
		// but we do always
		// need	to clear the previous state when the user starts typing	a word
		// (i.e. typed word's
		// length == 1).
		if (typedWord != null) {
			if (builder.size() > 1 || typedWord.length() ==	1
					|| (!allowsToBeAutoCorrected)
					|| mSuggestionsView.isShowingAddToDictionaryHint()) {
				builder.setTypedWordValid(!allowsToBeAutoCorrected)
						.setHasMinimalSuggestion(autoCorrectionAvailable);
			} else {
				SuggestedWords previousSuggestions = mSuggestionsView
						.getSuggestions();
				...
				builder.addTypedWordAndPreviousSuggestions(typedWord,
						previousSuggestions);
			}
		}
	...
	public void showSuggestions(SuggestedWords suggestedWords,
			CharSequence typedWord)	{
		final boolean shouldBlockAutoCorrectionBySafetyNet = Utils
				.shouldBlockAutoCorrectionBySafetyNet(suggestedWords, mSuggest);
	...
	private	boolean	pickDefaultSuggestion(int separatorCode) {
		...
		if (mBestWord != null && mBestWord.length() > 0) {
			TextEntryState.acceptedDefault(mWordComposer.getTypedWord(),
					mBestWord, separatorCode);
			mExpectingUpdateSelection = true;
			commitBestWord(mBestWord);
			// Add the word	to the user unigram dictionary if it's not a known
			// word
	...
	public void pickSuggestionManually(int index, CharSequence suggestion) {
		...
		if (suggestion.length()	== 1
				&& (mSettingsValues.isWordSeparator(suggestion.charAt(0)) || mSettingsValues
			...
			// Find	out whether the	previous character is a	space. If it is, as
			// a special case
			// for punctuation entered through the suggestion strip, it should
			// be considered
			// a magic space even if it was	a normal space.	This is	meant to
			// help	in case	the user
			// pressed space on purpose of displaying the suggestion strip
			// punctuation.
			final int rawPrimaryCode = suggestion.charAt(0);
			// Maybe apply the "bidi mirrored" conversions for parentheses
			final LatinKeyboard keyboard = mKeyboardSwitcher.getLatinKeyboard();
			final boolean isRtl = keyboard != null && keyboard.mIsRtlKeyboard;
			final int primaryCode =	Key.getRtlParenthesisCode(rawPrimaryCode,
					isRtl);

			final CharSequence beforeText =	ic != null ? ic
					.getTextBeforeCursor(1,	0) : "";
			final int toLeft = (ic == null || TextUtils.isEmpty(beforeText)) ? 0
					: beforeText.charAt(0);
		...
		if (!mHasUncommittedTypedChars)	{
			// If we are not composing a word, then	it was a suggestion inferred
			// from
		...
		LatinImeLogger.logOnManualSuggestion(
				mComposingStringBuilder.toString(), suggestion.toString(),
				index, suggestions.mWords);
		TextEntryState.acceptedSuggestion(mComposingStringBuilder.toString(),
				suggestion);
		...
		// first entry
		// AND either:
		// - There is no dictionary (we	know that because we tried to load it =>
		// null	!= mSuggest
		...
		// We used to look at mCorrectionMode here, but	showing	the hint should
		// have	nothing
		...
		final boolean showingAddToDictionaryHint = index == 0
				...
				// If "suggestion" is not in the dictionary, the hint should be
				// shown.
		...
		if (!showingAddToDictionaryHint) {
			...
			// Updating the	predictions right away may be slow and feel
			// unresponsive	on slower
			// terminals. On the other hand	if we just
			// postUpdateBigramPredictions() it will
	...
	private	void commitBestWord(CharSequence bestWord) {
		...
		final InputConnection ic = getCurrentInputConnection();
		if (ic != null)	{
			mVoiceProxy.rememberReplacedWord(bestWord,
					mSettingsValues.mWordSeparators);
			if (mSettingsValues.mEnableSuggestionSpanInsertion) {
				final SuggestedWords suggestedWords = mSuggestionsView
						.getSuggestions();
	...
	public void updateBigramPredictions() {
		...
		final CharSequence prevWord = EditingUtils.getThisWord(
				getCurrentInputConnection(), mSettingsValues.mWordSeparators);
		SuggestedWords.Builder builder = mSuggest.getSuggestedWordBuilder(
				sEmptyWordComposer, prevWord, mKeyboardSwitcher
						.getLatinKeyboard().getProximityInfo());

		if (builder.size() > 0)	{
			// Explicitly supply an	empty typed word (the no-second-arg version
			// of
	...
	private	void addToOnlyBigramDictionary(CharSequence suggestion,
			int frequencyDelta) {
	...
	private	void checkAddToDictionary(CharSequence suggestion,
		...
		if (null != mSuggest &&	null !=	mUserUnigramDictionary)	{
			final boolean selectedATypedWordAndItsInUserUnigramDic = !selectedANotTypedWord
					&& mUserUnigramDictionary.isValidWord(suggestion);
			...
			if (needsToAddToUserUnigramDictionary) {
				mUserUnigramDictionary.addWord(suggestion.toString(),
						frequencyDelta);
			}
		}

		if (mUserBigramDictionary != null) {
			// We don't want to register as	bigrams	words separated	by a
			// separator.
			// For example "I will,	and you	too" : we don't	want the pair
			// ("will" "and") to be
			// a bigram.
			final InputConnection ic = getCurrentInputConnection();
			if (null != ic)	{
				final CharSequence prevWord = EditingUtils.getPreviousWord(ic,
						mSettingsValues.mWordSeparators);
	...
	// "ic"	must not null
	private	boolean	sameAsTextBeforeCursor(final InputConnection ic,
			CharSequence text) {
	...
	private	void revertLastWord(final InputConnection ic) {
		...
		ic.deleteSurroundingText(1, 0);
		final CharSequence textToTheLeft = ic.getTextBeforeCursor(
				mCommittedLength, 0);
		ic.deleteSurroundingText(mCommittedLength, 0);

		// Re-insert "separator" only when the deleted character was word
		// separator and the
		// composing text wasn't equal to the auto-corrected text which	can be
		// found before
		// the cursor.
	...
	// Notify that language	or mode	have been changed and toggleLanguage will
	// update KeyboardID
	// according to	new language or	mode.
	public void onRefreshKeyboard()	{
		if (!CAN_HANDLE_ON_CURRENT_INPUT_METHOD_SUBTYPE_CHANGED) {
			// Before Honeycomb, Voice IME is in LatinIME and it changes the
			// current input view,
			// so that we need to re-create	the keyboard input view	here.
			setInputView(mKeyboardSwitcher.onCreateInputView());
		}
		// When	the device locale is changed in	SetupWizard etc., this method
		// may get called via
	...
	private	void updateKeypressVibrationDuration() {
		mKeypressVibrationDuration = Utils.getCurrentVibrationDuration(mPrefs,
				mResources);
	}
	...
	private	void updateCorrectionMode() {
		...
		final boolean shouldAutoCorrect	= mSettingsValues.mAutoCorrectEnabled
				&& !mInputTypeNoAutoCorrect;
		mCorrectionMode	= (shouldAutoCorrect &&	mSettingsValues.mAutoCorrectEnabled) ? Suggest.CORRECTION_FULL
				: (shouldAutoCorrect ? Suggest.CORRECTION_BASIC
						: Suggest.CORRECTION_NONE);
		mCorrectionMode	= (mSettingsValues.mBigramSuggestionEnabled
				&& shouldAutoCorrect &&	mSettingsValues.mAutoCorrectEnabled) ? Suggest.CORRECTION_FULL_BIGRAM
				: mCorrectionMode;
		if (mSuggest !=	null) {
	...
	private	void updateSuggestionVisibility(final SharedPreferences	prefs,
			final Resources	res) {
		final String suggestionVisiblityStr = prefs
				.getString(
	...
	protected void launchSettingsClass(
	...
	protected void dump(FileDescriptor fd, PrintWriter fout, String[] args)	{
		...
		p.println("  mComposingStringBuilder="
				+ mComposingStringBuilder.toString());
		p.println("  mIsSuggestionsRequested=" + mIsSettingsSuggestionStripOn);
		p.println("  mCorrectionMode=" + mCorrectionMode);
		p.println("  mHasUncommittedTypedChars=" + mHasUncommittedTypedChars);
		p.println("  mAutoCorrectEnabled="
	...
	// BEGING: add Keyboard	focus move
	int nCurKeyboardKeyNums;
	Keyboard nCurrentKeyboard;
	List<Key> nKeys;
	int nLastKeyIndex = 0;
	List<CharSequence> nSuggestionsBuffer;
	private	boolean	hideFlag = false;
	private	boolean	downFlag = false;
	int nCandidateCursorPosition;

	private	void setFields() {
		if (null == inputView) {
			return;
		} else {
			nCurrentKeyboard = inputView.getKeyboard();
			nKeys =	nCurrentKeyboard.mKeys;
			nCurKeyboardKeyNums = nKeys.size();
			nLastKeyIndex =	inputView.getLastKeyIndex();
		}
	}

	public void initializeFields() {
		if (inputView != null && isInputViewShown() && isExtractViewShown()) {
			nCandidateCursorPosition = 0;
			nSuggestionsBuffer = null;
		}
