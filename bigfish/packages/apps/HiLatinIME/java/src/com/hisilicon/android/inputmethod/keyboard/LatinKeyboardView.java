...
package com.hisilicon.android.inputmethod.keyboard;
...
import com.hisilicon.android.inputmethod.accessibility.AccessibleKeyboardViewProxy;
import com.hisilicon.android.inputmethod.deprecated.VoiceProxy;
import com.hisilicon.android.inputmethod.keyboard.PointerTracker.DrawingProxy;
import com.hisilicon.android.inputmethod.keyboard.PointerTracker.TimerProxy;
import com.hisilicon.android.inputmethod.latin.LatinIME;
import com.hisilicon.android.inputmethod.latin.R;
import com.hisilicon.android.inputmethod.latin.StaticInnerHandlerWrapper;
import com.hisilicon.android.inputmethod.latin.Utils;
...
public class LatinKeyboardView extends KeyboardView implements PointerTracker.KeyEventHandler,
    ...
    public LatinKeyboardView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mHasDistinctMultitouch = false;//context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_TOUCHSCREEN_MULTITOUCH_DISTINCT);
	...
	private boolean moveCursorToTouchedKey(MotionEvent me) {
		float touchX = me.getX();
		float touchY = me.getY();
		if (keys != null && keys.size() != 0) {
			for (int i = 0; i < keys.size(); i++) {
				int keyLeftX = keys.get(i).mX;
				int keyLeftY = keys.get(i).mY;
				int keyRightX = keyLeftX + keys.get(i).mWidth;
				int keyRightY = keyLeftY + keys.get(i).mHeight;
				// touch position within key square
                if (touchX + 1 >= keyLeftX && touchX + 1 <= keyRightX
                        && touchY - 11 >= keyLeftY && touchY - 11<= keyRightY) {
			lastKeyIndex = i;
			focusFlag = true;
