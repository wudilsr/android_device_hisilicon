...
import com.hisilicon.android.mediacenter.common.MyDialog;
import com.hisilicon.android.mediacenter.common.NewCreateDialog;
import com.hisilicon.android.mediacenter.ftp.DBHelper;
...
public class SambaActivity extends CommonActivity implements Runnable {
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (splitedPath.length == 1) {
			...
			musicPathItem = suboper.add(Menu.NONE, MENU_DEFAULT_PATH, 0,
					getResources().getString(R.string.default_path));
            ...
            if (listFile.size() == 0) {
                if (num == 0) {
					...
					musicPathItem.setEnabled(false);
                }
                menu.getItem(2).setEnabled(false);
            } else {
                if (num == 0) {
...
//            li = adapter.getFiles();
     ...
     * CNcomment:对菜单操作进行细化操作： 对文件操作 “粘贴”，“重命名”及“删除” 对服务器操作
     */
    ...
    /*
     * search samba server
     * CNcomment:搜索samba服务器
     */
    void searchServers() {
        //BEGIN : ADD FOR NOT FIND THE SAMBA SERVER
    	File file = new File("/data/app/samba/var/locks/gencache.tdb");
    	if(file.exists())
    	file.delete();

    	File file1 = new File("/data/app/samba/var/locks/gencache_notrans.tdb");
    	if(file1.exists())
    	file1.delete();
    ...
    private void showSetNewMountDlg(Map<String, Object> clickedNetServer,
        ...
        // while the net is error ,show the login dialog
        if(hasTheShortCut()) {
